...
# ⚠️ 2026-08-10：原先这里有一道门④，断言"装载时用的那张表"与"权威白名单"两者相等，
#    实现靠 AST 解析方法源码去猜有没有人改过局部那份表。**外审连续三轮攻破它**
#    （`|=` 增强赋值 / 通过别名 mutate / 用普通赋值顶替类名……写法枚举不完）。
#    根子是同一件事存了两份。**已把两份合成一份**
#    （`_MOCK_FALLBACK_BY_CATEGORY` 是唯一定义，`_MOCK_DRIVER_CLASSES` 从它派生），
#    "两份分叉"这件事不复存在，这道门连同那段 AST 解析一起删掉。
#    —— 删掉 > 换地方取数 > 收紧 > 新加一套东西。
