...
def test_formal_failure_is_counted_once_across_alert_lifecycle(db):
    ...
    assert alerts.emit_execution_failed_alert(execution.id) is True
    ...
    assert alerts.emit_execution_failed_alert(execution.id) is False
    assert db.query(Alert).count() == 1
    assert get_alert_summary(db).total_active == 0


@pytest.mark.parametrize("status,source,validation_pass", [
    ("completed", "test_case_runner", False),
    ("failed", "commissioning_adhoc", None),
    ("failed", "test_plan_runner", None),
    ("failed", "vrt", None),
])
def test_non_system_failure_sources_do_not_create_alerts(
    db, status, source, validation_pass,
):
...
def test_alert_commit_failure_does_not_rollback_execution_terminal_state(
    ...
    assert alerts.emit_execution_failed_alert(execution_id) is False
    db.expire_all()
