"""
Keysight MXG/EXG Signal Generator Driver
========================================

Real HAL Driver for Keysight X-Series Signal Generators.
"""
...
class MxgScpi:
    ...
    SET_FREQ = "SOURce:FREQuency:CW {freq}"
    SET_POW = "SOURce:POWer:LEVel {power}"
    OUTP_ON = "OUTPut:STATe ON"
    OUTP_OFF = "OUTPut:STATe OFF"


class RealKeysightMxgDriver(SignalGeneratorDriver):
    """Real HAL for Keysight MXG/EXG SigGen"""

    def __init__(self, instrument_id: str, config: Dict[str, Any]):
        super().__init__(instrument_id, config)
        self.ip_address: str = config.get("ip", "192.168.100.50")
        self._visa_rm = None
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            idn = self._query(MxgScpi.IDN).strip()
            logger.info(f"[MXG] Connected to {idn}")
            self._set_status(InstrumentStatus.CONNECTED)
            return True
        except Exception as e:
            logger.error(f"[MXG] Connection failed: {e}")
    ...
    async def set_cw(self, frequency_hz: float, power_dbm: float) -> bool:
        try:
            self._write(MxgScpi.SET_FREQ.format(freq=frequency_hz))
            self._write(MxgScpi.SET_POW.format(power=power_dbm))
            self._query(MxgScpi.OPC)
            return True
        except Exception as e:
            logger.error(f"[MXG] set cw failed: {e}")
            return False
