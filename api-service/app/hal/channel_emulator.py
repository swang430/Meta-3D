...
支持哪些模式由各驱动的 `adapter_manifest` 声明（P2-57 起），
`get_supported_load_modes()` 只是它的派生读法 —— 子类不得重写（有门守）。
"""
...
class ChannelEmulatorDriver(InstrumentDriver):
    ...
    信道加载架构:
        ...
        支持哪些模式由各驱动的 `adapter_manifest` 声明，不由子类各自硬写。
        - 支持哪些模式**由各驱动的 manifest 声明**，不存在「所有仿真器必须支持」
          的共同基线（该断言曾写在这里，实况是 FS16 从未实现 .asc 上传播放）
    ...
    def get_supported_load_modes(self) -> List[ChannelLoadMode]:
        """该仿真器支持的信道加载模式 —— **从 manifest 派生，不各自硬写**。

        ⚠️ P2-57 之前这里的默认实现是 `return [EXTERNAL_WAVEFORM]`，
        于是任何不重写它的驱动都**静默宣称**支持 .asc 上传播放。那是
        fail-open：FS16 正是这样宣称了一个它从未实现的能力
        （`upload_asc_files` 调用抛不受控的 `AttributeError`）。

        现在改成 fail-closed：没有 `adapter_manifest` 的驱动**不宣称任何模式**。
        新增型号必须显式声明才能被选中，不再继承一个假的共同基线。
        """

        # ⚠️ 与 `channel_emulator_implements` / `channel_emulator_rejection`
        #    共用取数函数。此前这里自己写 `getattr(type(self), ...)`，是第三处
        #    各写一遍 —— 内审 R2 实证：同一个对象在三处能得到不同答案。
        manifest = channel_emulator_manifest_of(self)
        if manifest is None:
            return []
        return [ChannelLoadMode(mode) for mode in manifest.supported_load_modes()]
    ...
    # ==================================================================
    # 抽象操作接口（子类必须自己实现；未实现即 NotImplementedError）
    #
    # ⚠️ P2-57：以下 14 个方法**曾经整段掉在类体之外** —— 它们被嵌在模块级
    #    函数 `normalize_channel_model_entries` 内部（自 2026-05-13），
    #    于是每次那个函数运行时被定义成局部函数，**永远不是驱动方法**。
    #    实证：`hasattr(ChannelEmulatorDriver, "upload_asc_files")` 曾为 False，
    #    FS16 调用它们抛的是不受控的 `AttributeError` 而非 `NotImplementedError`。
    #    F64 与 Mock 各自实现了全部 14 个，把这个结构缺陷完全掩盖 —— 测试恒绿。
    #    守它的是 `test_p2_57_channel_emulator_manifest.py` 的类体完整性不变量门。
    # ==================================================================
