...
class ProbePathLossCalibrationService:
    ...
    async def start_calibration(
        ...
        except Exception as e:  # noqa: BLE001 - 外层租约失败不许换形状
            logger.error("Path-loss calibration job lease failed: %s", e)
            # Codex #359 R1 P2: 外层租约的取/放失败走既有结果路径 —— 保住 message 与
            # 已累计的 warnings（含收割到的清理失败），不让全局 handler 的 409 把它们
            # 换成另一种形状。证书照旧不落库。
            self._harvest_acquire_warnings(warnings, "job lease")
            return CalibrationResult(
                success=False,
                message=f"Instrument lease failed for path-loss calibration: {e}",
                warnings=warnings,
            )
        # 计算统计数据
    ...
    async def start_calibration_for_lab_profile(
        ...
        try:
            ...
            logger.error("Path-loss lab-profile calibration job lease failed: %s", e)
            # Codex #359 R1 P2: 同 start_calibration —— 外层租约失败走既有结果路径。
            self._harvest_acquire_warnings(warnings, "job lease")
            return CalibrationResult(
                success=False,
                message=f"Instrument lease failed for path-loss calibration: {e}",
...
class MultiFrequencyPathLossService:
    ...
    async def calibrate_frequency_sweep(
        ...
        try:
            ...
            logger.error("Multi-freq calibration job lease failed: %s", e)
            # Codex #359 R1 P2: 同 start_calibration —— 外层租约失败走既有结果路径。
            return CalibrationResult(
                success=False,
                message=f"Instrument lease failed for multi-frequency calibration: {e}",
                warnings=warnings,
            )
        self.db.commit()
