"""Seed the canonical test-sequence library.

Bundles 15 reusable test-step sequences covering Sub-6 / mmWave TRP/TIS,
MIMO channel modeling, calibration, 5G NR, and the 8-step Virtual Road
Test workflow. Originally lived in ``scripts/init_sequences.py``;
consolidated here so first-boot ``python -m scripts.bootstrap`` covers
the whole default-data surface.

Idempotency: per-row lookup by ``(name, created_by='system')``. If a
system-seeded sequence with the same name already exists, the row is
left alone (user edits preserved). User-created sequences with the same
name (different ``created_by``) are never touched.
"""
...
_SEQUENCES: list[dict] = [
    {
        ...
        "steps": [
            {"step_number": 1, "name": "连接信道仿真器", "type": "connect_instrument",
             "parameters": {"instrument_type": "channel_emulator"}},
            {"step_number": 2, "name": "设置工作频率", "type": "configure_frequency",
    ...
    },
    {
        ...
        "steps": [
            {"step_number": 1, "name": "设置测量频率", "type": "configure_frequency",
             "parameters": {"frequency_mhz": 2400}},
            {"step_number": 2, "name": "开始TRP测量", "type": "run_measurement",
             "parameters": {"measurement_type": "TRP", "theta_step": 15, "phi_step": 30}},
            {"step_number": 3, "name": "保存测量结果", "type": "save_results",
    ...
    },
    {
        ...
        "category": "Measurement",
        "steps": [
            {"step_number": 1, "name": "配置基站仿真器", "type": "configure_instrument",
             "parameters": {"instrument_type": "base_station", "tx_power_dbm": -70}},
            {"step_number": 2, "name": "开始TIS测量", "type": "run_measurement",
    ...
    },
    {
        ...
        "steps": [
            {"step_number": 1, "name": "选择信道模型", "type": "select_channel_model",
             "parameters": {"model": "SCME_UMa"}},
            {"step_number": 2, "name": "设置多普勒频移", "type": "configure_doppler",
             "parameters": {"speed_kmh": 30}},
            {"step_number": 3, "name": "配置空间相关性", "type": "configure_correlation",
    ...
    },
    {
        ...
        "steps": [
            {"step_number": 1, "name": "选择校准探头", "type": "select_probes",
             "parameters": {"probe_count": 8}},
            {"step_number": 2, "name": "运行校准测试", "type": "run_calibration",
    ...
    },
    {
        ...
        "category": "5G NR",
        "steps": [
            {"step_number": 1, "name": "设置5G频段", "type": "configure_frequency",
             "parameters": {"frequency_mhz": 3500, "bandwidth_mhz": 100}},
            {"step_number": 2, "name": "配置波束成形", "type": "configure_beamforming",
...
def _seed(db: Session) -> SeedResult:
    ...
    for seq in _SEQUENCES:
        existing = (
            db.query(TestSequence)
            .filter(
                TestSequence.name == seq["name"],
        ...
        if existing is not None:
            skipped += 1
            continue
        db.add(TestSequence(**seq))
        inserted += 1

    db.flush()
    if inserted:
        logger.info("[sequences] seeded %d system sequences", inserted)
    return SeedResult(inserted=inserted, skipped=skipped)


sequences_seeder = Seeder(
    name="sequences",
    version=1,
    description=f"{len(_SEQUENCES)} canonical test-step sequences (Sub-6/FR2 + VRT 8-step)",
    run=_seed,
)
