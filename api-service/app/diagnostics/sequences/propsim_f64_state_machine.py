    ...
    *,
    ...
    try:
        ...
        if initial_bypass != "0":
            ...
            # ⚠ 先核**旁路档真的回到 0** 再谈状态。固件在旁路中完全可能照样报一个
            # "可操作"的 STOPPED/RUNNING (手册七态里没有 BYPASS, 旁路下报什么是未定义
            # 的) —— 只核 STATE? 就会在"其实还在旁路里"的情况下继续发 GO(灰色地带),
            # 或者在跳过旁路探测时直接发 GOS。旁路档的唯一权威来源是 MODEL:STATIC?
            # (§20.4.6.26), 判旁路只能问这条 (Codex #229 第六轮 P1)。
            #
            # ⚠⚠ 这是"写完必须核回读"这条规则在本剧本的**第 4 个**写旁路站点。
            # 前三个 (进旁路探测 / 探测后退旁路 / 恢复旁路档) 分别在第三、五轮和最初
            # 就补了, 唯独这个开跑前的漏到第六轮 —— 因为我每次只修被点到的那一处,
            # 没把规则当成一类去搜。`test_every_bypass_write_is_verified` 现在把这条
            # 钉成结构性断言: 再加写站点忘了核对, 测试直接红。
            exited = await p.read_bypass("退旁路后 MODEL:STATIC? (确认真退出)", expect="0")
            if exited != "0":
                raise _Abort(
                    f"开跑前退旁路后, 旁路档仍是 {exited or '读不到'} 而不是 0 —— "
                    "仍在旁路中, 此时发 GO/GOS 的行为手册未定义, 剧本不碰。"
                )
