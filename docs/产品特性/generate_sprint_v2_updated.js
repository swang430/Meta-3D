    ...
    // ======================== 封面 ========================
    {
      ...
      children: [
        ...
        new Paragraph({
          ...
          children: [new TextRun({ text: "v2.1 (Based on latest HAL completion) — 融合代码现状的精确执行方案", size: 28, color: "2E5984", font: "Arial", italics: true })]
        }),
        gap(),
        new Table({
          ...
          rows: [
            new TableRow({ children: [
              ...
              dCell("v2.1 (Based on latest HAL completion)", 6360)
            ]}),
            new TableRow({ children: [
        ...
        heading("1.2 v2.1 (Based on latest HAL completion) 与 v1.0 的核心差异", HeadingLevel.HEADING_2),
        para([
          { text: "v1.0 的制定未充分考虑已有代码资产。", bold: true },
          { text: " 本 v2.1 (Based on latest HAL completion) 基于对当前代码仓库的逐文件审计，精确识别了每个模块的完成度，将工作量聚焦在「最后一公里」——真实 SCPI 驱动编写与真实测量数据灌入。以下是关键差异：" }
        ]),
        ...
        bullet("完成 Level 0–4 全链路校准（路损 + RF 链路 + 端到端 + 信道校准），校准数据写入 PostgreSQL 数据库", "bl2"),
        ...
        new Table({
          ...
          rows: [
            ...
            new TableRow({ children: [
              ...
              dCell("MockBaseStation 以及 UXM 5G 和 CMW500 的物理 HAL 驱动已全部完成并注册", 4260)
            ]}),
            // Signal Analyzer
            new TableRow({ children: [
        ...
        new Table({
          ...
          rows: [
            ...
            new TableRow({ children: [
              ...
              dCell("代码适配层准备（v2.1 (Based on latest HAL completion) 新增）", 3600),
              dCell("Day 0", 1200),
        ...
        new Table({
          ...
          rows: [
            ...
            new TableRow({ children: [
              ...
              dCell("接口类型（SCPI/REST）", 2400), dCell("物理驱动已就位", 2160, { shading: DONE_FILL })
            ]}),
            new TableRow({ children: [
              dCell("信号分析仪", 2400), dCell("R&S FSW", 2400),
              dCell("GPIB/LAN 地址", 2400), dCell("物理驱动已就位", 2160, { shading: DONE_FILL })
            ]}),
            new TableRow({ children: [
        ...
        bullet("确认 PostgreSQL 数据库 中 instrument_categories 表已有正确的仪器分类记录", "bl4"),

        heading("4.3 Phase 0.5 — 代码适配层准备（v2.1 (Based on latest HAL completion) 新增）", HeadingLevel.HEADING_2),
        para([
        ...
        para([
          { text: "v2.1 (Based on latest HAL completion) 调整说明：", bold: true },
          { text: " v1.0 安排了 5 天进行 HAL 开发。由于三层适配器架构（InstrumentDriver → [Type]Driver → Real[Model]Driver）已完整就位，Mock 驱动均可正常运行，且 F64 真实驱动已完成 1198 行双管线 SCPI 代码，目前连基站仿真器真实驱动也已完成，工作量仅剩信号分析仪及 E2E 联调。因此压缩至 3 天。" }
        ]),
        para([
        ...
        para("当前 base_station.py 已经极大丰富，同时 cmw500_base_station.py 和 uxm_base_station.py 已通过解读数万页官方 SCPI 实现了 FRC 选择、PS承载打通、信令控制与轮询逻辑。"),
        ...
        heading("9.1 关键路径（v2.1 (Based on latest HAL completion) 更新）", HeadingLevel.HEADING_2),
        para("本冲刺的关键路径："),
        para([
          { text: "F64 SCPI 补全（Day 1）→ 基站驱动（Day 2）→ Level 4 信道校准（Day 7）→ 首次 E2E 测试（Day 12）", bold: true }
        ]),
        para([
          { text: "v2.1 (Based on latest HAL completion) 调整：", bold: true },
          { text: " 由于 F64 已有原型驱动，关键路径从 v1.0 的「从零建立 VISA 连接」变为「补全 SCPI 指令集」，风险显著降低。但基站仿真器驱动仍是未知领域（可能是 SCPI 或 REST API），需提前确认接口类型。" }
        ]),

        heading("9.2 风险登记册（v2.1 (Based on latest HAL completion) 更新）", HeadingLevel.HEADING_2),
        new Table({
...
const outputPath = "/Users/Simon/Tools/MIMO-First/docs/产品特性/静态MIMO-OTA冲刺计划_3-4周_v2.1 (Based on latest HAL completion).docx";
