...
> **优先级说明**: T0 (基础设施) → T1 (核心功能) → T2 (稳定性) → T3 (高级功能) → T4 (运维优化)

### T0 - 基础设施 (Sprint 1)

| 功能 | 位置 | 状态 | 工作量 | 描述 |
|------|------|------|--------|------|
| **Auth Context** | 多处 | ❌ 未实现 | 3-5天 | 用户身份识别，JWT 认证，多处依赖 |
| **Statistics Real Data** | statistics_service.py | ⚠️ Mock | 2-3天 | 替换模拟数据，连接真实执行历史 |

### T1 - 核心功能 (Sprint 2)

| 功能 | 位置 | 状态 | 工作量 | 描述 |
|------|------|------|--------|------|
| **Queue Reordering** | QueueTab.tsx:115,319 | ❌ 未实现 | 2天 | 拖拽/按钮排序测试计划顺序 |
| **Alert System** | dashboard.py:44 | ❌ 未实现 | 3-4天 | 告警模型、API、WebSocket 推送 |
| **Sync Phase 1** | channel-engine-service | 📋 设计完成 | 3-4天 | L2 实时通信层 (ZeroMQ + SharedMemory) |

### T2 - 稳定性 + HAL (Sprint 3)

| 功能 | 位置 | 状态 | 工作量 | 描述 |
|------|------|------|--------|------|
| **WebSocket Reconnection** | gui/src/api | ❌ 未实现 | 2天 | 断线重连、消息缓冲 |
| **Sync Phase 2** | api-service/services | ❌ 待实现 | 4-5天 | HAL 同步服务 (Clock/Trigger) |
| **Topology CRUD API** | api-service/api | ⚠️ 部分 | 2天 | 设备拓扑配置完善 |

### T3 - 高级功能 (Sprint 4)

| 功能 | 位置 | 状态 | 工作量 | 描述 |
|------|------|------|--------|------|
| **Report Comparison** | report_service.py:401 | ❌ 未实现 | 3天 | 多报告对比分析 |
| **Scenario Navigation** | ScenarioCard.tsx:106 | ❌ 未实现 | 1天 | 场景→测试管理导航 |
| **Sync Phase 3** | - | ❌ 待实现 | 依硬件 | 真实仪器集成验证 |

### T4 - 运维优化 (Sprint 5)

| 功能 | 位置 | 状态 | 工作量 | 描述 |
|------|------|------|--------|------|
| **Dashboard Comparison** | dashboard.py:45 | ❌ 未实现 | 1天 | 对比项选择跟踪 |
| **Docker 部署** | - | ❌ 未实现 | 2天 | API 服务容器化 |
| **监控和日志** | - | ❌ 未实现 | 2-3天 | 结构化日志、Prometheus |
| **Sync Phase 4** | - | ❌ 待实现 | 持续 | 延迟优化 (<5ms) |

### 硬件同步架构

详细设计: [HARDWARE-SYNC-ARCHITECTURE.md](design/HARDWARE-SYNC-ARCHITECTURE.md)
...
│  层级    │   精度       │   实现                       │
├─────────┼─────────────┼──────────────────────────────┤
│ L0 时钟  │   < 1 ns    │   硬件 10MHz 参考 (不涉及软件) │
│ L1 触发  │   < 1 μs    │   硬件触发总线 (软件配置)      │
│ L2 参数  │   < 10 ms   │   ZeroMQ + SharedMemory      │
│ L3 控制  │   < 100 ms  │   REST/WebSocket (当前架构)   │
└─────────┴─────────────┴──────────────────────────────┘
```
...
| Phase | 描述 | 状态 | 完成度 |
|-------|------|------|--------|
| Phase 1 | ChannelEngine核心集成 | ✅ 完成 | 100% |
| Phase 2 | Road Test API + WebSocket | ✅ 完成 | 100% |
| Phase 3 | 前期OTA/Conducted | ⚠️ 部分 | 40% |
| Phase 4 | GUI和前端集成 | 🔄 进行中 | 70% |
| Phase 5 | 报告系统 | ✅ 完成 | 100% |
...
| 2025-12-10 | v1.1 | 添加技术债务清单 (P0/P1/P2) |
| 2025-12-11 | v1.2 | 添加硬件同步架构路线图 |
| 2025-12-11 | v1.3 | 重构优先级体系 (T0-T4)，整合软件功能与硬件同步 |
