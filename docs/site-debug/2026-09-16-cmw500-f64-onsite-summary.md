...
| D17 | **2026-09-18 补记（PR #480 内审发现，原稿未记）**：执行 `f8f5fd90`（B41 TDD）Cell-ready 功率观察两次采样，F64 输入口 2 均为 **−108.0 dBm**（无信号；输入口 1 为 −28.9 / −30.7 dBm），16 路输出均值 −52.3 / −53.8 dBm，比上一次取到观察的执行 `d058d9de`（−50.0 / −49.8 dBm）低 2.3 / 3.9 dB。功率观察照常放行 —— 它只拦「读不到数」，判不了「无信号」 | 已登记 Discovered，待 triage |
...
## 5. 下一轮安排（第 0 步已由 PR #480 合并；其后顺序已由用户 2026-09-18 批准；WIP = 1）

> **以下为 2026-09-17 原稿，保留审计，不代表现状。** 现状以 [roadmap](../roadmap-first-call.md) 顶段与 Current Focus 为准。与实际合入不同的两处：
...
**下次现场的前置清单**：**用合并后的版本（PR #480）复跑一次 `dbd53e6f` 同款用例并补记** —— 过审改了 CMW 路由写前的错误队列读弃、未确认回执路径与 F64 stop 回执的证据键，这几处从未在真机上跑过（§4.1 的约定）；关闭调试机代理（D16）；要测 TDD 就带支持对应频段的 DUT；多方位用例等 P2-74 修完再带；
