...
SCME UMa 衰落下，单发 5000 子帧窗口读到 **45.82 Mbps、ACK 100 %、BLER 0 %**（该 RMC 的理论满额），报告生成、
小区关闭与租约释放都有同次证据。正式判词仍是 `UNKNOWN`：无路损校准、无现场认证，另有两处**纯软件缺口**
会让 CMW500 永远拿不到正式判词（§4 D4 / D5）。
...
代价：现场提交了 6 次（1 次文档 + 5 次代码：3 处 CMW 驱动 / 执行器真机缺口、1 个功率观察功能、1 处 GUI 布局），后端当天重启 17 次；这些提交**尚未过审、未推送**（§6）。
...
| 13:24 | 操作员经原始 SCPI 端点对 F64 发 13 条只读查询（STATE / WRAP / TRIG / MODEL:STATE / MODEL:CONT / `CH:MOD:TIME?`） | DiagnosticRun ×13 |
...
| **P0-8b** DUT Attach 后 DL 非 0 % ACK / 不失真 | ✅ 完成（P0-8 整项收口） | 衰落下 ACK 100 % / BLER 0 %，吞吐 = RMC 满额（N100 Q16 T13 ×2 层 ≈ 45.84 Mbps）。最难的衰落态已满额，bypass 半不再单独要求 |
...
> 「代码落地」是独立的事，见 §6。若评审改变了窗口或回执语义，须在下次现场用合并后的版本复跑一次并补记。
...
| D6 | 窗口 confirmed、吞吐 `throughput_valid=true`，但 `base_station.throughput.azimuth.000` 仍列为缺失 —— 可能只是 D4 的连带，待核 | P2 |
...
| D11 | **用户现场发现**：FDD 预验证非必要（§4.2 P2-55 行）；runbook §2 由必做降为可选 | 文档 |
...
| D14 | 操作员需要 F64 运行态（STATE / WRAP / TRIG / MODEL 进度）而无序列可用，改用原始 SCPI 端点 13 次；`CH:MOD:TIME? n` 两次空回复，语义未知 | P3 候选 |
...
- **「现场不写 driver 代码」这条铁律当天被打破**：5 次代码提交（另有未提交的 F64 stop 回执改动），每次保存触发 uvicorn reload（`DEBUG=true`），后端全天重启 17 次，
...
- 操作员 13 次使用原始 SCPI 端点，说明 F64 运行态缺一个只读快照序列（D14）。
