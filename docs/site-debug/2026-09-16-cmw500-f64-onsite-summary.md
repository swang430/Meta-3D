...
| D17 | **2026-09-18 补记（PR #480 内审发现，原稿未记）**：执行 `f8f5fd90`（B41 TDD）Cell-ready 功率观察两次采样，F64 输入口 2 均为 **−108.0 dBm**（无信号；输入口 1 约 −29 dBm），16 路输出均值 −52.3 / −53.8 dBm，比上一次执行 `d058d9de`（−50.0 / −49.8 dBm）低约 2.5–4 dB。功率观察照常放行 —— 它只拦「读不到数」，判不了「无信号」 | 已登记 Discovered，待 triage |
