# 多端口输入电平 —— 为什么一个全局旋钮控多端口 / 为什么 per-port 是标定不是闭环
...
>
> **门要不要软化的决策**：已评估，**保持现状**（见 §5）。本文是设计解释，不是 open action。
...
4. **正解 = per-port 标定 + 前馈**（roadmap `#2001 (2)(3)`，backlog only）：每个端口**确实有
   自己的量**，但那是"标定常数"（现场 SA/VNA 测一次 `cable_loss_by_port_db` 落库，前馈进功率
   预算），不是闭环里乱拧的变量。补偿掉已知线缆损耗后**剩下的**不平衡才是"真问题"。
...
（F64 内部把 PGA 挡位映射回去，测出的 dBm 仍是②的真值）。
...
**你的直觉是对的，但"独立量"该是标定常数，不是闭环变量。** 答案分四层：
...
### 4.5 正解：per-port **标定 + 前馈**

每个端口**确实有自己的量** —— 但它是**标定常数**：现场用 SA/VNA 把每个 port 的
`cable_loss_by_port_db` 测一次、当已知量前馈进功率预算，补掉已知线缆损耗后**剩下的**
不平衡才是真问题。这就是你说的"每端口独立量"，只是落成"**标定常数前馈**"而不是"盲环变量"——
既补偿了物理必然的偏差，又**保住了发现故障的能力**。这是 CTIA MPAC OTA 的标准做法，
也是 roadmap `#2001 (2)(3)` 记录的方向（需现场实测那张表，**backlog only，当前不推进**）。

---

## 5. 决策记录（2026-06-05：保持现状，已结）

`#143`（已合并）加了 **imbalance 遥测**（`imbalance_db = max(avg) − min(avg)` + 容忍带 +
`system_warning`），**不改 converged/fail 门**。是否进一步**软化门**（改用 imbalance 带）/
做 per-port cable cal（`#2001 (2)(3)`）/ 把操作点不确定度按 GUM 累加进报告（`#2002`）——
**评估后决定：保持现状，都不推进。**

- 当前 strict per-input 绝对窗口门工作正常；**软化它没有可证明的收益，反而有放过坏 setup
  的风险**。想改一个 working 的门，举证收益的责任在"想改"一方；在拿出证据（如现场实测
  imbalance 宽到 strict 门频繁误杀）之前，默认维持。
- `#2001 (2)(3)` 与 `#2002` 降级为**纯 backlog 笔记**，不是待拍板项。

两个仅供将来参考的边界（来自 `#2002`，当前不影响）：严格 PFS/PWS 场景 AUTOSET 改 PGA 可能引
group-delay→phase shift（当前 power-only PFS 不受影响，TR 37.977 F.2）；VRT 跨场景切 cell config
时 PAPR 漂，操作点需 re-setup（VRT 接入 InputLevelController 时一起做）。
...
- roadmap backlog：`docs/roadmap-first-call.md` 的 `#2001` / `#2002` 条目（backlog only）。
