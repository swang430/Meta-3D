...
4. **正解 = per-port 标定 + 前馈**（roadmap `#2001 (2)(3)`）：每个端口**确实有自己的量**，
   ...
   判定 + 漂移监控告警。
5. **决策（2026-06-05）：保持现状，不软化门，不追 `#2001(2)(3)` / `#2002`。** 当前 strict
   per-input 绝对窗口门工作正常，软化它没有可证明的收益、反而有放过坏 setup 的风险 ——
   举证收益的责任在"想改"的一方。`#2001(2)(3)`（cable cal / 漂移）与 `#2002`（报告不确定度）
   降级为**纯 backlog 笔记**，不是待决策项；除非将来出现真实问题（如现场实测 imbalance 宽到
   strict 门频繁误杀），否则不动。详见 §5。
...
## 5. 语义决策记录（2026-06-05：保持现状，已结）

> **决定：保持现状。** 当前 strict per-input 绝对窗口门工作正常；**软化它没有可证明的收益，
> 反而有放过坏 setup 的风险**，因此不软化、不追 `#2001(2)(3)` / `#2002`。它们降级为纯
> backlog 笔记，不再是"待拍板"项。**举证责任**：想改一个 working 的门，得先拿出收益（例如
> 现场实测 imbalance 宽到 strict 门频繁误杀的证据）；在那之前默认维持。
>
> 下面保留当初识别出的"如果要改才需回答"的问题，**仅作背景**，不是 open action：
...
但**不改 converged/fail 门**（这层到此为止，遥测够用）。*假如*将来要把它升级成"真门"，
才需要回答下面几件事（都属测试有效性 / 计量政策，代码层替不了）：
...
| | | (2) per-port cable balance cal + 前馈 | ✋ 不推进（保持现状，§5；backlog only） |
| | | (3) 漂移监控告警 | ✋ 不推进（backlog only） |
| **`#2002`** | 操作点的**声明侧**："我为此声明多大不确定度" | 操作点不确定度按 GUM 累加进报告 combined U | ✋ 不推进（保持现状，§5；backlog only） |
...
| 项 | 状态 | 说明 |
|----|------|------|
| imbalance 遥测（指标 + 容忍带 + 告警） | ✅ 已做（`#143`） | 到此为止，遥测够用 |
| 软化 converged/fail 门改用 imbalance 带 | ✋ 不做（保持现状） | 无可证明收益 + 有放过坏 setup 风险（§5 决策） |
| per-port cable balance cal 落库 | ✋ 不做（backlog only） | 同上；真要做需现场 SA/VNA 实测每根线损耗 |
| 报告 combined uncertainty（GUM, k=2） | ✋ 不做（backlog only） | 同上；真要做需定计量政策 + 现场标定真值 |
