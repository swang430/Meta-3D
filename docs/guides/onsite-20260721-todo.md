...
>
> **2026-08-06 用户后续纠正（覆盖下方初稿的“没有新数据”结论）**：7 月 21 日实际完成了
> DUT attach（UXM 显示 CONN）和转台四方向测试；命令角 0/90/180/270° 对应反馈角
> 90/180/270/360°，码字1块速率分别为 1408/1502/1483/1392 blocks/s，ACK 为
> 99.9%–100%。当时 F64/UXM 工作点主要手动调定，软件只控转台并读取吞吐，缺少关键
> SCPI 的发送→接受→生效证据，所以该结果证明物理链可工作，但不能关闭正式自动化验收。

## 一、当天实际发生了什么(诚实综述)

- **目标**:重跑一条干净的测试链(UXM 配频点功率 → F64 load .smu → 调输入参考/峰均比 →
  设输出 → DUT attach → 启动信道播放),在此基础上做可重复性 + 降功率实验。
- **实际结果（按 2026-08-06 用户纠正后的事实）**:当天大量时间消耗在**恢复 F64/UXM 会话 +
  排障**上。加了 6 个 F64 控制端点；F64 PropSim 被操作搞**卡死一次**（现场重启 PropSim
  才恢复）；UXM 配置在后端重启后**变成错误默认值**被排查出来。其后实际完成 attach +
  四方向吞吐，但依赖手动工作点，未形成可证明完整软件控制链正确的 SCPI 闭环。
...
- [~] **P0-2 测试流程必须显式下发 + 读回验证 UXM 完整配置(不假设保留)**
      — **本地半实施完成 (2026-07-27, S1/S2/S3/S5 + 现场探针), 剩 S4/S6 现场**
  - 原始事实:后端重启后 UXM 变 `623334 / BW100 / -50`;我假设配置保留只激活小区,
    小区起不来 (`ACTive=1` 但 `STATus=OFF`)。**查证后问题重定义** (设计稿
    docs/design/p0-2-uxm-config-single-source-design.md, 七根因 R1-R7):
    问错寄存器 (R1/R2) + 写完不 APPLY 不生效 (R3) + 核对读自写值 (R4) +
    HAL-init 假绿 (R6) + binding 压过默认 (R7)。
  - ✅ **已落地 (本地)**: D1 状态换源 `BSE:STATus:NR5G:<cell>?` + 手册枚举白名单
    (attach 轮询/get_cell_state, 去掉"发了命令=ON"缓存断言, 超时带字面值);
    D2 写入契约 (ON 态批次收尾必发一次 `BSE:CONFig:NR5G:APPLY`, OFF 不发);
    D3 两层核对定位 (一层配置回读 ≠ 二层生效, 判绿只认二层); D5 HAL-init
    `applied=False` → 就绪行 `warn`(GUI 聚合黄灯, 头行计数含 warn);
    D6 inherit 补状态证据行。+14 行为门 (含 4 发变异实跑全红)。
  - ✅ **现场检查全部预建成诊断序列** (用户 2026-07-26 要求, 不留现场写脚本口子):
    - S4 命令拼写重验 (R5 五条) → `uxm_manual_spelling_probe` (已有, #229);
    - R4 缓存/生效判定 + R2 开关/状态对照 + R7 binding 取证 →
      **`uxm_config_truth_probe`** (新, 带恢复保障: 写必回原值+APPLY+终读确认);
    - S6 端到端 attach → `baseStation_attach_check` (已有)。
  - ⏳ **剩现场 (S4/S6)**: 跑上述三序列, R4/R5 结论抄回设计稿 §7; D4 按结论
    删"本机不支持"清单错项 + 下发序对齐手册 16 步 (需真机, 禁盲试)。
  - **[用户 review] 泛化契约**已写进设计稿 §4 (值只从测试参数来 / 手册序下发 /
    显式生效动作 / 两层核对 / 三方一致才报绿), 本项只在 UXM 兑现, 其余仪表逐台补
    (F64 的二层 = STATE?, F64R-1 已完成大半)。
...
- [ ] **F64R-12 HAL 里 38 处静默吞异常 —— 出事日志无痕** (2026-07-26 用户问"这些提示
  ...
  - ✅ **"防新增"半边已下沉成棘轮门** (2026-07-26 C 组): `tests/test_rule_gates.py`
    G4 按同一谓词**逐文件×函数**精确锁 38 (#232 R2 P2: 净计数会放过跨函数换位),
    新增/换位即红; **修存量时同步下调门里的基线** (精确相等断言, 下降不调基线也红
    —— 有意如此, 逼进度锁死)。同函数内换位是已知边界, 修存量时自然消解。
    本条剩"修存量"半边。
...
- [ ] **F64R-10 ⛔已撤销重定义: 旧驱动对象的活连接被替换时无人认领** (2026-07-26 晚,
  ...
  - **重定义 (第三版, #230 两轮 Codex 纠正后的可达性矩阵)**: 真正有害的是**孤儿连接**,
    ...
    **F64 的覆盖点机制存在但无可达调用链。**
  - **可达的覆盖入口在 UXM, 两条** (#231 两轮 P2 纠正后口径, 均已核):
    ① 测量路 — HAL init 开的会话被**首个**测量步 `base_station.connect()` 覆盖
    (`measure.py:263`, 入口不关旧 `uxm_base_station.py:335`); 该步 finally →
    cleanup → disconnect 关当前会话置 None, 后续步骤从 None 连用完即断,
    **一次 init 一条不累积**; ② 诊断路 — `baseStation_attach_check` 每次运行
    `bs.connect()` (`:130`) 且全序列无 disconnect, **操作员反复跑就逐次累积**。
    解法在 UXM connect 入口 (关旧再开新 / 活句柄复用), 且 **"关旧成功+开新失败"
    窗口 UXM 懒重连帮不上** — `_silent_reconnect_visa` 失败置 None 收场而
    `_do_write` 见 None 直接抛不触发重连 (`uxm_base_station.py:2273-2282`),
    该窗口是设计必答题; UXM 置-None 写法与 F64 "绝不置 None" 纪律相反的既有差异
    一并进设计, 不顺手修。转台同型走 F64R-14 (非 VISA)。
...
- [ ] **C-1 [backlog] precheck_strict_input_level 要不要补 session 旁路** (2026-07-26
  ...
  - 已在 `tests/test_rule_gates.py` G3 豁免表**显式登记** (带理由, 豁免对象从权威
    消失时豁免表自红)。**若拍板"补"**: 四站点各加一处 (照 #133 cell_config 模式),
    然后从豁免表删掉 — 门自动开始管它。**若拍板"不补"**: 现状即终态, 本条关闭。
  - 判断要点: 真硬件 bring-up 时输入电平闭环不收敛, 操作员需不需要**会话级**跳过
    (不改 TestCase)? 现场撞过一次就该补; 没撞过维持现状。
