...
## 2b. 现场三开关速查(2026-07-20 晚落地)

| 开关 | 拨法 | 默认 | 拨到另一档的时机 |
|---|---|---|---|
| **开关 1** UXM 配置来源 | 吞吐脚本 `UXM_CONFIG_MODE=inherit` / 测试计划步骤表单「仪表使用参数」区 | `dispatch`(主动下发+回读对账) | 下发路径出意外(如风险⑧ ON 态写触发重配)时切 `inherit`:跳过**小区级参数**下发(ARFCN/BW/功率/小区级层数),频率核对改从仪器读回比对(知情继承;读不回会显式告警"核对未发生")。**⚠ inherit 仍会写 UXM**:MAC 吞吐配置(TDD/MCS/HARQ)、CELL ON、RRC 按 TestCase 推层数(层数未纳入读回核对)、输入闭环调 DL 功率;且跑完收尾会 CELL OFF——第二次 inherit 继承的是"上次跑完态"。IRAT 下重载驱动不动 UXM 小区(profile 兼容门拒 auto-apply,07-03 实录),EMQuest 态天然保留 |
| **开关 2** F64 直通模式 | attach 诊断序列参数 / 测试计划步骤参数 `f64_bypass_mode`(GUI 均有输入框) | `3`(校准旁路,-10 dB,-96 RSRP 实证态);计划路留空=正常衰落回放 | CONN 后 4 层配置/数据面起不来 → 拨 `2`(Butler,官方为建 MIMO 链设计);注意电平从 -10 dB 漂到模型平均 |
| **开关 3** 执行入口 | 现场脚本 vs 正式测试计划执行 | 两路都已到位(#217 runner + #218 队列 + 07-21 晨表单输入框,均 E2E 验证过) | 首跑建议正式计划路(测试管理 → 执行队列 → 行内开始);异常再退脚本保底 |

**仪表使用参数 GUI 入口**(07-21 晨落地):测试管理 → 步骤编排 → 选中步骤 → 折叠区「**仪表使用参数(现场调试旋钮)**」——6 个旋钮:UXM 配置来源(开关1)/ F64 直通模式(开关2)/ F64 输入参考电平 + 峰均比(手动定标,成对填)/ F64 输出增益 / 闭环起点功率。全部留空 = 现行为。⚠ 计划在队列中时步骤只读:改参数先出队 → 改 → 重排队(discovered 已记,待放宽)。
...
| [x] | ~~执行队列 5 月僵尸清理~~ ✅ 07-20 拍板全清(实际 801 条,`scripts/cleanup-test-queue.py`) | 07-21 晨复发 13 条 = 全量测试套件写开发库(`test_feature_gaps.py` 无隔离,discovered 已记);复发就再跑一遍脚本 `--keep-name 现场调试` |
