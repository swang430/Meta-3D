...
| 开关 | 拨法 | 默认 | 拨到另一档的时机 |
|---|---|---|---|
| **开关 1** UXM 配置来源 | 吞吐脚本 `UXM_CONFIG_MODE=inherit` | `dispatch`(主动下发+回读对账) | 下发路径出意外(如风险⑧ ON 态写触发重配)时切 `inherit`:跳过**小区级参数**下发(ARFCN/BW/功率/小区级层数),频率核对改从仪器读回比对(知情继承;读不回会显式告警"核对未发生")。**⚠ inherit 仍会写 UXM**:MAC 吞吐配置(TDD/MCS/HARQ)、CELL ON、RRC 按 TestCase 推层数(层数未纳入读回核对)、输入闭环调 DL 功率;且跑完收尾会 CELL OFF——第二次 inherit 继承的是"上次跑完态"。IRAT 下重载驱动不动 UXM 小区(profile 兼容门拒 auto-apply,07-03 实录),EMQuest 态天然保留 |
| **开关 2** F64 直通模式 | attach 诊断序列参数 `f64_bypass_mode`(GUI 表单可改) | `3`(校准旁路,-10 dB,-96 RSRP 实证态) | CONN 后 4 层配置/数据面起不来 → 拨 `2`(Butler,官方为建 MIMO 链设计);注意电平从 -10 dB 漂到模型平均 |
| **开关 3** 执行入口 | 现场脚本 vs 正式测试计划执行 | 现场脚本(彩排两遍) | 正式执行链路 Simon 在补(PR 待),到位并验证后可切;明天保底走脚本 |
