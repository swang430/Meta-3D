...
> **2026-09-22 软件进展（P1-80）**：上述软件修复已落地。序列现先归档开场 residue，再在每条
> 业务查询后立即排水，并在整段诊断租约内关闭 1 Hz 仪表监控、持有 F64 可重入 SCPI 事务锁，
> 让已启动或新启动的后台 SCPI 都不能插入两者之间；
> 非零错误归属到刚执行的查询，错误队列不可判、非法值域（含 `VALid?` 多余字段）或错误 payload
> 均 fail-closed。`license`、`calibration`、`user_alignment` 已分列 `CONFIRMED / UNKNOWN`，且
> `CONFIRMED` 只表示回复可信，不表示校准有效或用户对齐启用。查询集合未变化，也未新增/猜测
> SCPI。P1-2 仍未关闭，下一步仍是同一 F8800A 的干净队列复验。
