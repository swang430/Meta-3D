...
- `[discovered on-site 2026-07-03 during EMCenter 复盘]` **EMCenter 失败层级下探:IP 通(ping ~1ms)但 12 个常见 TCP 端口全拒(5025/2001/23/80/443/4000/4001/9221/10001/1024/5024/5555)→ 设备侧 LAN 远程控制服务未启用嫌疑**,在 6/4 修的驱动协议层(裸命令+CR+5025)**之前**还有一层;5/27 无响应可能是两层叠加(服务未开 + 协议错)。解法 = 现场 EMCenter 前面板(触屏)System/Interface 查 LAN/SCPI 远程服务是否 Enabled + 实际端口号(顺带记插卡槽位型号,补 P2-9 现场半清单);拿到端口一条命令即可验证协议修正。今日决策不变:通路人工已配,不阻塞主线。
