...
## 4⁺. 外审（Codex #273）两条 P2 —— 都成立，已修

**P2-a `resp_len` 量的是 strip 之后的长度** —— `body = response.strip()` 在前、
`full_len = len(body)` 在后，所以那个字段不是承诺的"原始响应长度"，而且
**whitespace-only 响应（`"   \n"`）与真空响应都成 `resp_len=0`，无法区分**。
本片正是拿"空回复 60,565 条"当立项证据的 —— 量错长度等于把自己的证据量废。
已改成 `raw_len = len(response)`（原始、未 strip），truncated 标记的分母另记
strip 后的长度，两个数在 docstring 里写明区别。配门 M14 + 两条新用例。

**P2-b 我在修内审 F2 的那一次，把同一个错又犯了一遍** —— F2 抓的是
`test_default_limit_is_2000` 把"文档承诺可调的旋钮"钉成断言（用一次旋钮测试套就红）。
我改掉了它，却在**替代门**里硬写 `[truncated 2000/2001]` —— 同样的缺陷，同一次提交。
已改成 `expected = hal_base._resolve_resp_max()` 从 settings 现算，并加一条
`_SCPI_LOG_RESP_MAX == _resolve_resp_max()` 的不变量（字面量化会红）。

> 这两条连起来是同一个教训：**修 A 时顺手写下的 B，没有经过 A 那道判据。**
> 今晚这个形态第四次出现（#268 → P1-25 → P1-26 band-delete → 本片）。

## 5. 验收（怎么证明修好了）

**实跑结果（2026-08-03，含内审后补强）**：

- 门 **24 条全绿**；**15 条变异逐条实跑，全部让门变红**；还原后复跑 24 绿。
  ...
  | **M14** | **`resp_len` 量 strip 后长度（whitespace-only 与真空混为一谈）** | **Codex #273 P2** |
  | **M15** | **生效端门把期望值写死成 2000（旋钮配 200 就红）** | **Codex #273 P2** |

  ⚠️ **M15 在默认配置下必然绿** —— 写死的 `2000` 与现算值恰好相同，缺陷不可观察；
  换到 `LOG_SCPI_RESP_MAX=250` 再跑才红（已实跑：基线 24 绿 / 变异 1 failed）。
  **判"门漏"之前先把变异放进能让缺陷显形的配置里** —— 这条已写进
...
- 全量 **2892 passed, 4 skipped, 0 failed**（148s）。（这个数随门的条数走：内审时
  2887、补内审门后 2890、补 Codex 门后 2892 —— 所以 roadmap 那边**不再重复记数字**，
  只指向本节。）
- **旋钮下的合法部署实跑**：`.env` 配 `LOG_SCPI_RESP_MAX=200` → 门 **24 绿**
  （Codex #273 P2 指出初版会在此配置下红，已修；`.env` 已还原并校验）。
