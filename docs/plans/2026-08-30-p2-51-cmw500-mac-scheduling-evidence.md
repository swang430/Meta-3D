...
>
> ⚠️ **状态更新（2026-08-31，P1-74）**：本清单第 11 行与「参数映射」表中 `stat_count` 那格写的
> 「`EBLer:SFRames` 命令归窗口层所有、**当前未驱动** = 已登记缺口」，是 **2026-08-30 P2-51 交付当时**
> 的现状。**P1-74 非现场半已把它驱动起来**：窗口层在 `SCONdition NONE` 之后下发 `EBLer:SFRames`
> 并回读比对，值取自 TestCase 的 `stat_count`。「本片不发」对 P2-51 自身仍然成立（统计基不归 MAC
> 配置层），此处不回写表格内容，只标注现状已变，避免读者据此以为该命令至今无人驱动。
> P1-74 的**现场半仍未完成**。
