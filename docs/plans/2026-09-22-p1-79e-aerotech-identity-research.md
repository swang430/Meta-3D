...
- Ensemble Windows C/.NET SDK 有基于已连接 controller handle 的 controller information API，例如 `EnsembleInformationGetName()` / `controller.Information.Name`；
...
| Ensemble C SDK `Information.h` / .NET 官方样例 | `EnsembleInformationGetName(handle, ...)`、`controller.Information.Name` 经 Windows SDK 连接读取 controller name | 不是 raw ASCII/TCP 命令；没有证明当前 macOS API 服务可加载 Windows SDK，也没有给同一接口的固件读取 | 不把 SDK 安装版本或配置名冒充现场设备回读 |
...
若厂商确认 Ensemble 只能通过 Windows SDK 取身份，需要一个与 API 服务分离、部署在受支持 Windows 主机上的受控采集器。它至少要解决同一设备绑定、认证、时间新鲜度、重连/漂移、传输安全和 execution freeze；SDK 的 controller name 也未必等同于硬件 model，固件来源仍须单独核实。这已是新子系统，不属于 P1-79E 的最小修复，不能在当前驱动里顺手加一个配置字段替代。
