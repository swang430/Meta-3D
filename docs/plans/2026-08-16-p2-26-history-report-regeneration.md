...
### Task 4: Ready PR 与 P1 清零外审

1. 推送分支，创建 Ready PR，触发 `@codex review`。
2. R1 核实并处理本片内意见，TDD 修复后触发 R2。
3. R2 无 P1 立即 merge；R2 有 P1 则修复、内审、回归后触发 R3，此后只以 P1 为阻塞继续复审，直到覆盖最新 HEAD 的外审无 P1。R2 及后续 P2/P3 只报告，不扩轮。
