...
| 命令 | 手册原文结论 |
|---|---|
| `SYSTem:APPLication[:NAME]`（APP_SELECT） | **原文存在**：`Application Mode : NSA \| SA`；`Range : ... \| LTE_NR_IRAT \| ...`。**在 LTE_NR_IRAT 运行期间可不可用 —— 手册未说明**（无任何切换限制描述） |
| `ROUTe:NR5G:<cell>:HARDware:TX/RX:ANTenna<n>:PORT`（MIMO_TX/RX_ANT_PORT） | **手册完全不存在这两条命令**；也未提供 BSE: 下显式「天线→物理口」路由等价命令（`BSE:CONFig:NR5G:<cell>:DL:ANTenna:CONFig` 是枚举型天线配置，能力不同，不作等价替换） |
| `...PDSCH:AMC:ENABle`（PDSCH_AMC_ENABLE） | **手册不存在任何 `:AMC:` 节点的独立开关**；DL 自适应由 `BSE:CONFig:NR5G:<cell>:SCHeduling[:<BWP>]:<fc>:<sc>:DL:RRESource:APOLicy` 控制（`Range : FIXed \| BLER \| DYNamic \| CQI \| PMI \| RI \| CQIRi \| PMIRi \| CQIPmiri \| CR`，Default `FIXed`）—— 与 profile 既有注释一致 |
...
- **`critical_undefined` 从失败因子降为如实披露**：改名 `critical_not_in_profile`，
  进 `extra` + log + summary 披露，**不再参与 `success`**；
- `success = (not critical_unsupported) and (not critical_unverified_actions)
...
| 有实测 UNSUPPORTED critical | False | BLOCKER 列名（不变；不再夹带未定义段落） |
| 有 INFERRED_ONLY mandatory ACTION | False | BLOCKER 列名（不变） |
| 以上皆无（未定义与否均可） | **True** | applicable 全支持 + 未定义能力如实披露 |
...
- **fail-closed 原样保留**：profile 有定义、实测 -113/-114 → 仍 BLOCKER；
  ...
  `success=False`、失败原因**收窄且如实**（只剩那 2 条 ACTION）；5G_NR_Test 全 clean
  端到端变绿（该方言无 mandatory ACTION 定义）——「方言没有的命令不再判失败」在两个
  方言上都可观察。
...
| ①b 5GNR：端到端变绿 | clean 固件下 `success is True` + summary 含未定义披露 | 红（现状恒 False） |
