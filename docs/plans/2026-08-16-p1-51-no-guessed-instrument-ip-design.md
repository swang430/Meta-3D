...
3. UXM/CMW500 只把语法完整的 TCPIP VISA resource 原样交给 PyVISA；普通 host 或
   ...
   但不一致时，在任何 I/O 前明确报配置冲突；两个完整 VISA resource 还必须连同传输类型与
   HiSLIP 子地址完全一致，禁止 preflight、界面配置与 connect 指向不同仪表或 Test App。
...
3. UXM/CMW500 对普通 endpoint 正确构造 VISA resource，对完整 TCPIP VISA resource 保持原样；
   地址、端口及完整 VISA resource/HiSLIP 子地址冲突与空白值均在 ResourceManager 前失败；
