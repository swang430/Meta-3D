...
3. UXM/CMW500 将显式 `endpoint` 作为 VISA resource 使用；其余驱动从显式 endpoint 解析 host。
...
3. DriverRegistry auto 在无地址时选择 Mock，有显式 endpoint 时才可选 Real；
4. fresh bootstrap 七类 connection 的地址均为空；已有连接再次 seed 后保持原值；
5. 全仓生产代码不再含真实驱动地址兜底。
