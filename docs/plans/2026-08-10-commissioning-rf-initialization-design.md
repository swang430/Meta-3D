...
> 状态：2026-08-11 已接入所有正式 MIMO OTA 吞吐量执行入口；待真机同次 execution 复验。
...
采用共享的“RF 初始化 → 受控 attach → 动态门 → 正式采样”编排边界。当前唯一正式
实现位于所有 MIMO OTA TestCase 共用的 `MeasureExecutor`，测试管理和完整暗室首测
只负责建立同一类执行并启用该边界，不各自复制硬件动作。

- 只移动 `set_passthrough_mode`：改动小，但仍依赖预加载 `.smu`，不能解决冷启动。
- 在共享 `MeasureExecutor` 内建立显式初始化阶段（采用）：模型加载、频率、工作点和
  attach 状态属于同一执行事务；测试管理与暗室首测天然共用，失败即停止。若以后出现
  第二个正式消费方，再抽成独立服务，避免当前先造一层无消费方的机制。
...
- “测试管理 → 运行 MIMO OTA 用例”和完整“暗室首测”会写入
  `managed_rf_attach=true`，执行静态 PRECHECK 后进入上述共享编排；
- PRECHECK 不读取初始化前的 UE 状态/能力，并将连接门明确标为 `deferred`；
- MEASURE 在最终 RF 状态重新确认 UXM `CONN`，真实仪表严格门失败时禁止进入
  AUTOSET 和方位采样；
- “调试序列 + 单阶段”是故障定位入口，不代表完整吞吐量流程，因此不写该标记。
