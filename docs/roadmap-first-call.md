...
**P1-76 → P2-68 → P2-69 → P2-70**，均为待启动，详见条目；P3-23 为流程优化方案，P2-57 残项为后续平台设计，
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | 原批准队列至 P2-62 已合并，产品开发 WIP=0。本次 triage 后 P1-76（监控假读数）优先，随后 P2-68/P2-69 两项已复现产品缺陷、P2-70 CMW 抽样载体缺口，均待启动，顺序见顶部。P2-57 残项仍待设计，资产拓扑不得放进 manifest；P3-23 是回归流程优化方案。P2-32 位于功能启用池，P3-20/P3-21 位于非阻塞维护池，均不得自动启动。现场静区线性 XY 扫描平台仍保持 Hardware Blocked。 |
...
**P3-23 — 回归与审查流程去重（方案已整理，待落实）**：目标是消除同代码版本重复全量、
纯文档触发后端全量，以及“建议默认 backlog”和 AGENTS 的冲突。具体分档、版本证明、生产路径
验证与量效记录见 [复盘 §4](plans/2026-09-05-development-review-and-triage.md#4-回归策略优化方案待落实不声称现行强制流程已改变)。
落实时只改 CLAUDE/reviewer 的执行方法并统一引用 AGENTS；本次没有修改现行测试硬门。
不新建测试平台、不批量删测试、不恢复并行 agent；测试门自身的问题不升级功能 P1。

**P3-1～P3-19 ✅ Done；P3-20/P3-21 已转非阻塞维护池；P3-22 ✅ PR #369；P3-23 待落实。** 已完成项的完整 What / Fix / Acceptance 详情已迁出 → [`roadmap-archive.md`](roadmap-archive.md)；新增项的当前范围与顺序只看顶部 Current Focus 表。速览：
