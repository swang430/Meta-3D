...
**Current Focus = P1-60「最近一次手工执行的校准、信道与时间真值对齐」**（2026-08-22
用户提供 `app_export.jsonl` 后立项）。可观察故障为：32 端口相位补偿只加载 29 项、四方位
路损只命中 3 项、资产已声明 40 MHz 却报“带宽未知”、UMa 资产按 UMi 运行，以及人读报告
编号比本地日志少 8 小时。开发、全量回归与 fresh 内审已完成；数据库/API 的机器时间仍保持
UTC，仅人读名称使用显式 `OPERATOR_TIMEZONE`（默认 `Asia/Shanghai`）。Codex R1 两条 P2
已按 TDD 收口：显式 vendor `.smu` 保留 `Indoor/Highway` 等既有场景标签，普通原生合成仍只
接受 3GPP 场景且显式冲突 fail-closed；容器不再把自身 UTC 误当操作员时区。R1 尾修相关链
**210 passed**、全后端 **4243 passed / 5 skipped**，GUI production build、`compileall`、
单一 Alembic head 与 `diff-check` 通过；fresh 尾审 P1/P2/P3=0。设计与证据见
...
~~P2-37~~ ✅ → ~~P2-38~~ ✅ → ~~P2-36~~ ✅ → ~~P2-31~~ ✅ → ~~P3-22~~ ✅ → **P1-60**。
...
| **P1-60** | 最近一次手工执行的校准、信道与时间真值对齐 | 🔄 PR #371 R1 两条 P2 已按 TDD 收口；尾审 P1/P2/P3=0，待 R2 |
