...
**Current Focus = P2-30「校准/方向图任务级仪表租约，避免逐点重连」（下一项，待开工；2026-08-21 P2-29 收口）**。

> **⚡ 并行批次模式（2026-08-21 用户拍板，本批次内覆盖 WIP=1）**：剩余 7 项 +
> 自 Discovered 提升 3 项（P1-58 / P2-35 / P2-36）共 **10 片，分两波、每波 5 个并行开发**
> （2026-08-21 用户把并发从 10 降到 5：第一波 P1-58/P2-35/P2-30/P2-33/P2-34，
> 第二波 P2-31/P2-32/P2-36/P3-20/P3-21 —— 按冲突面分波：校准服务与日志面板各自错开），每片独立
> worktree/分支/设计稿，流程不变（设计 → 内审 → 外审 → 合并）；外审回 Codex
> （额度已恢复）。合并按片串行进行、逐片解冲突；roadmap 收口集中处理，
> 各片 PR 不碰本文件。批次清单：P2-30..P2-36、P3-20、P3-21、P1-58。
...
| **P1-58** | `uxm_scpi_compatibility` 在 IRAT 方言上恒失败（`_CRITICAL_NAMES` 含 IRAT profile 不存在的 `TDD_PATTERN`）——判据按当前方言 profile 派生 | ⬜（2026-08-21 自 Discovered 8-06 提升，活 P1） |
| **P2-35** | `current_execution_id` 测试间泄漏隔离（commissioning:539 set 无还原）——治掉全量唯一已知失败 | ⬜（2026-08-21 自 Discovered 8-21 提升） |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → ~~P1-50~~ ✅ → ~~P1-51~~ ✅ → ~~P1-52~~ ✅ → ~~P1-53~~ ✅ → ~~P2-26~~ ✅ → ~~P1-54~~ ✅ → ~~P1-55~~ ✅ → ~~P1-56~~ ✅ → ~~P2-27~~ ✅ → ~~P2-28~~ ✅ → ~~P1-57~~ ✅ → ~~P2-29~~ ✅ → **P2-30 ⏭**  → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；P2-29 已由 PR #354 完成（状态细节看顶部 Current Focus）；P2-30 待开工，当前 WIP=0。现场物理单位/方向/偏置/型号实证仍保持 Hardware Blocked。完整编号、范围与状态只看顶部 Current Focus 表。 |
