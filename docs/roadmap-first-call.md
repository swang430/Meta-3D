...
**P2-11 (TestCase 驱动仪表配置架构) Phase 1/2/3/5/6 done + UXM 端口路由 path B (#1974) 收口。Phase 6 一致性网四根线 (频率 #109 + DL MIMO layers #114 + 调制阶数 #124 + 生效 MCS #126) 全部落地; UXM 端口路由/TDD/调度 path B TestCase 驱动 (后端 #127 + GUI #128) 闭环; 2026-05-31 核心参数审计 (架构文档 §8) 分 A/B/C/D 类。P2-10 (F64 精细化) Step 1/2/3 本地框架 done (#116/#125); P2-12 (SCD .smu) slice 1-4 + 扩展门 done (#116-#123) —— 三条 P2 线本地全收口, 各剩现场半 (P2-11 DL power 待操作点语义 / P2-10 现场真值 / P2-12 slice 5 SCPI 生成)。DUTProfile (#1965, 三层能力交叉校验, 用户 6-01 提) **四阶段全收口** (#134/#135/#136/#137; 详见 backlog line 1975)。**2026-06-05 用户排期 SIMProfile → 新建 P2-13 (SIM/eSIM 身份+鉴权声明 + SIM↔UXM 一致性 fail-loud), 设为 Current Focus** —— DUTProfile 收口后本地候选, 跟它平行 (能力层 vs 身份/接入层)。真 P0 (P0-3/4/5) 仍现场 blocked, 按治理「P0 全 blocked 时 Current Focus 可挪本地项」。P2-13 档 B (Phase 1-3: 声明实体 + precheck cross-check/防对错卡/鉴权 fail-loud 分根因 + GUI) **本地可启动**; 档 A (Phase 4: UXM HSS 自动 provision) 待现场确认 S8711A SCPI 鉴权命令。下次现场 (校准天线/SGH/真 DUT 到位) Current Focus 仍切回 P0-4→P0-3→P0-5 依赖链。**
