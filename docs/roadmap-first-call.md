...
**Current Focus = P3-19 / calibration warnings 证书持久化子片** —— tail 字节预算、端口清理 allowlist、UXM SOCKET、UXM ARFCN、mock-only 收口已分别由 PR #331–#335 合并。当前片为 `ProbePathLossCalibration` 增加 nullable warnings JSON（历史 `NULL` 保持“此前未记录”，不得洗成空列表），让 legacy 与 lab/topology 两个 live 写点精确落库，经 `/latest` 和校准报告数据继续带出；并统一 RF-chain / multi-frequency 成败响应的 warnings 通道。下一片处理 QZ / 方向图 / 多频扫频借用 acquire 时的清理警告穿透。
