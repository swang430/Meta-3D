...
**Current Focus = P3-18 / 诊断序列串行化收口子片** —— PDF 转义、G11 三覆盖面与 p08 零残留已分别由 PR #326/#327/#328 合并。回查入口全集后确认：P1-46 / PR #296 已在 run endpoint 给全部 `safe_during_test=False` 序列加破坏性诊断 token；后续 PR #304 再让整个 sequence body 持有全局 `InstrumentTestLease`。前者拒绝第二条破坏性序列/正式执行，后者让 F64 手工 SCPI 端点经共享租约排队。本片不再重复造锁，只补“序列未退出前 F64 手工 GO 不得进入驱动”的跨入口行为门并关闭 stale Discovered。手写类型审计递归比对继续保持 P3-18 后续逐片 WIP=1。条目见下方 P3-18。
...
本地序列现切到 **P3-18 / 诊断序列串行化收口子片**（P1-29 已由 PR #320、P1-38 已由 PR #321、P1-27 已由 PR #322、P2-22 已由 PR #323、P2-23 已由 PR #324、P2-24 已由 PR #325、P3-18 PDF/G11/p08 子片已由 PR #326/#327/#328 收口）。本轮日志设计与逐片实施计划见
...
| P3-18 | 门/测试精化批（G11 三覆盖面 in=location/curl 动词/schema 类型 + p08 零残留站点参数化 + PDF 渲染管道其余转义入口 + 诊断序列 run endpoint 串行化 + 手写类型审计尺子改逐层递归比对）——2026-08-02 拍板，各配变异。**PDF 子片已由 PR #326 合并**：封面与共享步骤自由文本统一在 `Paragraph` 边界转义，并去除 MIMO 双重转义。**G11 子片已由 PR #327 合并**：非 path 参数按 `(name, in)` 比对；散文识别 curl `-X/--request` 动词；响应 properties 前先拒绝 object/array 类型不兼容；合成 checker 变异同时锁住 location 与 schema 集成，完整规则门 50 passed。**p08 子片已由 PR #328 合并**：按当前实现全集枚举成功执行路径 10 个 `residue` 站点（9 个主流程窗口 + 共同收尾；中止专用站点已有独立失败场景门），同一条 fake 未认领错误逐站注入；任一站点删除都会让对应参数用例由 fail-closed 变成错误 PASS，变异实跑 1 failed / 其余 9 passed。**诊断序列串行化子片本地完成**：入口全集回查证明 P1-46 / PR #296 的 exclusion token 覆盖破坏性序列↔破坏性序列/正式执行，PR #304 的全局 `InstrumentTestLease` 覆盖整个 sequence body 与 F64 手工操作；本片补跨入口并发门，临时绕过共享租约会得到 1 failed。 | 🔄 |
