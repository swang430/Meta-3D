...
**~~P1-25~~ ✅ → ~~P1-26~~ ✅ → ~~P1-30~~ ✅ → ~~P1-31~~ ✅ → ~~P1-32~~ ✅ → ~~P1-33（本地半）~~ ✅ → ~~P1-34~~ ✅ → ~~P1-35~~ ✅ → ~~P1-36~~ ✅ → **P1-28** → P1-45 → P1-46 → ~~P1-39~~ ✅ → P1-43 → P1-44 → P1-42 → P1-40 → P1-41 → P1-37 → P1-29 → P1-38 → P1-27 → P2-22 → P2-23 → P2-24 → P3-18 → P3-19**（逐片 WIP=1，全流程照旧；P1-28 为 2026-08-02 用户新增 backlog 当日拍板插入；**P1-30 为 2026-08-03 用户指定插队"把日志提上来先完成"**；**P1-31 / P1-32 为 2026-08-03 UXM KPI 修复（#275）带出、用户当日拍板插到队首**；**P1-34 为 2026-08-05 用户手工测试当场提出、当日拍板插到队首**；**P1-39 为 2026-08-06 用户问「测试例编号除了 log 还能在哪查」当场发现的断链，用户明示「进roadmap，优先完成」插到队首**；**P1-40 为同日用户定的日志分文件方向，紧随其后**）。**当前队首 = P1-28**（2026-08-06 用户当场指定插到队首 —— 失配**已经发生**不是将来时）（P1-25 / P1-26 / P1-30 / P1-31 / P1-32 / P1-33 本地半 / P1-34 / P1-35 / P1-36 / **P1-39** 已收口）。一句话索引：
...
- **P1-43** 日志翻页：看得到 200 行 / 20000 行扫描窗口以外的历史（2026-08-06 从 P1-39 拆出，要动后端取数契约）
...
> **正确** = 有的都是真的（P1-37 回读侧标 `simulated`；P1-38 清 674 条假告警）；
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P1-25~~ ✅ → ~~P1-26~~ ✅ → ~~P1-30~~ ✅ → ~~P1-31~~ ✅ → ~~P1-32~~ ✅ → ~~P1-33（本地半）~~ ✅ → ~~P1-34~~ ✅ → ~~P1-35~~ ✅ → ~~P1-36~~ ✅ → **P1-28** → P1-45 → P1-46 → ~~P1-39~~ ✅ → P1-43 → P1-44 → P1-42 → P1-40 → P1-41 → P1-37 → P1-29 → P1-38 → P1-27 → P2-22 → P2-23 → P2-24 → P3-18 → P3-19**（2026-08-02 拍板二批队列 + 08-03 用户指定 P1-30 / P1-31 / P1-32 插队 + 08-05 用户指定 P1-34 插队 + **08-06 用户指定 P1-39 插队「优先完成」、P1-40 紧随**；**队首现为 P1-28**（08-06 用户指定插队），P1-25 / P1-26 / P1-30 / P1-31 / P1-32 / P1-33 本地半 / P1-34 / P1-35 / P1-36 / **P1-39** 已收口；一批 10 片已全收 #256–#265）|
...
> 『这个得现场验』的 backlog，同时问它落在哪个序列里」，但**从来没人做过这个映射**；
...
### P1-43 — 日志翻页：看得到 200 行（及扫描窗口）以外的历史 ⬜（2026-08-06 立项，从 P1-39 拆出）
...
**设计待定（先 review 再写码）**：游标式「加载更早 N 条」vs 时间范围选择器。
⚠️ 无论哪种都要先回答：`/tail` 被面板 **3 秒轮询**，取数不能整文件扫
（`:107` 那条注释就是为此设的）——翻页必须是**显式动作**触发的独立取数，
不能让轮询路径变重。
...
### P1-41 — 修 UXM 排错误队列停不下来的那个循环 ⬜（2026-08-06 立项，Codex #288 R1 促成）
...
**两个 bug 叠在一起**：① 终止条件挂在「队列排空了」上，而拿一条仪器不认识的命令去问，队列**必然永远不空**；② 该方言的错误查询命令形式可能本身就是错的（与 `Uxm5GNRTestAppProfile` 非 IRAT 方言那条同源 —— `uxm_command_profiles.py:66` 的 `ERR = "SYSTem:ERRor?"` 是否适用于所有方言**未经手册确认**）。

⚠️ **动手第一步是定位那 20 万行的实际调用栈，别假设** —— `uxm_base_station.py:3053-3058` 那处排队循环**已经有上界**了（`for _ in range(limit)`，docstring 还写着「原来是 `while True`，遇到一直吐错误的仪器会挂死」）。所以爆量要么来自**别的入口**（同文件 1338 行还有一个 `while True`，**未核**），要么上界是事后加的。

⚠️ **涉 UXM SCPI 语义，动手前必查 NotebookLM**（`236d9621-e3ce-4ed1-a8e1-7819b674dbcd`），且要拿**手册原文**不是它的推断 —— 问「`5G_NR_Test` / IRAT 各方言下错误队列查询的完整命令形式」以及「`-113` 之后正确的排空终止条件」。

**与 P1-40 的分工**：P1-40 ③ 的速率闸门是**止血**（保护磁盘，先做）；本片是**治因**。两件都要做，**闸门优先**。
...
- `[discovered 2026-08-02 during P1-25 内审 F5]` **[→ 提升 P3-18 (2026-08-03)]** **手写类型审计的尺子只看一层，"12 组 OK"不可信** —— P1-25 用"手写类型顶层键 ⊆ live 响应顶层键"过了 18 组，判 12 组 OK。内审实证至少一组是假绿：`MonitoringFeedsResponse` 顶层 `feeds` 对得上所以判 OK，但元素 live 是 `{name,value:number,unit,timestamp}` 而手写 `MetricItem` 是 `{label,value:string,trend?,id?}` —— **键名不同、值类型不同**；今天不炸只因消费点 `App.tsx` 的 `metricFeeds` 只写不读（eslint 有 unused 告警为旁证）。尺子的盲区：① 嵌套元素形态 ② 值类型 ③ 可选/null 语义 ④ 数组 vs 分页信封 ⑤ 只过了响应没过请求体 ⑥ 单向 ⊆ 只抓缺不抓多。正解 = 逐层键集合 + 值类型对 live JSON 递归比对（一次性脚本即可，不必进仓）。
