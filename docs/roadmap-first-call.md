...
**Current Focus = P1-57「全局 LabProfile / 当前暗室上下文统一」（设计完成，等待 Claude 实施；WIP=1）**。2026-08-19 人工测试发现同一工作区三套并行选择：探头与暗室配置显示 `CAICT-16-Probe-Dual`，射频拓扑编辑器独立选中 `CAICT-5-13`，暗室首测又使用 `CAICT-Lab-1`。本片把运行态唯一真值固定为“全局显式 LabProfile → `LabProfile.chamber_config_id` → ChamberConfiguration”：顶部统一选择并同时展示 LabProfile/暗室；拓扑编辑器删除独立目标暗室选择；topology API 按 `lab_profile_id` 复用 `resolve_current_chamber()`，不再相信自由提交的 chamber；未保存拓扑或已创建 commissioning session 时阻断切换。TestCase/历史记录的既有绑定保持独立。设计与 Claude 实施计划见 [`P1-57 设计`](plans/2026-08-19-p1-57-global-lab-chamber-context-design.md) / [`P1-57 计划`](plans/2026-08-19-p1-57-global-lab-chamber-context.md)。P2-28 已由 PR #352 完成并合并。P1-55
