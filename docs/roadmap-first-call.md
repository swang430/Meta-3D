...
P0-9 关闭后开放的 P0 只剩 P0-5（UXM 5G NR，等现场），按治理规则 2 焦点降到非现场队列：用户 2026-09-17 批准先做「现场改动落地」（已由 PR #480 完成），2026-09-18 批准其后按队列继续，**当前片 = P2-76**（P2-73 已由 PR #485 合并；P2-75 已由 PR #486 合并；P2-74 本地半已由 PR #482 合并；P1-80 软件半已完成，P1-2 仍待同机现场复验；见下方 Current Focus（非现场）行）。
...
Current Focus（非现场）= **P2-76：`input_level_calibration` 空回读不得记成功（本地实现已完成，待内审 / 外审）**。手动 F64 输入参考/crest 的「下发成功」与「功率验证成功」已拆开：Cell ON 前空/部分回读只记 `pending_cell_ready`，不假成功、也不提前中止；随后复用同次执行既有 Cell-ready 活动输入功率观测完成验证，缺失、部分读数、端口漂移或观测拒绝均保持 `success=false`，严格模式在方位角测量前 fail-closed，非严格模式只允许诊断继续。该片不新增 SCPI、不拿配置值冒充实测值，也不建立第二份功率真值。P2-75 已由 PR #486 合并：`instrument_idn_sweep` 只处理「所选 LabProfile binding × 当前启用类别」，并且不再发任意 `*IDN?`。P1-80 软件半已按[设计](plans/2026-09-22-p1-80-f64-license-query-attribution-design.md)完成；它不会替代 P1-2 的同机现场复验。P2-76 合并后队列：P2-77（信道资产频率语义，设计先行、须先有手册出处）；各片照常走「设计稿 → 用户过目 → 实现 → 内审 / 外审」，WIP = 1。P2-71 调研已形成[证据裁决](plans/2026-09-13-p2-71-f64-file-provisioning-research.md)，设备侧仍待现场；P2-57 / P2-70 已由 PR #476 / #475 合并。**
