...
**Current Focus = P2-27「修复 9 组前端手写契约与 live OpenAPI 不一致」（设计完成，TDD 待开始，WIP=1）**。P1-56 PR #350 的补充 Codex 外审已覆盖最终 merge commit（含尾修 `bd71879`）且无 P1；用户随后明确继续下一项。P2-27 已逐组裁决：运行时恒出的 Pydantic 默认字段应在 response serialization schema 中标 required，不把 GUI 响应机械可选化；真正的前端漏项与 chamber create 请求方向单独收窄。设计与计划见 [`P2-27 设计`](plans/2026-08-18-p2-27-openapi-contract-alignment-design.md) / [`P2-27 计划`](plans/2026-08-18-p2-27-openapi-contract-alignment.md)。P1-55
...
| **P2-27** | 修复 9 组前端手写契约与 live OpenAPI 不一致 | 🔄 设计完成；TDD 待开始 |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → ~~P1-50~~ ✅ → ~~P1-51~~ ✅ → ~~P1-52~~ ✅ → ~~P1-53~~ ✅ → ~~P2-26~~ ✅ → ~~P1-54~~ ✅ → ~~P1-55~~ ✅ → ~~P1-56~~ ✅ → **P2-27 🔄** → P2-28 → P2-29 → P2-30 → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；P1-56 补审已覆盖最终 merge commit 且无 P1。当前 WIP=1：P2-27 已完成九组真实可选性裁决与设计/计划，下一步按 TDD 修正 response serialization schema、前端真实漏字段与 chamber create 请求方向。现场物理单位/方向/偏置/型号实证仍保持 Hardware Blocked。完整编号、范围与状态只看顶部 Current Focus 表。 |
