...
P0-9 关闭后开放的 P0 只剩 P0-5（UXM 5G NR，等现场），按治理规则 2 焦点降到非现场队列：用户 2026-09-17 批准先做「现场改动落地」（已由 PR #480 完成），2026-09-18 批准其后按队列继续，**当前片 = P2-77**（P2-73 / P2-75 / P2-76 已由 PR #485 / #486 / #487 合并；P2-74 本地半已由 PR #482 合并；P1-80 软件半已完成，P1-2 仍待同机现场复验；见下方 Current Focus（非现场）行）。
...
Current Focus（非现场）= **P2-77：信道资产中心频率语义（软件实现中）**。PROPSIM User Reference Rev 10.2 已给出可核对边界：§20.4.6.1 / §20.4.6.2（pp.280–281）是逐通道组中心频率写入/回读，§20.4.6.8（p.282）是逐组允许范围，且越界请求可被仪表自动钳位。因此只对冻结 manifest 明确宣告 `set_center_frequency_bounded` 的 F64 vendor asset 把资产频率解释为工程默认；本次 TestCase 频率必须通过全组限值预检、写错误门和全组精确回读才成为执行真值。旧 manifest/plan、FS16、Mock 与非 vendor asset 继续按固定频率 fail-closed；GUI 只消费服务器语义，不复制频率范围。P2-76 已由 PR #487 合并。P2-77 软件验证不替代同一真实 F8800A 上的多频点/越界/逐组回读现场验收；各片照常走「设计稿 → 用户过目 → 实现 → 内审 / 外审」，WIP = 1。P2-71 调研已形成[证据裁决](plans/2026-09-13-p2-71-f64-file-provisioning-research.md)，设备侧仍待现场。**
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **当前唯一 WIP：P2-77（信道资产频率语义，软件实现 / 验证中）**。P2-73 / P2-75 / P2-76 已由 PR #485 / #486 / #487 合并；P2-74 本地半已由 PR #482 合并，P1-80 软件半已完成，P1-2 仍在 ON-SITE-BLOCKED。P1-79A～E 为低优先级候选，不在队列内。P2-68 / P2-69 / P2-70 已由 PR #471 / #473 / #475 合并；P2-57 静态资产来源声明和注册对账由 PR #476 合并，`.smu` 拓扑仍归 ChannelAsset 独立待评估。P2-71 设备侧发布/验约仍 Hardware Blocked。P2-77 合并后只关闭软件半，真实 F8800A 有界调频矩阵仍在 ON-SITE-BLOCKED。P1-77 / P1-76 已按 P3-23 记录试行数据。P2-32 位于功能启用池，P3-20/P3-21 位于非阻塞维护池，均不得自动启动。现场静区线性 XY 扫描平台仍保持 Hardware Blocked。 |
...
| D10 `.smu` 中心频可被 API 改写，资产频率不排他 | → **P2-77**（软件实现中；现场半 Hardware Blocked） | 手册边界已由 PROPSIM User Reference Rev 10.2 §20.4.6.1 / §20.4.6.2（pp.280–281）、§20.4.6.8（p.282）核实；采用 A 方案：vendor asset 登记频率只是工程默认，仅在冻结有界能力 + 全组限值预检 + 写错误门 + 全组精确回读全部成立时，TestCase 频率才是本次执行真值。旧冻结件/其他 adapter/其他 source type 不放宽 |
