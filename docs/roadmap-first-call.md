...
**Current Focus = 暂停（P1-56「转台动作真值门与诊断载体」本地半已由 PR #350 收口；Codex R2 的执行生命周期 stop-generation P1 已按 TDD 尾修，fresh 内审 P1=0，相关回归通过；按两轮上限直接合并并如实记录尾修未获外审覆盖，WIP=0）**。正式 TestCase 在创建 task 前、commissioning run-all/单阶段/adhoc 在等待租约前保留 generation，PRECHECK/REFERENCE 或排队期间急停后 MEASURE/cleanup 不得重新启动车台。按用户要求合并后暂停；不得自动开始 P2-27，等待用户明确继续。P1-55
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → ~~P1-50~~ ✅ → ~~P1-51~~ ✅ → ~~P1-52~~ ✅ → ~~P1-53~~ ✅ → ~~P2-26~~ ✅ → ~~P1-54~~ ✅ → ~~P1-55~~ ✅ → ~~P1-56~~ ✅ → P2-27 → P2-28 → P2-29 → P2-30 → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；P1-56 本地半由 PR #350 收口。真实 Aerotech `MOVEABS/HOME` 已具备有限 PFBK 变化与目标容差门；`aerotech_positioner_motion_truth` 仅在站点显式确认 degree user-units、安全范围和 feed 后运行，使用有仓内出处的 `MOVEABS ... XF...`，归档 VFBK/PFBK raw 轨迹，ABORT 只以全部实际轴有限 VFBK 精确为零保守确认停止。动作全集已收口单位环绕、无出处命令/位号、人工 stop 后重启、task-fault、断线重放、未证明单位假 degree、cleanup 停止未确认却断线、运动中/未知反馈污染稳定缓存，以及双轴健康门；正式 TestCase 与 commissioning 三入口在等待租约前保留同一 operation generation，MEASURE/cleanup 与真实 TX 锁内共同消费。R2 尾修 fresh 内审 P1=0、相关回归通过，按两轮上限未触发 R3。当前 WIP=0，按用户要求暂停，不自动开始 P2-27；现场物理单位/方向/偏置/型号实证仍保持 Hardware Blocked。完整编号、范围与状态只看顶部 Current Focus 表。 |
