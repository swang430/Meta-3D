...
P0-9 关闭后开放的 P0 只剩 P0-5（UXM 5G NR，等现场），按治理规则 2 焦点降到非现场队列：用户 2026-09-17 批准先做「现场改动落地」（已由 PR #480 完成），2026-09-18 批准其后按队列继续，**当前片 = P2-78**（P2-73 / P2-75 / P2-76 / P2-77 已由 PR #485 / #486 / #487 / #488 合并；P2-74 本地半已由 PR #482 合并；P1-80 软件半已完成，P1-2 仍待同机现场复验；P2-78 由用户 2026-09-22 独立 triage 后批准进入 todo；见下方 Current Focus（非现场）行）。
...
Current Focus（非现场）= **P2-78：F64 工程替换时清除旧 bounded-frequency proof（软件实现 / 验证中）**。P2-77 已由 PR #488 合并（merge `dcdb1b57`），但其外审发现 GCM/local、B2 与 ASC 三条 `CALC:FILT:FILE` 成功路径都可绕过 unload；新工程确认替换后、后续有界调频证明写入前若任务取消，上一工程的 proof 会继续留在驱动缓存。P2-78 在共同的“FILE 完成且紧邻错误门干净”边界清除旧 proof，并保证动作发生在首次后续 `await` 前；FTP 失败或 CLOSE 未确认时保留旧工程证明，FILE 被拒则沿用既有 unload/failure reset。不得新增/猜测 SCPI。P2-77 软件验证仍不替代同一真实 F8800A 上的多频点/越界/逐组回读现场验收；各片照常走「设计/边界 → 实现 → 内审 / 外审」，WIP = 1。P2-71 调研已形成[证据裁决](plans/2026-09-13-p2-71-f64-file-provisioning-research.md)，设备侧仍待现场。**
...
| D18 F64 工程替换后可能沿用上一工程 bounded-frequency proof | → **P2-78**（软件实现 / 验证中） | PR #488 R2 发现 GCM/local、B2、ASC 成功替换工程时都可绕过 unload，且替换后取消会把旧 proof 留给新工程；在 FILE 紧邻错误门确认干净后、首次后续 await 前统一清除，失败前置路径不得误清除 |
...
| **P1-79E**（= U-8） | `positioner.azimuth.NNN` 恒 `unknown`（角度回读无误，误差 0.0°） | Aerotech 驱动的身份快照把 `model` / `firmware_version` 写死为 `None`（无安全的型号 / 固件查询） | 前置：厂商手册里找到安全的身份查询；找不到就保持 unknown |
...
- `[→ P2-78；discovered 2026-09-22 during PR #488 R2；用户同日批准进入 todo]` **F64 工程替换后可能沿用上一工程的 bounded-frequency proof** —— GCM/local、B2 与 ASC 三条 `CALC:FILT:FILE` 成功路径都不经过 `_apply_unload()`；一旦 FILE 已替换工程、但任务在新 proof 写入前取消，旧工程的有界调频证明仍留在驱动缓存，后续消费者可能把它误认成新工程证据。验收：三条路径都在 FILE 完成且紧邻错误门干净后、首次后续 `await` 前清除旧 proof；FTP 失败或 CLOSE 未确认时保留旧工程证明，FILE 被拒则沿用既有 unload/failure reset；不新增/猜测 SCPI。
