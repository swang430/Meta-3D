...
**Current Focus = P3-19 / acquire 清理警告穿透子片** —— tail 字节预算、端口清理 allowlist、UXM SOCKET、UXM ARFCN、mock-only 与 calibration warnings 持久化已分别由 PR #331–#336 合并。当前片收口 QZ XPD/grid、方向图角度扫描、多频扫频四种循环：公共 acquire 可选接收 warning sink，并在每次调用（含异常）的 finally 立即收割，避免下一点开头清零前一点的 cleanup warning；各族成功/失败 `CalibrationResult` 均携带同一 warnings。验收只到**底层结果通道**，不宣称现有 GUI/live HTTP real 入口或三张结果表/报告已留痕。
