...
**Current Focus = P1-57「全局 LabProfile / 当前暗室上下文统一」（实施完毕待审：Task 1–6 已落地，GUI 契约 34 绿 + build/compileall/diff-check 过，后端全量与内审/外审进行中；WIP=1）**。2026-08-19 人工测试发现同一工作区三套并行选择：探头与暗室配置显示 `CAICT-16-Probe-Dual`，射频拓扑编辑器独立选中 `CAICT-5-13`，暗室首测又使用 `CAICT-Lab-1`。本片把运行态唯一真值固定为“全局显式 LabProfile → `LabProfile.chamber_config_id` → ChamberConfiguration”：顶部统一选择并同时展示 LabProfile/暗室；拓扑编辑器删除独立目标暗室选择；topology API 按 `lab_profile_id` 复用 `resolve_current_chamber()`，不再相信自由提交的 chamber；未保存拓扑或已创建 commissioning session 时阻断切换。TestCase/历史记录的既有绑定保持独立。设计与 Claude 实施计划见 [`P1-57 设计`](plans/2026-08-19-p1-57-global-lab-chamber-context-design.md) / [`P1-57 计划`](plans/2026-08-19-p1-57-global-lab-chamber-context.md)。P2-28 已由 PR #352 完成并合并。P1-55
...
| **P1-57** | 全局 LabProfile / 当前暗室上下文统一；拓扑、探头与首测不得各自选暗室 | 🔄 实施完毕待审（PR #353：R1 三条 P2 已修，R2 进行中） |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → ~~P1-50~~ ✅ → ~~P1-51~~ ✅ → ~~P1-52~~ ✅ → ~~P1-53~~ ✅ → ~~P2-26~~ ✅ → ~~P1-54~~ ✅ → ~~P1-55~~ ✅ → ~~P1-56~~ ✅ → ~~P2-27~~ ✅ → ~~P2-28~~ ✅ → **P1-57 🔄** → P2-29 → P2-30 → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；P1-57 已由 2026-08-19 人工测试确认并提升为 Current Focus；Task 1–6 已在独立 worktree 实施完毕（PR #353，外审进行中），状态细节看顶部 Current Focus。当前 WIP=1，不启动 P2-29。现场物理单位/方向/偏置/型号实证仍保持 Hardware Blocked。完整编号、范围与状态只看顶部 Current Focus 表。 |
