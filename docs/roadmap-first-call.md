...
**Current Focus = P2-26（设计完成，进入 TDD，WIP=1）**。P1-53 已由 PR #346（merge `da65888`）完成多暗室探头校准隔离，并按两轮 Codex 外审规则收口；R2 P1 尾修经 TDD、完整回归与 fresh 内审 P1/P2/P3=0 后合并，依规则未触发 R3。P2-26 复用 P1-27 已交付的 provenance-aware builder，为被 409 封锁的历史 MIMO 报告补后端恢复状态与 GUI 显式恢复入口：只有唯一关联执行仍存在时允许重生成；缺可信来源仍输出 UNKNOWN/N/A，不放宽详情/下载门，不自动批量改写历史。设计见 [`P2-26 设计`](plans/2026-08-16-p2-26-history-report-regeneration-design.md)。
...
| **P2-26** | 历史 MIMO 报告 UNKNOWN/N/A 的重新生成与恢复界面 | 🟡 设计完成 / TDD 中 |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → ~~P1-50~~ ✅ → ~~P1-51~~ ✅ → ~~P1-52~~ ✅ → ~~P1-53~~ ✅ → P2-26 🟡 → P2-27 → P2-28 → P2-29 → P2-30 → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；当前 P2-26 设计完成并进入 TDD，WIP=1；P1-53 已由 PR #346 完成并按两轮外审规则收口。完整编号、范围与状态只看顶部 Current Focus 表。 |
