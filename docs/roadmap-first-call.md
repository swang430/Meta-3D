...
**Current Focus = P3-18 / 手写类型递归审计收口子片** —— PDF 转义、G11 三覆盖面、p08 零残留与诊断序列串行化已分别由 PR #326/#327/#328/#329 合并。本片把 P1-25 的“只看顶层键”换成 live `app.openapi()` → 临时 `openapi-typescript` → TypeScript 双向/方向性 assignability，逐层覆盖嵌套、数组、值类型、optional/null 与请求体；审计临时生成物不进仓。按当前 live 消费全集复核 18 组：9 组方向安全、9 组仍有声明差异；同时按“去掉 > 换源”删除 7 条零消费 live fetch，并把演示监控帧改名为 `MockMonitoringFeedsResponse`，不再冒充 `/monitoring/feeds`。剩余 9 组已进 Discovered，不阻塞这个 P3 审计片。条目见下方 P3-18。
...
- **P3-19** 日志/告警/留痕卫生批（tail 上限 / app.log 噪声 / 校准 warnings 持久化+清理警告可见 / UXM 两组 P3 / **mock-only 遗留响应类型与 handler 收口** / **`is_docker_pid` 改 allowlist**）
...
| P3-18 | 门/测试精化批（G11 三覆盖面 in=location/curl 动词/schema 类型 + p08 零残留站点参数化 + PDF 渲染管道其余转义入口 + 诊断序列 run endpoint 串行化 + 手写类型审计尺子改逐层递归比对）——2026-08-02 拍板，各配变异。**PDF 子片已由 PR #326 合并**：封面与共享步骤自由文本统一在 `Paragraph` 边界转义，并去除 MIMO 双重转义。**G11 子片已由 PR #327 合并**：非 path 参数按 `(name, in)` 比对；散文识别 curl `-X/--request` 动词；响应 properties 前先拒绝 object/array 类型不兼容；合成 checker 变异同时锁住 location 与 schema 集成，完整规则门 50 passed。**p08 子片已由 PR #328 合并**：按当前实现全集枚举成功执行路径 10 个 `residue` 站点（9 个主流程窗口 + 共同收尾；中止专用站点已有独立失败场景门），同一条 fake 未认领错误逐站注入；任一站点删除都会让对应参数用例由 fail-closed 变成错误 PASS，变异实跑 1 failed / 其余 9 passed。**诊断序列串行化子片已由 PR #329 合并**：入口全集回查证明 PR #296 exclusion token + PR #304 全局 `InstrumentTestLease` 已共同覆盖序列与 F64 手工操作，本片补跨入口并发门。**手写类型递归审计子片本地完成**：从 live `app.openapi()` 临时生成 TS 真值，对当前 18 组请求/响应做方向性递归 assignability；9 组安全、9 组差异登记 Discovered。删除 7 条零消费 live fetch；`/monitoring/feeds` 的只写不读 query/state 一并删除，演示帧显式改名 `MockMonitoringFeedsResponse`。定点 + 完整规则门 52 passed，GUI production build 通过。 | 🔄 |
| P3-19 | 日志/告警/留痕卫生批（tail 反向扫描字节上限 + 执行失败告警通道 + 校准 warnings DB 持久化 + 借用 acquire 的清理失败警告可见 + UXM 终止符大小写 + UXM ARFCN 三条加固 + mock-only 遗留响应类型与 handler 收口 + `is_docker_pid` 改 allowlist）——2026-08-02 拍板。⚠️ **原含的「app.log 噪声治理」已于 2026-08-05 摘出提升为 P1-35**（用户手工测试当场要求提前），此处不再重复 | ⬜ |
...
- `[discovered 2026-08-02 during P1-25 全量审计]` **[→ 提升 P3-19 (2026-08-03)]** **四个手写 *Response 形态错，但对应 fetch 函数零消费** —— `TestCasesResponse` 声明 `cases` / `TestTemplatesResponse` 声明 `templates` / `ReportTemplatesResponse` 声明 `reportTemplates` / `DashboardResponse` 声明 camelCase 三键 `systemStatus`+`activeAlerts`+`liveMetrics`；而 live `/test-plans/cases` 返回 `{items,total}`、`/reports/templates` 返回 `{templates,total,page,page_size}`、`/dashboard` 返回 snake 四键 `{summary,live_metrics,active_alerts,recent_tests}`。**P3-18 手写类型子片已完成 live client 半**：`fetchDashboard` / `fetchTestTemplates` / `fetchTestCases` / `fetchRecentTests` / `fetchAlerts` / `fetchReportTemplates` / `fetchMonitoringFeeds` 七条零消费函数均删除，不再有人把这些类型当 live 响应。**P3-19 剩余范围收窄为 mock-only 遗留**：上述演示 fixture 类型与 `mockServer` handler 仍在，需按真实用途改名或删除；不要再回填成 live 契约。⚠️ 若保留 fixture，嵌套层必须单独命名，不能再让 `RecentTest` 等演示形态冒充 live。
...
- `[discovered 2026-08-02 during 上条内审 F4]` **[→ 提升 P3-19 (2026-08-03)]** **`is_docker_pid` 是 denylist，默认动作是"杀"，只覆盖 macOS/Linux 三种形态** —— 判据 = `ps -o comm=` 含 `com.docker` / `docker-proxy` / `rootlesskit`。**未覆盖且失效是静默的**（直接回到"杀"）：colima/lima 用 host 上的 `ssh -L` 做转发（拿 ssh 当判据会误杀用户自己的 ssh，宁可漏判也没加）、OrbStack / Rancher Desktop 的 helper 进程名无环境实证（不猜）。彻底解法 = 反过来做 **allowlist**：只杀 comm 命中 `python|node|uvicorn|vite` 这类自家 dev 进程，其余一律不杀 —— 让"没见过的进程"落到**不杀**（代价 = 端口没腾出来 + 一条提示）而不是**杀**（代价 = 别人的引擎/网络栈全死）那一侧，且不必逐个厂商枚举。独立小片。
