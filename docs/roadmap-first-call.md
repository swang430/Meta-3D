...
**Current Focus = P3-19 / 端口清理进程 allowlist 子片** —— P3-18 五片已由 PR #326–#330 全部合并；P3-19 首片 tail/history 16 MiB 字节预算已由 PR #331 合并。当前片把端口清理从容器进程 denylist 反转为本项目开发进程 allowlist：只有进程类型命中 `python*` / `node` / `uvicorn` / `vite` **且 cwd 位于本仓**才可被自动终止，Docker、ssh、其他项目、未知 helper 与身份取证失败的 PID 一律不杀并让启动 fail-closed。两条调用链 `safe-start.sh` / `cleanup-ports.sh` 共用 `port-guard.sh` 判据，不混入 UXM 或日志生产者。
...
| P3-19 | 日志/告警/留痕卫生批（~~tail 反向扫描字节上限~~ ✅ PR #331 + 执行失败告警通道 + 校准 warnings DB 持久化 + 借用 acquire 的清理失败警告可见 + UXM 终止符大小写 + UXM ARFCN 三条加固 + mock-only 遗留响应类型与 handler 收口 + **当前片：端口清理进程 allowlist**）——2026-08-02 拍板。**拆序**：~~tail 字节预算~~ ✅ → 端口清理安全方向 → UXM 两片 → mock-only 类型 → warnings 两片 → 日志/告警生产者。⚠️ 原含的 app.log 噪声治理已于 2026-08-05 摘出提升为 P1-35。 | 🔄 |
