...
Current Focus（非现场）= P2-64（✅ 本地完成，待外审合并；下一片 P2-65）。**
...
**~~P1-74~~（✅ 非现场半 #429，现场半待真机） → ~~P1-75~~（✅ #431） → ~~P2-64~~（✅ 本地完成，待外审） → P2-65 → P2-66 → P2-67 → P2-54 → P2-55 → P2-56 →
...
在真实型号与手册确定前保持 HOLD，位于队列末尾且不阻断任何已批准片。P2-64 已按批准启动并完成本地实现，合并前 WIP 仍为 1；P2-65 尚未启动。
...
2. **非现场队列（本地开发 / 手册取证 / 自动化验证）**：先消化已经取得的现场证据；
   **P2-9 的精确 `ERROR 3` 分类与 NEW-1 的活动 F64 输出集合本地半均已完成**；
   ...
   下一批：**当前非现场 WIP = P2-64 外审收口**（Adapter-scoped Mock 能力；本地实现已完成），随后 P2-65～P2-67
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **当前非现场 WIP=P2-64 外审收口（本地实现已完成）**；NEW-1/P2-42～P2-53 已分别由 PR #407/#408/#409/#410/#411/#412/#413/#414/#415/#417/#420/#422/#424 合并，后续现场热修/分型号配置保存由 PR #425/#426/#427 合并。已批准的雷达顺序为 ~~P1-74~~（✅ 非现场半 #429）→ ~~P1-75~~（✅ #431）→ **~~P2-64~~（✅ 本地完成，待外审）** → P2-65～P2-67 → P2-54～P2-62 → P2-63（HOLD，队列末尾）。该本地顺序不关闭、不降级 ON-SITE-BLOCKED 中原有 First-call Todo。P2-32 位于功能启用池，P3-20/P3-21 位于非阻塞维护池，均不得自动启动。现场静区线性 XY 扫描平台仍保持 Hardware Blocked。 |
...
### P2-64 — Adapter-scoped Mock 能力与注册身份（✅ 本地完成，待外审合并）
...
**本地实现（2026-09-01）**：HAL 在构造唯一 `MockBaseStation` 时注入 selected model 的已注册、
...
### P2-66 — BaseStation 执行证据不变量与终态语义（批准入雷达，未启动）
...
- `[resolved locally by P2-64 on 2026-09-01；discovered 2026-08-30 during UXM/CMW500 Mock 对照执行]` **MockBaseStation 使用多厂商能力并集并靠型号字符串猜 adapter（P2）** —— 已删除能力并集与型号嗅探；HAL 注入 selected registered manifest，Mock 的身份、RAT、operation、窗口、指标及 UE 诊断 RAT 形状只从该真值派生，未知/漂移 fail-loud，模拟值仍不进入正式 KPI。**出口：→ P2-64（本地完成，待外审合并）**。
