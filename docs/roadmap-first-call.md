...
**Current Focus = P3-19 / UXM ARFCN custom map 加固子片** —— P3-19 的 tail 字节预算、端口清理 allowlist、UXM SOCKET 终止符已分别由 PR #331/#332/#333 合并。当前片收口三项既有配置边界：attach schema 与 runtime fallback 共用一份默认值；`nr_band_arfcn_map` 大小写键归一后若值冲突即在驱动加载时失败；整数字符串在加载时归一为 `int`，非整数/布尔/域外值同样 fail-loud。下一片才处理 mock-only 类型与 handler。
...
| P3-19 | 日志/告警/留痕卫生批（~~tail 反向扫描字节上限~~ ✅ PR #331 + ~~端口清理进程 allowlist~~ ✅ PR #332 + ~~UXM 终止符大小写~~ ✅ PR #333 + 执行失败告警通道 + 校准 warnings DB 持久化 + 借用 acquire 的清理失败警告可见 + **当前片：UXM ARFCN 三条加固** + mock-only 遗留响应类型与 handler 收口）——2026-08-02 拍板。**拆序**：~~tail 字节预算~~ ✅ → ~~端口清理安全方向~~ ✅ → ~~UXM SOCKET~~ ✅ → UXM ARFCN → mock-only 类型 → warnings 两片 → 日志/告警生产者。⚠️ 原含的 app.log 噪声治理已于 2026-08-05 摘出提升为 P1-35。 | 🔄 |
