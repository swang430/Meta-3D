...
**Current Focus = P1-29**（2026-08-10 起）—— `/dashboard/alerts/summary` 被路由遮蔽，驾驶舱告警计数条恒坏，修法是挪一行声明顺序。条目见下方 P1-29。

> **~~P1-48~~ ✅ 2026-08-10 完成**（2026-08-09 插队，兼作 Gemini 外审首测对象）。六片全部 merge 进 main：#307 S0 / #308 日志线 / #313 删掉四条整体返回随机数的报告接口（净 −898 行）/ #312 路损校准拒绝模拟驱动 / #310 报告线 / #314 虚拟路测不再编数。
> **代价记录**：外审 **24 轮 52 条**（P1 35 条，口径与逐 PR 明细见下方 P1-48 条目），其中 #314 一个 PR 占 10 轮 22 条；复盘后 ①内审改成每次 push 前都过（新增轻量档）②「改之前先列全集」写进三份规则文档 ③规则整理 #316（消 8 处手工同步契约、轮次上限改分级）。
...
本地序列切到 **P1-29**（P1-48 已于 2026-08-10 收官，见上方 Current Focus）。本轮日志设计与逐片实施计划见
...
- ~~**P1-48**~~ ✅ 日志/报告分不出哪台仪表是真的 —— P1-37 的标记只到 `scpi.log`，app.log / 报告 / VRT 三个消费端一个没接上（2026-08-10 六片全合）
...
### ~~P1-48~~ — 日志与报告分不出哪台仪表是真的（P1-37 的三个缺口）✅（2026-08-10 完成，六片全合）

> **✅ 2026-08-10 收官记录**（下面原文是开工前写的方案，保留备查）
>
> | 片 | PR | 做了什么 |
> |---|---|---|
> | S0 基础 | [#307](https://github.com/swang430/Meta-3D/pull/307) | 两个自称真机的 mock 驱动改回实话；`_MOCK_FALLBACK_BY_CATEGORY` 成为唯一 mock 表，`_MOCK_DRIVER_CLASSES` 从它派生（外审连三轮攻破守门测试后的根因修复） |
> | 日志线 | [#308](https://github.com/swang430/Meta-3D/pull/308) | `hal_mode` 改成按每台仪表自己的标取值，不再读全局单例；`app.hal.scpi` 之外的三处直连 logger 也接上标记 |
> | 删编数接口 | [#313](https://github.com/swang430/Meta-3D/pull/313) | 删掉四条整体返回随机数的报告接口（**+70/−968，净 −898**）：`/statistics/compare`、`/benchmark`、`/time-series` 等 |
...
>
> **代价与复盘**：外审 **24 轮 52 条**（其中 P1 35 条）——
> 口径：`gh api repos/:owner/:repo/pulls/<n>/comments` 的 inline 条数，逐 PR 为
> #307 4轮9条 / #308 4轮10条 / #310 4轮7条 / #312 2轮4条 / #313 0轮0条 / #314 10轮22条。
> **#314 一个 PR 就占 10 轮 22 条**（下面那句 12 轮是把两次「推了但没触发外审」也算进去的口径，
> 按 review 记录只有 10 轮）。
> ⚠️ 本行初版写「27 轮 30 条」，是凭印象填的、没重新导出 —— 外审 #317 抓出。
