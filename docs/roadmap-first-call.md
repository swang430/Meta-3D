...
| **P1-64** | 无真实静区纹波证据时不得显示预检通过或把 0.7 dB 兜底叙述为实测 | 🔄 Current Focus；TDD 实现完成，完整回归与 fresh 内审中 |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | P1-64，WIP=1。P2-32 位于功能启用池，P3-20/P3-21 位于非阻塞维护池，均不得自动启动。现场静区线性 XY 扫描平台及物理单位/方向/偏置/型号实证仍保持 Hardware Blocked。完整编号、范围与状态只看顶部 Current Focus 表。 |
