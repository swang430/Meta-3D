...
Codex R2 又指出旧 PDF 只带校准/吞吐 schema 标记时仍可绕过新应用快照门直接发布旧 PASS；
共享报告信任谓词已收窄为必须同时具备规范化 `path_loss_application` 与自洽的
`formal_path_loss_verified`，旧产物转入安全重建。验证：R2 受影响链 + 完整 rule gates
**272 passed**、GUI 契约 **5 passed**、全后端 **4313 passed / 5 skipped**、GUI production
build、`compileall`、单一 Alembic head 与 `diff-check` 均通过；R2 尾修 fresh 内审
**P1/P2/P3=0**，待覆盖最新 HEAD 的 R3。设计与证据见
...
| **P1-62** | 已应用但来源未知的路损证书不得被叙述为“无证书/未补偿” | 🔄 PR #373；R1/R2 的正式判定与旧 PDF 信任绕过均已按 TDD 修复，回归/尾审通过，待 R3 |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | P1-62，WIP=1；R2 P1 尾修、回归与内审已完成，待覆盖最新 HEAD 的 R3。P2-32 位于功能启用池，P3-20/P3-21 位于非阻塞维护池，均不得自动启动。现场物理单位/方向/偏置/型号实证仍保持 Hardware Blocked。完整编号、范围与状态只看顶部 Current Focus 表。 |
