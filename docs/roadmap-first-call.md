...
**Current Focus = P2-28「诊断序列持久化完整 raw 或可靠 trace pointer」（设计中，WIP=1）**。P2-27 已由 PR #351（merge `6d3e4b5`）完成九组手写契约与 live OpenAPI 对齐；R1 两条 P2 已按 TDD 修复，R2 覆盖最终 HEAD 且无 P1，仅报告一条“后续拆分 connection 请求/响应 schema”的 P2，按两轮规则不阻塞、不自动积压。P2-28 采用持久化结构化 `sequence_evidence` 的保守方案：与审计行同生命周期保存 `summary/log/steps.raw/extra/duration`，详情按 id 读取，列表保持轻量；不依赖会轮转的日志指针，不从旧 2KB 摘要猜证据。baseline 诊断/迁移/完整 rule gates 146 passed。设计见 [`P2-28 设计`](plans/2026-08-18-p2-28-diagnostic-evidence-persistence-design.md)。P2-27 的设计与计划见 [`P2-27 设计`](plans/2026-08-18-p2-27-openapi-contract-alignment-design.md) / [`P2-27 计划`](plans/2026-08-18-p2-27-openapi-contract-alignment.md)。P1-55
...
| **P2-28** | 诊断序列持久化完整 raw 或可靠 trace pointer | 🔄 设计中；baseline 146 passed |
...
| **P2-33** | 日志体验包：CRITICAL、traceback 搜索、级别多选、重复请求抑制 | ⬜ |
| **P2-34** | 正式执行失败告警的发布结果契约 | ⬜ |
| **P3-20** | 删除或重写失效的校准导入/导出链 | ⬜ |
| **P3-21** | 统一 UXM 两套诊断错误队列读取逻辑 | ⬜ |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → ~~P1-50~~ ✅ → ~~P1-51~~ ✅ → ~~P1-52~~ ✅ → ~~P1-53~~ ✅ → ~~P2-26~~ ✅ → ~~P1-54~~ ✅ → ~~P1-55~~ ✅ → ~~P1-56~~ ✅ → ~~P2-27~~ ✅ → **P2-28 🔄** → P2-29 → P2-30 → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；P2-27 已由 PR #351 merge `6d3e4b5` 收口，R2 无 P1。当前 WIP=1：P2-28 已完成入口/消费全集与方案裁决，选择与审计行同生命周期的结构化完整证据，不依赖不可靠日志指针；baseline 146 passed，下一步写实施计划并按 TDD 开发。现场物理单位/方向/偏置/型号实证仍保持 Hardware Blocked。完整编号、范围与状态只看顶部 Current Focus 表。 |
...
- `[discovered 2026-08-06 during P1-45 external review, Codex #295 R1]` **[→ 提升 P2-28，2026-08-18 已启动] 诊断序列完整 output / trace pointer 持久化缺口** —— `POST /api/v1/diagnostic-sequences/{key}/run` 的 live response 含完整 `steps/raw`，但 `DiagnosticContext.record_run()` 只把组合输出截成最多约 2048 bytes 的 `DiagnosticRun.output_excerpt`，该 endpoint 又没有传 `hal_trace_log_path`；离开 live 结果后，审计行可能既没有完整 raw，也没有可回取全量 trace 的指针。P2-28 已裁决采用与审计行同生命周期的结构化 `sequence_evidence`，不依赖日志轮转/清理后可能失效的指针，也不从旧摘要猜测证据。
