...
### P1-39 — 让人拿得到 ID：执行/用例编号在界面上可见可复制、一键跳日志 ⬜（2026-08-06 立项，**用户指定优先完成**）
...
5. **系统日志默认新在最上 + 方向可切**（2026-08-06 用户当场追加）。
...
⚠️ **第 5 条的前提先纠正**：用户说「系统日志最新在最下方，实时日志不是这样」——
实测**两个面板排序完全相同**（都是旧上新下）。真正的差别是
`ZoneLogsAlerts:166-169` 有 `autoScroll` 自动滚到底，`SystemLogViewer` 没有。
选**降序**而不是照抄 autoScroll 的理由：系统日志是**查证面板**不是 live tail，
且**一旦加了过滤条件，自动滚动就没有意义**（结果不是流式追加的）。
