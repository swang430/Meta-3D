...
**Current Focus = P3-19 / mock-only 遗留类型与 handler 收口子片** —— P3-19 的 tail 字节预算、端口清理 allowlist、UXM SOCKET、UXM ARFCN 已分别由 PR #331–#334 合并。P3-18 已删除七条零消费 live fetch；当前片继续删除它们遗留的 live-looking mock REST handler 与说谎 `*Response` 类型，并把仍需保留的 localStorage 演示 TestCase 嵌套类型显式改名 `MockTestCase*`。WebSocket 演示仍消费的 `MockMonitoringFeedsResponse` 与活动 `/dashboard/alerts/summary` mock handler 保留。下一片才处理 calibration warnings。
