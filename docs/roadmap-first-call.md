...
踩红两道厂商分支规则门；**第 0 步「现场代码落地」已由 PR #480 合并（merge `7aa652da`，2026-09-18）**：6 个现场提交 + 当时未提交的代码合成一个 PR 整体过审（三路全套内审 P1=0 / P2=8 / P3=8 全部处理；外审 Gemini R1→R2 无 high）。过审时去掉 / 改掉了现场的三处：功率观察的厂商分支、F64「接受 CLOSED」（真机不可达）、观察状态名（真机无信号读数是 −108 dBm，不是空值）。**用户 2026-09-18 批准其后按队列顺序继续**：P2-74（转台传输稳定性）→ P1-80（P1-2 许可序列修复）→ P2-73（`propsim_f64_p08_gate` 限 UXM）→ P2-75（`instrument_idn_sweep` 范围与身份）→ P2-76（`input_level_calibration` 空回读不得记成功）→ P2-77（信道资产频率语义，设计先行、须先有手册出处）。
...
| D12 功率观察窗的厂商分支踩红两道规则门 | ✅ **resolved（PR #480）** | 换成 `_cell_ready_power_observation_is_wired()`：看基站驱动是否实现配置功率回读，不看厂商身份；两道门转绿。取舍：判据来源是驱动实现而非冻结 manifest（走 manifest 需要 Mock 基站也支持回调，未做） |
...
- `[discovered 2026-09-18 during PR #480 内审；待 triage]` **Cell-ready 功率观察的严格门判不了「无信号」** —— 真机上无输入信号的实测读数是很低的有限值，不是空值：09-16 执行 `f8f5fd90`（B41 TDD）F64 输入口 2 两次采样均为 −108.0 dBm、输入口 1 约 −29 dBm，16 路输出均值 −52.3 / −53.8 dBm，比上一次执行 `d058d9de` 的 −50.0 / −49.8 dBm 低约 2.5–4 dB（`app.log.2026-09-16`；现场总结原稿未记）。严格门只拦「读不到数」，缺一路 TX 时照常放行（#480 已把放行状态改名为 `recorded` 并写明局限）。要判有无信号须换到带该语义的查询并有 PROPSIM 手册出处，不凭一次观察值加阈值。
- `[discovered 2026-09-18 during PR #480 内审；保留候选]` **三条观察，暂不立片** —— ① CMW 单发窗口「墙钟 + 1 s 宽限」只有一个真机样本（`dbd53e6f`：INIT 后 5.14 s 仍 RUN、5.24 s 到 RDY），大统计基下滞后是否变大未知，超宽限的代价是白跑一窗；② `test_p2_46` 的 AST 门抓不到 `getattr(x, "adapter_id") == "cmw500"` 这种写法（P2-43 门抓得到）；③ `ebler_repetition_continuous` 生产零调用，只剩命令表测试引用。
