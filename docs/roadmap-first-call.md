...
**删掉 4 / 换地方取数 4 / 收紧 1 / 新加一套东西 1 处**（S7 已取消；但外审指出虚拟路测那条链没有读模拟标记的地方，S1 里必须新加一处） —— ⚠️ **原写"加机制 0"与同句里
"唯一那片加机制"自相矛盾**（外审 P2 纠正），现统一为 1。S7 加的是一句固定免责文案，
不是字段/列/服务，且设计稿里论证了去掉/换源/收窄三种都不成立。**零新数据库列、零新 API 字段、零新服务。**
⏸️ **等用户拍板 5 个决策点后才动代码。**
