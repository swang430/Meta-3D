...
P0-9 关闭后开放的 P0 只剩 P0-5（UXM 5G NR，等现场），按治理规则 2 焦点降到非现场队列：用户 2026-09-17 批准先做「现场改动落地」（已由 PR #480 完成），2026-09-18 批准其后按队列继续，**当前片 = P2-75**（P2-73 已由 PR #485 合并；P2-74 本地半已由 PR #482 合并；P1-80 软件半已完成，P1-2 仍待同机现场复验；见下方 Current Focus（非现场）行）。
...
Current Focus（非现场）= **P2-75：`instrument_idn_sweep` 范围与身份（本地实现已完成，待内审 / 外审）**。该序列现只处理「所选 LabProfile binding × 当前启用类别」，逐项对照服务器当前 selected model、saved endpoint 与 loaded adapter；CMW500 / UXM / F64 只投影 connect 时已缓存的身份，转台 / RF Switch 明示 `not_applicable`，Mock / 缺失快照为 `UNDETERMINED`，配置或 adapter 漂移为 `BLOCKER`。序列不再发任意 `*IDN?`，也不再为历史 binding 抢占仪器控制权。P2-73 已收口：`propsim_f64_p08_gate` 的 metadata 要求 BaseStation，租约锁内复用服务器权威 resolver，仅真实 UXM 可在 Remote/F64 I/O 前通过；CMW500、Mock、缺少/漂移均 fail-closed，且诊断结果归档同一份冻结 binding。P1-80 软件半已按[设计](plans/2026-09-22-p1-80-f64-license-query-attribution-design.md)完成；它不会替代 P1-2 的同机现场复验。P2-75 合并后队列：P2-76（`input_level_calibration` 空回读不得记成功）→ P2-77（信道资产频率语义，设计先行、须先有手册出处）；各片照常走「设计稿 → 用户过目 → 实现 → 内审 / 外审」，WIP = 1。P2-71 调研已形成[证据裁决](plans/2026-09-13-p2-71-f64-file-provisioning-research.md)，设备侧仍待现场；P2-57 / P2-70 已由 PR #476 / #475 合并。**
...
| D1 `instrument_idn_sweep` 范围与身份契约 | → **P2-75**（本地完成，待外审） | 「所选 LabProfile binding × 当前启用类别」+ connect 缓存身份投影；零新增仪器 I/O |
