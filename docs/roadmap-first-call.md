...
| 界面 | 显示 ID 吗 |
|---|---|
| 测试用例库（`TestCaseLibrary.tsx:434`） | ❌ `tc.id` 只当 React `key`，不渲染 |
| 执行历史（`HistoryTab.tsx:302/378`） | ❌ `record.id` 只当 `key` 与「生成报告」的 loading 标记 |
| 报告详情（`ReportViewer.tsx:279-284`） | ⚠️ 有「执行ID」卡片，但 **`slice(0,12)` 截断、不可复制**，且**只有 execution id，测试例 id 一个字都没有** |
...
1. 执行历史每行加 ID 列（显示前 8 位，点击复制全长）
2. 用例库卡片同样加（测试例 ID，规格同上）
3. 报告页「执行ID」卡片改成**可复制全长**（去掉 `slice`）
4. **一键跳转** —— 执行历史那行加「看日志」按钮，直接带 `execution_id` 跳到
   系统日志页并预填过滤。前 3 条只是让人能**手抄**，第 4 条让人**不用抄**。
