...
- `[discovered 2026-07-04 during #206 补扫]` **QZ 验证 / 方向图扫描借用 acquire 的清理失败警告不可见** —— `quiet_zone_validation_service`（XPD ×2 + grid 扫描）与 `probe_calibration_service`（方向图扫描）各自新建局部 `ProbePathLossCalibrationService` 调 `acquire_sa_power_via_ce_tone`，acquire 把 stop_tx/clear_passthrough/stop_tone 清理失败 append 进 `_last_acquire_warnings`，但这三条流程从不收割、且下一次 acquire 开头即清空 → 清理失败在 QZ/方向图结果里静默（校准证书路径 #206 已修全出口收割）。proper = 三个 helper 加 warnings 穿透（签名改动）+ 各自结果对象带 warnings。
