...
Current Focus（非现场）= P2-66（本地实现完成，正在验证/审查；P2-65 已由 PR #434 完成）。**
...
在真实型号与手册确定前保持 HOLD，位于队列末尾且不阻断任何已批准片。P2-64/P2-65 已由 PR #433/#434 完成；P2-66 本地实现完成、待审查合并，WIP=1。
...
2. **非现场队列（本地开发 / 手册取证 / 自动化验证）**：先消化已经取得的现场证据；
   **P2-9 的精确 `ERROR 3` 分类与 NEW-1 的活动 F64 输出集合本地半均已完成**；
   ...
   下一批：**P2-64/P2-65 已由 PR #433/#434 完成；P2-66 本地实现完成、正在验证/审查**；P2-67
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **当前非现场 WIP=1：P2-66 本地实现完成，正在验证/审查；下一项 P2-67**；NEW-1/P2-42～P2-53 已分别由 PR #407/#408/#409/#410/#411/#412/#413/#414/#415/#417/#420/#422/#424 合并，后续现场热修/分型号配置保存由 PR #425/#426/#427 合并。已批准的雷达顺序为 ~~P1-74~~（✅ 非现场半 #429）→ ~~P1-75~~（✅ #431）→ **~~P2-64~~（✅ #433）→ ~~P2-65~~（✅ #434）** → P2-66 → P2-67 → P2-54～P2-62 → P2-63（HOLD，队列末尾）。该本地顺序不关闭、不降级 ON-SITE-BLOCKED 中原有 First-call Todo。P2-32 位于功能启用池，P3-20/P3-21 位于非阻塞维护池，均不得自动启动。现场静区线性 XY 扫描平台仍保持 Hardware Blocked。 |
...
### P2-54 — RAT-neutral MAC Test Profile（批准入雷达，未启动）
...
### P2-66 — BaseStation 执行证据不变量与终态语义（本地实现完成，待审查合并）
...
**本地实现（2026-09-01）**：执行冻结重验 compatibility 外层/内层 digest、requirements、verdict 与
...
终态入口。当前正在完成全量验证、fresh 内审与外审，现场 blocker 未改变。

### P2-67 — BaseStation 日志与导出可追溯性（批准入雷达，未启动）
...
- `[local implementation complete by P2-66 on 2026-09-01；discovered 2026-08-30 during UXM/CMW500 Mock 对照执行]` **执行证据曾允许 manifest 与 requested config 自相矛盾且 `completed` 易形成成功假象（P2）** —— 共同 outcome 现已校验冻结 compatibility 并区分 pipeline/diagnostic/valid-test 终态，全部正式消费者 fail-closed；正在完成验证与审查。**出口：→ P2-66**。
