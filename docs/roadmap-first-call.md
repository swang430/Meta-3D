...
- `[discovered 2026-07-04 during Codex 兜底 R10]` **F64 九个参数设置方法 `_check_errors` 只 log 的同型假成功平行族** —— R10 修了 tone 族(set/stop calibration_tone 假成功,SCPI 拒绝只进错误队列不抛异常),域枚举发现 `set_path_loss / set_doppler / set_baseband_power / set_external_attenuators / set_output_path_loss / set_output_gain / set_crest_factor / set_input_measurement_mode / set_burst_trigger_level` 九个方法同型:写后 `_check_errors()`(→None 只 log)→ `return True`。其中 set_path_loss/set_output_gain 在校准补偿下发链,假成功 = 补偿没生效静默。agent 门审 F1 补两站点:**`upload_asc_files`(:1022)`CALC:FILT:FILE` 载入全程无 `_first_error` 门**(与 set_channel_model load 门自述的 P0-8 母题同型,Pipeline B 真硬件假成功比参数方法更重)+ `set_channel_model` 尾段(:951,CENT×N + ROUT 在 load 门外)。proper = 逐个换锁事务 + `_first_error` 门(五事务同型),共 11 站点,规模属独立 PR;`_check_errors` 届时评估是否退役。
