...
**Current Focus = P2-26（实现完成，内审 P1 收口中，WIP=1）**。P1-53 已由 PR #346（merge `da65888`）完成多暗室探头校准隔离；它按当时“两轮后直接收口”的规则在 R2 P1 尾修后合并，尾修未获 R3 覆盖。2026-08-16 用户已纠正规则：后续若 R2 仍有 P1，修复后必须继续 R3，直到覆盖最新 HEAD 的外审无 P1；P2/P3 不因此扩轮。P2-26 复用 P1-27 已交付的 provenance-aware builder，为被 409 封锁的历史 MIMO 报告补后端恢复状态与 GUI 显式恢复入口：只有满足单执行、非 VRT、PDF、唯一关联执行存在且未被其他恢复占用时才允许原子认领并重生成；缺可信来源仍输出 UNKNOWN/N/A，不放宽详情/下载门，不自动批量改写历史。设计见 [`P2-26 设计`](plans/2026-08-16-p2-26-history-report-regeneration-design.md)。
...
| **P2-26** | 历史 MIMO 报告 UNKNOWN/N/A 的重新生成与恢复界面 | 🟡 实现完成 / 内审 P1 收口中 |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → ~~P1-50~~ ✅ → ~~P1-51~~ ✅ → ~~P1-52~~ ✅ → ~~P1-53~~ ✅ → P2-26 🟡 → P1-54 → P1-55 → P1-56 → P2-27 → P2-28 → P2-29 → P2-30 → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；当前 P2-26 实现完成并处于内审 P1 收口，WIP=1；P1-54～56 已按用户要求预留为紧随其后的优先槽位，具体可观察故障与验收范围尚待补齐，未在本文虚构。完整编号、范围与状态只看顶部 Current Focus 表。 |
