...
**Current Focus = P2-26（PR #347 R2 P1 已修复、内审 P1/P2/P3=0，待 R3，WIP=1）**。P1-53 已由 PR #346（merge `da65888`）完成多暗室探头校准隔离；它按当时“两轮后直接收口”的规则在 R2 P1 尾修后合并，尾修未获 R3 覆盖。2026-08-16 用户已纠正规则：后续若 R2 仍有 P1，修复后必须继续 R3，直到覆盖最新 HEAD 的外审无 P1；P2/P3 不因此扩轮。P2-26 复用 P1-27 已交付的 provenance-aware builder，为被 409 封锁的历史 MIMO 报告补后端恢复状态与 GUI 显式恢复入口：只有满足单执行、非 VRT、PDF、唯一关联执行存在且经权威判据确认为 MIMO OTA、并且未被其他恢复占用时才允许原子认领并重生成；缺可信来源仍输出 UNKNOWN/N/A，不放宽详情/下载门，不自动批量改写历史。R1 指出的执行来源漏验已按 TDD 修复；R2 指出的 VRT archive 状态写点已收窄为数据库条件更新，已有 `generating` claim 时不得写回 `completed/pending` 或启动第二个 writer。R1 的崩溃残留 P2 不以启动自动复位实现：Gunicorn 平滑替换可让新旧进程短暂重叠，在没有 owner/lease 真值时复位会误放同一正式报告的并发写，因此维持 fail-closed。验证：报告链与完整 rule gates 144 passed、GUI 契约 4 passed、production build、compileall、diff-check 通过。设计见 [`P2-26 设计`](plans/2026-08-16-p2-26-history-report-regeneration-design.md)。
...
| **P2-26** | 历史 MIMO 报告 UNKNOWN/N/A 的重新生成与恢复界面 | 🟢 PR #347 R2 P1 已修复 / 待 R3 |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → ~~P1-50~~ ✅ → ~~P1-51~~ ✅ → ~~P1-52~~ ✅ → ~~P1-53~~ ✅ → P2-26 🟢 → P1-54 → P1-55 → P1-56 → P2-27 → P2-28 → P2-29 → P2-30 → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；当前 P2-26 已修复 PR #347 R2 的 VRT archive claim P1并通过 fresh 内审，待 R3；崩溃残留 claim 在缺 owner/lease 真值时维持 fail-closed，不做可能误放并发写的启动猜测复位；WIP=1。P1-54～56 已按用户要求预留为紧随其后的优先槽位，具体可观察故障与验收范围尚待补齐，未在本文虚构。完整编号、范围与状态只看顶部 Current Focus 表。 |
