...
**Current Focus = P1-73「CMW500 单载波 LTE 2×2 MIMO OTA」已立项，架构方案 #1 已获用户批准，当前进入完整设计与实施计划。2026-08-24 批准队列六片已全部完成：~~P2-41~~ ✅ #386 → ~~P1-68~~ ✅ #388 → ~~P1-69~~ ✅（设计稿获批）→ ~~P1-70~~ ✅ #392 → ~~P1-71~~ ✅ #394 → ~~P1-72~~ ✅ #396；P2-32 与维护池 P3-20/P3-21 按既定规矩不得自动启动。**

> **P1-73 — CMW500 单载波 LTE 2×2 MIMO OTA（已立项，设计中）**
>
...
> 拓扑层使用逻辑端口 `baseStation.DL1` / `baseStation.DL2`；UXM/CMW500 的具体物理连接器
> 必须由显式 LabProfile/拓扑绑定给出并经仪表回读核对，禁止把 UXM 的 `RF1`/`RF2` 名字
> 直接套到 CMW500。
>
...
> 文件名推断。
>
> **设计阶段必须补齐的现场事实**：① 实机固件与 KS520/相关 LTE 选件；② CMW500 的实际
> RX/TX1/TX2 连接器元组；③ UE 上行/attach 回传是否复用双向口或使用独立 link antenna，及其
> 拓扑身份；④ UXM/CMW500 共存时上游源选择开关的可读回位置；⑤ 两种信号源进入同一 F64
> 输入后的功率余量与路径补偿边界。任一项不得靠默认值或历史 UXM 配置猜测。
>
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **P1-73 CMW500 单载波 LTE 2×2 MIMO OTA 已立项、设计中**（架构方案 #1 已获用户批准；完整范围与硬件待核项见顶部 Current Focus）。2026-08-24 批准队列六片均已完成。P2-32 位于功能启用池，P3-20/P3-21 位于非阻塞维护池，均不得自动启动。现场静区线性 XY 扫描平台及物理单位/方向/偏置/型号实证仍保持 Hardware Blocked。 |
