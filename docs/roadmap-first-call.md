...
报告、小区关闭与租约释放均有同次证据；正式判词仍 `UNKNOWN`（无路损校准、无现场认证，另有 CMW500 两处纯软件缺口）。
...
P0-9 关闭后开放的 P0 只剩 P0-5（UXM 5G NR，等现场），按治理规则 2 焦点降到非现场队列：用户 2026-09-17 批准先做「现场改动落地」（已由 PR #480 完成），2026-09-18 批准其后按队列继续；该队列连同用户 2026-09-22 独立批准的 P2-78 均已收口。**P1-79A / B / C 已由 PR #491 / #493 / #494 合并；P1-79D 软件实现与本地验证已完成，交付状态以本片 PR 为准，合并后本地自动可执行队列为空。用户随后批准 P1-79E 取证片，调查结论为当前取得的 Ensemble 3.04 资料不足以覆盖型号/固件尚未确认的现场控制器，保持 fail-closed 并转 Hardware/Vendor-Protocol Blocked**（P2-73 / P2-75 / P2-76 / P2-77 / P2-78 已由 PR #485 / #486 / #487 / #488 / #489 合并；P2-74 本地半已由 PR #482 合并；P1-80 软件半已完成，P1-2 仍待同机现场复验；见下方 Current Focus（非现场）行）。
...
execution freeze、动作前同源复核和逐方位双坐标 evidence。本地结果不能替代同一 TestCase
方位与独立 HOME 最终 PFBK 的现场复验，故 **P0-5 正式自动化验收仍未关闭**。P0-8
...
不可查看的恢复报告。最终验证为相关链与完整规则门 **204 passed**、全后端
**4340 passed / 5 skipped**、GUI 契约 **3 passed** 与 production build；compileall、单一
Alembic head、diff-check 均通过。Codex R2 覆盖最终 HEAD 且无 P1，merge commit `75089a9`。
...
~~P2-57~~（✅ 已交付半 #448，声明残项见条目）→ ~~P2-58~~（✅ ① #450 / ✅ ② #452）→ ~~P2-59~~（✅ ① #454 / ✅ ② #456）→ ~~P2-60~~（✅ #457）→ ~~P2-61~~（✅ #458）→ ~~P2-62~~（✅ #459）→ P2-63（HOLD）**。
...
在真实型号与手册确定前保持 HOLD，位于队列末尾且不阻断任何已批准片。P2-64～P2-67 已由 PR #433/#434/#435/#436 全部合并，四片整组收口完毕；P2-54 已由 PR #437 合并；P2-55 的非现场半已由 PR #440 合并（其真机矩阵抽样复验待现场），P2-56 的 ① ② 两半已由 PR #444 / #446 合并（③ 现场半待真机），P2-57 已交付半由 PR #448 合并；P2-58 ① #450 / ② #452、P2-59 ① #454 / ② #456、P2-60 #457、P2-61 #458、P2-62 #459 均已合并。
...
1. **现场队列（只有真仪器 / DUT / SIM / 暗室在场时执行）**：**2026-09-17 更新** —— P0-9 已关闭；CMW500 在场时剩
   P1-74 / P1-4 现场半、转台复验（NEW-4 / P1-56，承接原 P0-9B-3 的方位 / HOME 内容）与已登记诊断（P1-2 修后复验、NEW-1、NEW-2、P2-71 设备半、P2-61/62）；校准未正式启动，不排校准。以下为此前表述：当前 CMW500 现场从 P0-9A
...
2. **非现场队列（本地开发 / 手册取证 / 自动化验证）**：先消化已经取得的现场证据；
   **P2-9 的精确 `ERROR 3` 分类与 NEW-1 的活动 F64 输出集合本地半均已完成**；
   ...
   能力矩阵，真机抽样待现场）。**P2-56 的 ① ② 两半已由 PR #444 / #446 合并**（① 建 LTE TDD 四维能力矩阵并**有意零新增可达状态**；② 才放开取值域并补上下发路径 —— ② 的全部意义就是新增可达状态，两半的性质相反，别把 ① 的那句话读到 ② 头上）。P2-59 ① #454 / ② #456、P2-60 #457、P2-61 #458、P2-62 #459 已合并。**原批准非现场队列已收口**，P2-56 仅剩 ③ 现场半待真机；后续待启动项以顶部为准。
...
>
> 已合并设计：`docs/plans/2026-08-26-p1-73-cmw500-lte-mimo-ota-design.md`；逐任务 TDD 实施计划：`docs/plans/2026-08-26-p1-73-cmw500-lte-mimo-ota-plan.md`。P1-73A 已建立通用 BaseStation contract、严格新旧配置 canonicalizer、LTE 单载波工作点、通用执行证据与逻辑 `DL1…DLN/UL1` 拓扑；CMW 正式能力仍保持关闭。fresh 内审发现并按 TDD 收口两条功能 P1：编辑 PCell 时保留全部 SCell，定制 MIMO mode 同样按所选 BaseStation 逻辑端口过滤拓扑。R1 三条功能 P1 已最小收口：RAT 不匹配在驱动写入前拒绝；LTE 缺显式峰值时绝对吞吐保留但比例与正式判决为 UNKNOWN/N/A；旧 UXM 证据迁移不再丢弃同行 F64/转台证据。R2 两条功能 P1 已继续最小收口：显式为 `null` 的 legacy NR SCS 不再静默采用默认值，已获准的 8 层配置完整映射为 `DL1…DL8/UL1`；R2 关于自定义拓扑节点识别的 P2 仅报告、不阻塞且不自动积压。R3 三条功能 P1 已按最小 TDD 收口：CMW 将通用 LTE band `B3` 正确翻译为 `OB3`；CMW 声明的 20 MHz / 4 层上限在驱动 I/O 前生效；可解析的 LTE 软件标准文件名必须与资产 `lte_dl_earfcn` 身份一致。R4 三条功能 P1 已按最小 TDD 收口：RRC 重配置改为驱动显式 opt-in，CMW 不再误调用继承的未实现方法；已列入频率一致性网的 BaseStation 缺身份时保持 unverified，不发布 `fully_verified=true`；commissioning 显式 NR SCS 只接受 15/30/60/120 kHz，显式 `0` 不再被静默改成 30。R5 三条功能 P1 已继续按最小 TDD 收口：真实 CMW 未开放 MAC 调度配置能力时仍可采集诊断值，但正式 KPI fail-closed，UXM 与 Mock 原路径不变；CMW 带宽严格按手册 `B014/B030/B050/B100/B150/B200` 精确映射且不再发送不存在的独立 UL 带宽写入，3 层等有损映射在 I/O 前拒绝；B42/B250 等选件门控频段在选件快照未知时不再放行。R6 补齐可解析 LTE 软件标准文件名的带宽一致性核对；两条 P2 仅报告、不阻塞、不自动积压。R7 按 CMW500 Base Software 手册的专用只读命令，在连接阶段冻结有效软件选件与可工作的硬件选件；任一查询失败仍保持未知，已安装 KB036/KS525 的 B42/B250 不再恒拒。最新全后端 **4766 passed / 5 skipped**，本轮受影响链与规则门 **186 passed**，GUI 相关契约 **16 passed**、production build（本轮无 GUI 改动）、compileall、单一 Alembic head 与 diff-check 通过；fresh 尾审 **P1/P2/P3=0**。
>
> P1-73A 已由 PR #400 以 merge commit `617b54dc` 合入。P1-73B Task 6–11 已按严格 TDD 完成：CMW500 连接只读取得型号/固件/有效软件选件/可工作硬件选件；execution 冻结严格七字段内部 2×2 route；route 写后逐字段回读；LTE 状态机、错误队列、超时/取消与安全 cleanup fail-closed；Extended BLER 的吞吐与 BLER 使用同一独立窗口且按手册字段分别解析；唯一持久化 rollout approval 默认关闭，`inherit` 永远只作诊断。正式 OTA 消费与报告白名单仍留在 P1-73C，本片没有提前打开正式能力。fresh 内审发现并按 TDD 收口 1 条功能 P1：专用授权接口原先只 flush 未 commit，HTTP 返回成功后可能回滚；现已要求服务器事务提交后才返回。Codex R1 又指出并已按 RED→GREEN 最小收口三条功能 P1：冻结 transport identity 改读锁内 `InstrumentConnection` 真值并拒绝旧 loaded driver 自证；七字段 route token 统一收窄为手册裸字母数字枚举，SCPI 分隔符不可进入 builder 或持久化 profile；`CONNect` 成功受理后的 `CONNecting`/`SIGNaling` 只轮询、不重复写动作，直至精确成功、拒绝或超时。R2 三条功能 P1 也已按 RED→GREEN 最小收口：断开前读取实时 Cell 状态而不信任旧缓存；Extended BLER 在 INIT 前显式写入并确认无限 timeout、continuous repetition 与无提前停止条件，意外提前结束不得发布正式 KPI；Cell ON 后的 `PENDing` 仅作有限轮询且不重复发送 ON。R2 修复验证：受影响链与规则门 **261 passed**，全后端 **4872 passed / 5 skipped**，compileall、单一 Alembic head `d73b5f6a1c20`、diff-check 均通过；fresh 尾审 **P1/P2/P3=0**。
>
> P1-73B 已由 PR #401 以 merge commit `58d64042` 合入。P1-73C Task 12–17 已完成：CMW500 进入与 UXM 共用的 MIMO OTA 方位扫描；版本化执行证据把冻结配置/内部 route、每次 attempt、lease/session、逐方位 Extended BLER 窗口、cleanup 与 transport release 绑定到逐指标信任入口；Analysis、报告、详情/下载、对比、历史、commissioning 与 GUI 统一消费该入口，吞吐与 BLER 独立降级，模拟/未知/旧 attempt/未完成 cleanup 或 release 的值保持 UNKNOWN/N/A。OperationalLab readiness 只读所选 LabProfile 的同一 InstrumentConnection，正式能力仍由专用 endpoint 显式启用且默认关闭，现场未确认项仅 Warning。Task17 fresh 内审发现 mock 基站缺少 idle release，导致 HAL reload 失败，已最小补齐；全后端 **4964 passed / 5 skipped**，focused **499 passed**，GUI 契约 **14 passed**、production build、compileall、单一 Alembic head `d73b5f6a1c20` 与 diff-check 通过；fresh 功能内审 **P1/P2/P3=0**。
> P1-73C 后经 Codex 外审逐轮收口最新 HEAD 的功能 P1，由 PR #402 以 merge commit
> `06b31b95` 合入。下列“可观察缺口 / 用户批准方案 / 交付流程”是 P1-73 实施前的设计记录，
> 只叙述当时的施工依据，不再表示当前代码缺口；8 月 27 日现场现状见本页顶部 Current Focus
> 与独立现场总结。
>
...
> 不可接受结果：2×2 下行链未真正建立，或把状态/错误字段误当成 KPI。
>
> **用户批准的架构方案 #1**：LabProfile 中 `baseStation` 是互斥逻辑角色，现场选择 UXM
...
> 准入只绑定型号、固件版本、DL MIMO 2×2 的 KS520，以及按本次 duplex 精确选择的
> KS500(FDD) 或 KS550(TDD)；外部
> RF router/开关状态不进入准入键，也不扩成逐次执行硬门。CMW 内部 `1CC - nx2` route 仍由
> 驱动按手册配置和回读，因为它是仪表生成双路下行的必要内部状态。
>
> **CMW500 内部路由门**：外部共用 F64 链不等于可以忽略仪表内部路由。LTE UE 手册
> §2.2.3.2 说明 `1CC - nx2` 为一条 UL、两条 DL，且两条 DL 必须使用不同 TX 模块与 RF
> 连接器；§2.6.8.1 给出 `ROUTe:LTE:SIGN<i>:SCENario:TRO:FLEXible`，分别配置
> RX、TX1、TX2，并标明最低软件版本 V3.5.40 与选件 CMW-KS520。P1-73 必须在任何正式采样前
> 核验固件、选件、场景与完整 RX/TX1/TX2 连接器元组；缺失、冲突或无法回读时 fail-closed。
>
> **首片范围**：只做 LTE 单载波 2×2、下行 transport-block 吞吐与 DL BLER；吞吐/BLER
> 只消费 Extended BLER 的手册定义字段及明确真实 provenance。排除 CA、4×4、CMW-B450x
> DAU/IP 层吞吐，也不假设 CMW 数字 IQ 外部衰落链——本架构由下游 F64 完成射频域信道仿真。
> RSRP/SINR 没有独立且可核对的厂商出处时继续 UNKNOWN/N/A，不从别的仪表方言、默认值或
> 文件名推断。
>
> **现场确认口径**：当前正式能力准入只要求实机型号、固件、KS520 与本次请求 duplex 对应的
> KS500(FDD)/KS550(TDD)，adapter
> 可注册但正式能力默认关闭，由用户显式启用。CMW 内部 route、UE attach 与 Extended BLER
> 状态仍按当前执行证据核验；外部 RF router/开关缺回读与现场接线待确认只显示 Warning，不用
> `Hardware Blocked` 阻止开发。端到端功率预算、开关/线缆损耗、F64 输入余量与完整路径补偿
> 延后到正式发布阶段，不作为 P1-73A/B/C 的开发阻塞项。
>
> **交付流程**：NotebookLM 仅作手册检索入口，所有仪器命令、参数、返回字段与前置条件仍须
> 落到仓库本地原始手册的章节/页码/原文；完成全集枚举与设计评审后，按 TDD → 相关/全量回归
> → GUI build / compileall / 单一 Alembic head / diff-check → fresh 内审 P1=0 → Codex 外审
> → merge 的标准流程实施。
...
`unavailable` / `diagnostic_proxy`，固定 0.7 已删除，ProbePattern 只作为黄色诊断代理展示；
PRECHECK 运行门仍能指出仪表/校准/DUT 的真实失败，但静区及最终 verdict 保持 UNKNOWN/N/A。
...
>    行 0% 带 `instrument_id`（当前代码复核同样 0%）。根因：`logging_config.current_instrument_id` contextvar **全仓零写方**，只有
>    `app.hal.base` 的 SCPI 包装层自己塞 `extra`。修法形状 = 给 contextvar 加写方（驱动操作入口 set / finally reset，一处），不改消息文本；
>    顺带把 Discovered 08-03「`/system-logs/tail|search` 无 `instrument_id` 过滤参数」一并收口。
...
> 9. ~~**[P1 候选，出发前硬门槛] 补齐无载体 blocker 的诊断序列**~~ ✅ **P1-65 / PR #380**（8 条，比候选多了 license 真值、fresh-start 真值、SIM 身份、连接空置四条）。
...
| **P1-70** | 信道验证第一激活批（temporal + doppler）：真 SA 驱动实现 `measure_pdp` / `measure_doppler_spectrum`（**必查 SA 手册 SCPI**。⚠ 实施对象纠偏：设计稿原写「X 系列」，但现场 SA 实测系 R&S FSVA3000（playbook 2026-05-27:68，P0-4 绑 `RealRsFsvaDriver`）且本地 X 系列两份手册均系假文件——真驱动落 `rs_fsva.py`（依据 R&S FSVA/FSV 手册 1176.7510.02─13 原文），X 系列保持 NotImplementedError 指路）；`channel_calibration` 两端点暴露 use_mock（P1-68 模式）；Orchestrator ×2（含 `execute_calibration_plan` 主链）同批改 request 传入；phase 只读 ×3 去死参；诊断序列补一条 SA 测量能力探针（出发前载体，禁盲试适用） | 真 SA 路径有行为门 + 变异（mock HAL 下走 fail-loud/能力探测语义）；CE+SA 闭环端到端可从 API 触达；现场补 FSV/A3000 I/Q Analyzer 手册 1178.8536.02─16，并用 `rs_fsva_iq_capability` 验证真实 FSVA3044 | ✅ #392；2026-08-27 现场前置两项已收口：探针在初始 OFF 时受控切到 ON，实测 `SRATe=32000000`、`BWIDth=25600000`、`RLENgth=1001`，最终确认恢复 OFF 且错误队列零残留（DiagnosticRun `df640983-f2e8-4d2c-ab64-de99fa38814f`、复跑 `4241e370-560f-459a-b77a-0754ba7bf7f8`） |
...
| **P2-32** | 校准真实启用拆片 | **A 多频路径损耗**：✅ 完成 CE+SA real 入口、显式 Real/Mock、逐探头 warning 持久化与报告投影，并移除随机旧入口；真实结果冻结 LabProfile/拓扑/RF chain/CE port，路由漂移后 fail-closed。**B 方向图**：✅ 软件片完成唯一 production caller、真实/Mock 显式授权、逐探头冻结路由与 warning、正式消费/报告 fail-closed；本地软件验证不替代 F64+转台+SA 真机验收。**C 静区**：仍需厘米级 XY 扫描平台，Hardware Blocked |
...
但各自验收条件独立核对。P2-55 扩域抽样与 P2-56 TDD 在基线可复现后进行；P1-4 以第二次执行对比收口。
UXM 与 CE 认证按相应设备/手册和前置证据就绪情况另排。以下状态是“未取得可核验的关闭证据”，
不是本次已经重新连接硬件实测。P2-55 的软件载体缺口已移交 P2-70，不再只写成等设备到场。

**2026-09-17 更新**：P0-9、P0-8、P2-51 / P2-55 / P2-56 现场半已关闭（见各行）。**校准尚未正式启动**（用户 2026-09-17 定）：
凡以「缺路损校准」为由的关闭条件，在校准正式启动前不阻塞当前项；校准启动时另立项，届时才需要信号分析仪到场并连通。
这只影响 roadmap 条目的关闭判定，**不改变执行门**：无校准时正式 KPI 照旧保持 `UNKNOWN` / N/A，也不得据此强升 Formal。

**2026-09-18 更新**：2026-09-16 现场代码已由 PR #480 过审合入。过审改了几处从未在真机上跑过的路径（CMW 路由写前的错误队列读弃与未确认回执、F64 stop 回执的证据键）——下次 CMW500 + F64 在场时，先用合并后的版本复跑一次 `dbd53e6f` 同款用例并补记；转台多方位用例：P2-74 本地半已由 PR #482 合并（merge `4d6050fb`，2026-09-19），**可以带去复验** —— 分段移动只在假控制器上验过，历史日志显示真机上此前从未成功完成过超过约 2 s 的移动；次序按下文 `### P2-74` 条目的「现场半」：先在原超时下复跑，再读出并记录 `Socket2Timeout` 实际值，最后才调大。
...
| **P2-51（现场半）** | **CMW MAC requested/applied 真机闭环** | ✅ **2026-09-16 现场完成**：`dbd53e6f` 同一 execution/attempt 内 MAC 回执 confirmed、单发窗口生命周期 confirmed（统计基 requested 5000 / applied 5000）、SAFE_IDLE（小区回读 `OFF,ADJ`）、transport release 与报告齐全。以下为此前表述： #420/#437 的本地实现不能代替真实下发、回读、窗口与安全释放证明；与 P0-9/P1-74 共用现场窗口，分别签收 | ✅ 当前 TM3/2 层 LTE MIMO_OTA TestCase；保存同 execution/attempt 的 MAC receipt、窗口、SAFE_IDLE/release 与报告。不是重新开发 #420 |
| **P2-52 / P2-54 UXM 证据边界** | **UXM 原生有限窗口与 IRAT MAC 错误队列权威来源** | **2026-09-09 原始手册复核纠偏**：`Examples > Measuring BLER` 给出 `CONTinuous:ALL 0`（Single）+ `LENGth:ALL 5000` + `STATe 1`，并规定 `progress-count` 到配置长度即结束，但 Application Mode 只列 NSA/SA，不能授权现场 `LTE_NR_IRAT` 写入。当前正式实现仍未冻结/下发/核验原生窗口，`uxm.error_queue` 也仍 unverified。必须先取得当前 Test Application/固件/选件下的 IRAT 厂商依据，以及可权威确认 cleanup/reset 的读取或操作员确认机制；否则维持 `clear_read_only` / unknown | ⚠️ 原拟真实写诊断经安全复核撤回：给结果贴 `unverified` 不能降低未取证全局写的硬件风险；仅记录“命令已发送/需复位”也不能阻止 lease 释放后的正式执行重入。现只保留零写 `uxm_window_boundary_probe` 辅助。依据齐全后另开实现片，先设计监控独占、持久 quarantine 与权威 reset 解除门，再实现两窗口诊断；现场证据未取得前本项保持 Blocked |
...
| **P1-74（现场半）** | **CMW500 Extended BLER 统计基不继承旧 session 的真机证明** | **2026-09-16：未关闭** —— 只取得一个 `SFRames 5000` 样本；且现场已把窗口改为 Single-Shot（统计长度 = 窗口本身，每次显式写 + 回读 5000），仍需两个不同 `stat_count` 的连续执行。 非现场半已由 PR #429 合并（下发 `EBLer:SFRames` + 回读 + 全域 fail-closed）。现场缺的是真机行为证明：在真实 CMW500 上用**至少两个不同统计长度**连续执行，保存原始写/读/错误队列与报告证据，证明统计基随本次 TestCase 变化、**不继承上一 session**。⚠️ 手册（Base SW UM p.139）说运行中改结果相关参数会重启测量并清零统计计数器 —— 该行为本地无法证伪 | ✅ 正式载体：当前 LTE UMa 20 MHz MIMO_OTA TestCase，改 `stat_count` 跑两次即可对照（窗口证据里 `trust.reason` 带 applied 值）；诊断辅助：CMW500 已登记序列 |
| **P2-56（现场半）** | **LTE TDD 正式路径的真机认证** | ✅ **2026-09-17 用户裁决关闭**：TDD 配置层已被真机全部接受（下述）；B41 下 attach 4/4 超时的原因经用户确认是**被测手机不支持 B41 频段**，不是系统缺陷。TDD 下的真实 Attach / 窗口尚无真机样本 —— 按「正式执行自带同次证据」口径，首个用支持 TDD 频段的 DUT 跑的正式 TDD 执行即是它的验证，不再单独排现场项。以下为 09-16 事实与此前表述：B41 TDD 下真机接受 `DMODe TDD` / `ULDL 1` / `SSUBframe 1`（本机选件含 KS510 / KS512 / KS550），`DLEQual ON` 先于 RMC 时 `RMC:DL2?` 与 `VERSion:DL2?` 回读跟随 DL1，未见顺序相关拒绝 —— 下列三个未知答了两个半；**但 UE attach 4/4 超时**（PS 恒 `ON`，一次瞬时 `SIGN`），同日 B2 FDD 3 s / 30 s 挂上（见[现场总结](site-debug/2026-09-16-cmw500-f64-onsite-summary.md) D7，原因已由用户确认）。 ① ② 两半已合（#444 / #446）：矩阵声明 + 下发回读均已就位，且已用 TDD profile 在 fake transport 上闭环。现场缺的是：真实 CMW500 上完成 Attach / 业务窗口 / SAFE_IDLE / release，并留下正式证据。⚠️ 本地证不了三件事：① 仪器是否真接受 `CELL:PCC:ULDL`（选件需 **KS550 and KS510**，pp.687-688）与 `SSUBframe`（只需 **KS550**，p.688；KS512 另限于 value 7+扩展CP / value 9）这两条写 —— **两条的选件要求不同**，别按同一套核；② `DLEQual ON` 对 `RMC:VERSion:DL2` 是「开的那一刻快照」还是「开着时持续传播」—— 本驱动把 DLEQual 排在 RMC 之前，而手册示例 §2.5.20（p.342）排在之后；③ TDD 帧结构两组相对 RMC 行的下发顺序（手册未给出顺序错了会怎样） | ✅ 正式载体：建一个 `duplex=tdd` 的 LTE MIMO_OTA TestCase（`uldl_configuration` / `special_subframe` 必填）跑一次，抄回 execution_id + `formal=` 那行 + 报告 |
| **P2-55（现场半）** | **能力矩阵里 `authoritative` 组合的真机抽样验证** | ✅ **2026-09-17 按用户现场结论改验收并收口**：开测前抽样对「这次测试」不必要 —— `cmw500_fdd_matrix_probe` 的产出只进 `DiagnosticRun`、正式链零读取；正式执行自带同次的写→错误队列→回读证据；TM1+1TX 在正式 schema 不可达；探针要求的 FDD 前置只有正式执行才会建立（当日 `a0395c8a` 因仪器在 TDD 被预检拒）。**TM3+2TX 格由 `dbd53e6f` 的同次 config / MAC 回执签收；TM1+1TX 格降为「正式 schema 开放 TM1 时的扩域前置」，不再占现场窗口。** 以下为此前表述： 非现场半已由 PR #440 合并（manifest 逐维度取值域 + 判定器逐维度对账；判据见驱动内矩阵声明上方注释）。现场缺的是：矩阵声明为 `authoritative` 的组合在真机上**确实可配置且回读一致**。抽样至少覆盖 TM1+1 天线（走表 2-37）与 TM3+2 天线（走表 2-38）两条路径，各自保存写/回读/错误队列证据。⚠️ 本地只能证明「声明与手册一致」，证不了「仪器真的接受」——尤其选件缺失时的实际行为（矩阵记的是手册 Options，不是整机 `*OPT?` 实况） | **正式 TestCase 仍只接受 TM3/2 层**。P2-70 新增 `cmw500_fdd_matrix_probe`（本地实现，本地验证完成，交付状态见 PR）：FDD/20 MHz/1CC-nx2 固定 `tm1_one` 与 `tm3_two`；关闭小区，仅做配置/回读/错误队列抽样，不 Attach、不测吞吐、不授予正式资格。TM1/ONE 在 nx2 下是 SIMO 1x2，不称 SISO。（以下为 2026-09-17 之前的表述，现已不再排现场：）现场各跑一次并保存完整诊断证据；跨 TM 的中间设置若被拒必须原样留证，不据手册猜自动耦合。正式两维扩域仍须后续设计，不能只改 Literal。 |
| **P0-9** | **CMW500 LTE 2×2 MIMO OTA 真实执行与正式证据闭环** | ✅ **2026-09-17 用户裁决整项关闭**：A / B-1 由 09-16 现场证据完成；B-2（路损校准）与 C（正式验收）以校准为前提 —— 校准尚未正式启动，不再作为本项关闭条件，校准启动时另立项；B-3（转台方位 / HOME 复验）的内容本就同时挂在 P0-5 行与 NEW-4 / P1-56 行，随那两行走。以下为关闭前的表述： **2026-09-16 现场：P0-9A ✅（`a4168179` / `dbd53e6f` 真实 Attach + 全流程 + 报告）；P0-9B-1 ✅（专用七字段 route query 真机回读逐字段一致）。** 仍缺：P0-9B-2 真实路损校准（SA 当日不可达）、P0-9B-3 多方位 / HOME 复验（当日 `-90°` 移动结局未知并急停）、现场认证，以及 CMW500 两处纯软件缺口（配置回执三字段恒 unknown、无频率身份回读 → 正式判词恒 `unknown`），见[现场总结](site-debug/2026-09-16-cmw500-f64-onsite-summary.md) §4。以下为此前表述： 2026-08-27 已证明 CMW500/F64/转台链到 Attach 前；P0-9B-3 已完成 `+90°` 偏置的 execution freeze 与证据消费。本地仍缺真实 DUT/SIM Attach、`PCCBBBoard` 专用有据 query 的真机复验、真实路径损耗校准、同一 TestCase 转台方位/HOME 复验，以及同一 attempt 的真实报告/cleanup/release 证据 | ✅ 正式载体：当前 LTE UMa 20 MHz MIMO_OTA TestCase；诊断辅助：CMW500/F64/Aerotech 已登记序列。按 P0-9A/B/C 顺序关闭；任一正式前置未知时允许诊断运行但 KPI 保持 UNKNOWN/N/A |
...
| **P2-71（设备半）** | **F64 `.smu` 常态发布、设备侧版本/字节验约与回滚** | 2024 通用 User Reference §20.4.13–14 列有 `MMEM` 文件命令，但 CAICT F8800A 历史实测 `MMEM:CDIR?` / `CAT?` 为 `-100`，FTP(21) 关闭；当前冻结/加载证明不了设备字节。原因未定，不能按通用手册上线自动发布；详见[调研结论](plans/2026-09-13-p2-71-f64-file-provisioning-research.md) | ⚠️ **能力分类半载体**：[`propsim_f64_health`](../api-service/app/diagnostics/sequences/propsim_f64_health.py) 的 `MMEM_CDIR/CAT` 可在独占维护窗口复核支持分类；现会把目标 query raw/异常与错误队列 raw 一并保存在 `extra.scpi_observations`，但序列仍发送 `*CLS`、消耗错误队列。运行前必须保全已有错误证据，执行中禁用。**文件取回/写入/替换、设备字节摘要、版本验约和回滚仍无受控载体**，不得在临时脚本或正式 TestCase 试探；本地/文档调研不关闭本行 |
| **P2-77（现场半）** | **F64 vendor asset 有界调频与全组回读证明** | 软件依 PROPSIM User Reference Rev 10.2 §20.4.6.1 / §20.4.6.2（pp.280–281）及 §20.4.6.8（p.282）实现「全组限值预检 → 写入/错误门 → 全组精确回读」，但 fake transport 不能证明真实 F8800A 的范围形态、自动钳位和逐组回读行为 | ⚠️ **正式 TestCase 现场验收**：在同一真实 F8800A、同一 vendor `.smu` 工程上保存两个合法中心频点的逐组 limits / requested / applied / 错误队列原始证据，再验一个越界频点在首个写入前拒绝；工程若返回多区间，再保存一个多区间样本。本地测试、Mock 或手工改资产登记都不关闭本行 |
...
| **NEW-2** 现场半 | **关掉 ATE socket 后 F64 前面板 Local 是否真的可用** | 2026-08-27 仍只有人工面板理解，数据库中没有可核对的 SUCCESS；代码只可确认 `ate_socket_released`，不得替仪器宣称前面板已 Local | ✅ 载体：[`propsim_f64_local_handback_check`](../api-service/app/diagnostics/sequences/propsim_f64_local_handback_check.py) 两段式人工确认；release 后先观察面板，confirm 时显式选择 `local` / `remote` 并填写原文。留空保持 UNDETERMINED，不再由 GUI 默认为否；人工观察仍不授予正式资格 |
...
| **NEW-4 / P1-56 现场半** | **转台收到 `MOVEABS X 0` 为什么不动** | **2026-09-16 新事件**：`MOVEABS X -90` 途中连接被 reset → 非幂等命令结局未知 → ABORT 读 `VFBK(X)` 得空串 → 急停（零速度确认），见[现场总结](site-debug/2026-09-16-cmw500-f64-onsite-summary.md) D8；运动真值序列当日未跑。 on-site real Aerotech。⚠ 2026-08-07 实测：14:23–17:44 共 **6 个 execution / 15 次** `MOVEABS X 0.0000`，编码器 `PFBK(X)` 一个计数都没动，而 `move_to` 仍返回成功并打印「Arrived: Az=90.00°」。**最危险的一条**：功率问题一旦修好走进方位循环，四个方位测的是同一个物理位置，产出一份看不出破绽的假数据，现有的门一个都不会红。⚠ 仓内厂商文档对象是 **Ensemble**，而驱动文件头写 **A3200**，型号不符待确认 | ✅ **本地载体已由 P1-56 / PR #350 收口**：[`aerotech_positioner_motion_truth`](../api-service/app/diagnostics/sequences/aerotech_positioner_motion_truth.py) 只在站点显式确认 degree user-units 与安全坐标范围后，使用仓内指南有出处的 `MOVEABS ... XF...` 做小步前进/返回，每 200ms 连采 `VFBK + PFBK` 最多 10s 并落 raw；ABORT 后只以全部实际轴有限 VFBK 精确为零保守确认停止。[补充 Codex 结论](https://github.com/swang430/Meta-3D/pull/350#issuecomment-5327846839)明确 reviewed commit `0f981339c5`（包含尾修 `bd71879`）且无重大问题。**2026-08-28 更新**：P0-9B-3 已把 degree、固定 +90° 偏置及来源/时间冻结进 execution，正式 writer 同源消费；同一 TestCase 方位与 HOME 终点仍须现场独立复验，故正式关闭继续 Blocked。 |
...
| NEW-2 面板 Local | 无 | ⚠️ 人工观察，载体已补（P1-65） | `propsim_f64_local_handback_check`（两段式证据记录） | 跑 `propsim_f64_local_handback_check`：release 段结束后先去面板看（§20.1：Remote mode 水印是否消失、Local Mode 按钮是否不再亮蓝），再以 confirm 段填写观察原文并显式选择 `local` / `remote`；未选择保持 UNDETERMINED |

**汇总**（数字由上表「覆盖形态」列统计，两个口径别混）：新增 P0-9 后，Blocked 表开放
**17 行**（不含 3 行划线）；P0-8 按 a/b 两个 gate 拆开后矩阵 **18 行**。
⚠️ **这两个数已失准，别拿它当现势**：本矩阵定于 2026-08-23，此后新增的现场半行（**P1-74** / **P2-55** / **P2-56**）都没有进这张覆盖矩阵，其中 P2-56 那行是本次（P2-56 ② 收口）加的。重算要逐行重新分类（上面 4 + 12 + 2 的拆分），属独立一件事 ——在那之前，**以「Blocked on hardware」表本身为准，不以这里的计数为准**。
...
- ~~**候选 9 [P1 候选，出发前硬门槛]**~~ ✅ **P1-65 / PR #380** 补齐无载体 blocker 的诊断序列：`emcenter_switch_health`（P2-9，只读握手 + 槽位/状态回读，
...
These are still the highest-priority items overall. **2026-09-17 更新**：P0-9 已由用户裁决关闭（A / B-1 现场完成；校准相关的 B-2 / C 待校准正式启动时另立项）。以下为此前表述：当前先按 P0-9A 用 CMW500 完成真实 Attach
...
> 各 P0 的 acceptance（**2026-09-17 更新**：校准未正式启动期间，协议里的 SA 与校准两个 Phase 不排，信号分析仪不需要到场）；并固化 CAICT 教训: 出发前硬门槛 + 铁律「现场不写 driver
...
### P0-8 — F64 driver 现场修复落地 (port 3334 + 输入信号参考/crest + 加载 gate + 默认 .smu) ✅ Done（a 半 2026-08-27 / b 半 2026-09-16 现场）
...
### P0-9 — CMW500 LTE 2×2 MIMO OTA 真实执行与正式证据闭环 ✅ Closed（2026-09-17 用户裁决；A / B-1 现场完成）

**可观察故障**：P1-73 与现场修复已让 CMW500/F64/转台链运行到 UE Attach 前，但当前仍
无法合法发布正式 LTE 吞吐/BLER：UE 尚未 Attach，CMW500 专用七字段 Route query 尚未经
真机复验，现场无有效路径损耗校准；Aerotech `+90°` 坐标合同虽已完成 execution freeze 与
evidence 消费，但尚未用同一 TestCase 现场复验方位和独立 HOME PFBK；也没有一份真实 completed
execution 的报告证明 cleanup 与 transport release 属于同一 attempt/lease/session。任何一项缺失时继续保持诊断执行和 UNKNOWN/N/A，不能用
人工面板观察或旧标志补绿。
...
**2026-09-16 现场结果**：P0-9A ✅ —— `a4168179`（14:48，UE 3 s 挂上）与 `dbd53e6f`（15:35，UE 30 s 挂上）两次 completed，
后者读到真实单发窗口 45.82 Mbps / ACK 100 % / BLER 0 %，并生成报告。P0-9B-1 ✅ —— 专用 `…SCENario:TRO:FLEXible?`
...
正式判词会停在 `frequency_identity_not_fully_verified`；已按根因拆成 P1-79A～E。A/B 已由 PR #491/#493 合并；C 已完成共同强制证据投影的软件实现与本地验证、正在走本片 PR 外审；D 待 C 合并后执行；E 已完成取证并转 Hardware/Vendor-Protocol Blocked（伞形项 P1-79 撤销，见下方 2026-09-17 triage checkpoint）。这些缺口都在与校准无关的资格、强制证据两层；报告的 KPI 结论另有 ANALYSIS 层的路损校准条件，见现场总结 §5。详见[现场总结](site-debug/2026-09-16-cmw500-f64-onsite-summary.md)。
...
unknown；不得以写入成功、空错误队列或通用 query 的保留 Controller 字段代替。（该专用 query 已于 2026-09-16 真机复验通过，见上方现场结果与 U-10。）
...
**Status**: 🚧 现场程序化证据未关闭。2026-08-27 在清理历史 `-100` 后，操作员曾按
`propsim_f64_license_truth` 的手册只读路径判通过；2026-09-16 的连续两次运行随后证明旧序列会把
带错误队列条目的空回复显示为绿色。P1-80 已完成软件修复：开场 residue 单独归档、诊断期间关闭后台
仪表监控且整段持有 F64 SCPI 事务锁、错误逐查询归属、空回复与畸形返回按命令值域 fail-closed，并分列
license/calibration/user-alignment。剩余动作是在同一 F8800A
的干净错误队列上复跑留证；不恢复旧探针，也不把本地回归替代现场结果。
...
> 本条剩余现场半 = 用 P1-80 修复后的 `propsim_f64_license_truth` 对账实测。
...
### P1-74 — CMW500 Extended BLER 统计基真值闭环 ✅ 非现场半（#429）/ 🚧 现场半

**可观察故障**：P2-51 已证明 CMW500 LTE FDD 2×2 满 RB 固定 RMC 的配置与回读，但正式
Extended BLER 窗口没有从 TestCase 的 `stat_count` 驱动并回读 `EBLer:SFRames`。在 continuous
窗口下，仪表可能继承上一 session 的统计子帧数；业务参数看似相同，吞吐/BLER 的统计基却不同，
正式 KPI 因而不可重复。

**范围**：统计基归 Measurement Window 所有，不塞回 MAC 配置层。执行冻结明确的 LTE 统计请求，
在窗口清理/启动前按 R&S 手册有据的命令写入并回读 SFrames，把 requested/applied/confirmed、
exchange ids、attempt/lease/session 与窗口 digest 一并持久化；任何写入、回读、错误队列、超时、
取消或窗口形状异常均 fail-closed。不得从请求值、默认值、旧缓存或 Mock 回填确认。
...
execution `7ae66c69-edc5-422e-8681-d8ad56c23e64` 的冻结证据同时出现 `adapter=uxm`、UXM
manifest `rats=[nr5g]` 与 requested `radio_technology=lte`；现有冻结只校验 model/binding/driver/
endpoint，没有把 TestCase 结构化需求与 Adapter Manifest 对账。真实驱动虽有后置 RAT 拒绝，
MockBS 却无条件声明 NR5G+LTE，因而绕过错误组合。

**范围**：从当前已结构化的 `primary_carrier.radio_technology` 与执行所需 operation 构造不可变
requirements projection，在首次仪表 I/O 前与锁内冻结的 Adapter Manifest 对账；Real、Mock、
Diagnostic 使用同一门，不兼容即拒绝进入可运行 execution 且不生成报告。compatibility verdict、
requirements digest、manifest/binding digest 一并冻结，并在取得 lease 后、首次 I/O 前复核漂移。
判据不得读取 TestCase 名称、adapter 名称前缀或 Mock 自报的能力并集。P2-54 尚未落地前，MAC
profile kind/version 只保留显式 absent/unavailable 槽位，不由 P1-75 提前发明；P2-54 后再扩展同一
requirements schema，禁止另建第二套兼容性判据。

**验收与地点**：纯非现场片。至少覆盖 UXM+LTE、CMW500+NR、manifest 不含所需 operation、
binding/manifest 在冻结后漂移四类拒绝，以及 UXM+NR、CMW500+LTE 两类放行；拒绝必须发生在零
...
### P1-77 — LTE TDD TestCase 必须有可达的 MAC 帧结构配置入口（✅ PR #461）
...
**2026-09-05 实现记录**：GUI 已按 PCell 真值投影 LTE duplex，TDD 提供 ULDL `0..6`、special
subframe `0..7` 与仅 20 MHz 可见的 RMC version `0|1`；缺损旧记录保持空值并给出可操作错误。
服务器把 request-only authoring input 严格校验后生成既有 `lte_rmc@1` frozen profile 与 digest，
数据库只保存冻结真值。CMW500 命令字面量、顺序、回读、正式 provenance 和现场认证边界均未改变；
23 条历史记录没有被批量修改。OpenAPI 的 `TestCase.configuration` 仍是自由对象，本片没有新增公开
schema，故 OpenAPI/YAML/generated TS 四镜像不适用，不制造无消费方类型。

**本地验证与 P3-23 试行数据**：严格 RED→GREEN；受影响链 **282 passed**，全后端正确工作目录
复跑 **6409 passed / 5 skipped**，GUI 新契约 **12 passed**、production build、compileall、单一
...
该反例已按 RED→GREEN 收窄为检查字段是否出现，受影响链 406 passed；R1 重复请求 0 次。R2 对
...
### P1-76 — 实时监控不得展示固定/随机的假读数（✅ PR #469）
...
REST/WS 可发布固定吞吐/SNR/EIRP/温度。HAL 返回空或异常时，`monitoring._generate_fallback_data`
还生成随机值，响应没有 provenance。2026-09-05 在 `a0c671c4` 用两个不同的内存 driver 读数
10/90 调用生产聚合函数，两次都得到 `0/0/0/23`。没有连接硬件；未证明本路径进入正式报告。

**最小方向/验收**：先去掉默认/随机读数，再换源到权威品类及可信测量。不能仅改键拼写后
保留“功率默认加 3 dBi 就是 EIRP”等无依据推算。缺测/错误显示 unavailable，模拟必须显式标记且
...
**外审与合并**：Codex R1 对 `558b88ee` 于 2026-09-10 10:27:06Z 请求、10:33:22Z 回 clean（约 6 分
16 秒，无 inline）；同 HEAD 的独立 R2 于 10:33:43Z 请求、10:38:54Z 回 clean（约 5 分 11 秒，无
inline）。两轮各请求 1 次，重复请求 0 次；PR 随即于 10:39:24Z 以 merge commit `b4e840e0` 合入，无审后
尾提交。roadmap 状态由 2026-09-11 的独立纯文档收口 PR（分支 `p1-76-closeout`）回填。
...
**实施状态（2026-08-28，PR #408 已合并，merge commit `d2192c2b`）**：已提取唯一
`BaseStationExecutionSession`，formal runner、commissioning saved phase、adhoc 与 run-all
四类入口均复用同一生命周期：锁内冻结校验 → Remote acquire/transport identity 刷新 →
current attempt/lease/session 绑定 → operation → cleanup/release → terminal evidence。执行期间统一
关闭后台监控；Analysis/Report 延迟到真实 release 与终态证据落库之后。成功、业务拒绝、异常、
取消、acquire 失败、release 失败及终态持久化失败均按同一状态机 fail-closed；未修改厂商命令、
adapter SPI 或正式 provenance 白名单。设计与实施计划见
`docs/plans/2026-08-28-p2-42-base-station-execution-session-design.md` 和
`docs/plans/2026-08-28-p2-42-base-station-execution-session-plan.md`。
...
**实施状态（2026-08-29，✅ PR #409 已合并，merge `6493427f`）**：现有 vendor-neutral
...
fail-closed，设备拒绝仍立即中止。R1 发现的显式绑定 CMW Mock route 回执缺少冻结七字段 P1
也已收口：模拟 route 按完整字段记录 unknown，只允许诊断且不产生正式 applied route。扩大相关链
1067 passed、全后端 5130 passed / 5 skipped，
...
execution freeze 复用同一 projection/digest，sync 解析失败事务回滚且预览零仪器 I/O。注册目录公开
...
不会形成正式就绪假象。Codex R2 又发现 LabProfile 保存的 driver mode 可能与当前类别模式或实际加载
驱动分叉；现 resolver 在推导执行模式前精确复核三者，显式 `real`/`mock` 任一不一致均 fail-loud，
`auto` 只允许跟随实际驱动。设计与实施计划见
...
**实施状态（2026-08-29，已由 PR #412 合并）**：manifest v2 已把 RAT、
...
P2-46 已由 PR #412 以 merge commit `0330ad93` 合入。
...
Mock 全部 simulated/diagnostic。writer 冻结 registry snapshot/digest，投影、Analysis、报告、详情、
下载、比较、历史与 commissioning 共用 generic map；原两项只作兼容镜像。GUI 按 unit 显示全部
注册指标，ratio 不伪装成百分比，raw 不伪装成工程单位。未新增/猜测 SCPI，未改变正式 provenance
...
**真机复验为现场半**（取证清单 §7，本地测试不替代）—— ✅ 已于 2026-09-16 现场完成（执行 `dbd53e6f`，见 Blocked on hardware 表 P2-51 行）。
...
**✅ 已由 PR #424 合并（2026-08-30，merge `0efc2cff`）**：新增 14 个可参数化认证函数与测试域第三 adapter
...
### P2-54 — RAT-neutral MAC Test Profile ✅ Done（#437）

**可观察故障**：当前共同 MAC 请求由 NR 语义主导；LTE CMW500 只能把大量字段记为
`no_equivalent`，而 `mcs`、NR TDD pattern、HARQ processes、SCS、CSI-RS 与 LTE RMC/RB/TM
并不是同一组可互换参数。继续扩充一张“万能参数表”会让 adapter 接收无法如实表达的请求，或让
调用方把“操作成功”误读成“全部测试意图已生效”。

**范围/验收**：建立可辨识的 MAC Test Profile：共同层只保留 RAT-neutral 的测试意图、统计窗口与
指标需求；NR profile 与 LTE RMC profile 分别拥有自己的必填/可选字段、单位、枚举、手册来源和
profile digest。adapter manifest 声明接受的 profile kind/version；不支持或不可翻译的 profile 在首个
仪器 I/O 前拒绝。执行、commissioning、GUI、OpenAPI 与证据只按冻结 profile 消费，不新增厂商分支，
...
P2-55/P2-56 的能力矩阵与 LTE TDD/真机认证范围保持不变。**已由 PR #437 合并**（squash `0df04da4`，
2026-09-02）：内审三轮、外审 Gemini R1→R4 走到无 high；全量 5872 passed / 5 skipped。

### P2-55 — CMW500 LTE FDD MAC capability matrix ✅ 非现场半（#440）/ ✅ 现场半 2026-09-17 改验收后收口

**可观察故障**：P2-51 只闭环了 LTE FDD、2×2、满 RB、固定 RMC 的窄切片；系统尚不能在下发前
准确回答 1×1/2×2、满/部分 RB、DL/UL RMC、调制/TBS/TM、固定/AMC 与 HARQ 哪些组合可正式配置、
哪些只能诊断、哪些没有手册依据。当前“支持 MAC 配置”的单一布尔过宽。

**范围/验收**：只依据仓库 R&S 原始手册建立 profile-scoped capability matrix，并逐组合绑定命令、
参数域、回读、错误队列与证据强度；没有出处的维度保持 unavailable/diagnostic。CMW adapter 只把
既有或本片有据实现映射为逐字段 receipt，共同消费者零厂商判断。每个受支持组合均需请求/应用/回读
一致，部分确认不得发布总 confirmed；不允许用相邻配置、前面板观测或请求值补真。

**地点/依赖**：非现场取证/实现 + CMW 真机矩阵抽样复验；依赖 P2-54。现场复验完成前只开放已获
真实证据的窄组合，其余保持诊断或拒绝。
...
**2026-09-17 更新（用户现场结论，验收换源）**：开测前的矩阵抽样对「这次测试」不必要 —— 探针产出只进 `DiagnosticRun`、正式链零读取，正式执行自带同次的写→错误队列→回读证据。TM3 + 2TX 格由 09-16 执行 `dbd53e6f` 的同次 config / MAC 回执（`lte_transmission_mode=TM3`、`mimo_layers=2` 字段级 confirmed）签收；TM1 + 1TX 格降为「正式 schema 开放 TM1 时的扩域前置」，不再占现场窗口。现场半按此收口（见 Blocked on hardware 表该行）。上文「地点/依赖」里「现场复验完成前只开放窄组合」为此前表述；正式路径今天仍只接受 TM3 / 2 层，这一点不变。

### P2-56 — CMW500 LTE TDD 与真机认证（✅ 声明半 #444 / ✅ 实现半 #446 / ✅ 现场半 2026-09-17 用户裁决关闭）
...
**范围/验收**：在 P2-54 的 LTE profile 中加入有手册出处的 ULDL configuration、special subframe
...
| **② 实现半** | `configure_mac_throughput_test` 的 TDD 下发路径 + 表 2-39 满配行录入 + 放开 profile 的 **TDD** 取值域（`duplex` 与三个 TDD 字段；**不含** FDD 侧的 `transmission_mode` / `mimo_layers`）+ 活体 duplex 与 profile 的一致性校验 | ✅ **PR #446**（squash `a7bb2f0d`） |
| **③ 现场半** | 真实 CMW500 + 选件 + DUT/SIM 的 Attach / 业务窗口 / SAFE_IDLE / release / 正式证据认证 | ✅ **2026-09-17 用户裁决关闭**：09-16 真机全部接受 TDD 配置（`DMODe TDD` / `ULDL 1` / `SSUBframe 1`，`DLEQual ON` 后 DL2 跟随 DL1，选件含 KS510 / KS512 / KS550）；B41 下 attach 4/4 超时的原因是被测手机不支持 B41 频段，不是系统缺陷。TDD 下的真实 Attach / 窗口尚无真机样本，由首个用支持 TDD 频段 DUT 的正式 TDD 执行自带的同次证据验证，不再单独排现场项（见 Blocked on hardware 表该行） |

⚠️ **以下整段是 ①-era 的论证留档，描述的都是 ① 合并当时（#444）的状态，不是现状。**
② 已把它说的每一条都改掉了 —— 留着是为了记住这四件为什么必须同片做完。

> **① 有意不引入任何可达状态**：`duplex` 当时是 `Literal["fdd"]`，三个新字段只接受 `None`。
> 矩阵**声明**完整取值域（供人和 API 查询「TDD 哪些配比能正式配、为什么不能」），
> 但 profile 层不放行 —— 因为放开取值域会立刻造出「profile 说 TDD、仪器活体是 FDD」这一格，
> 而驱动当时只拿活体 duplex 跟字面量 `"FDD"` 比、从不跟 profile 比，那会把 TDD 用例静默按 FDD 配掉。
> 放开 TDD 取值域、补下发路径、补活体/profile 一致性校验、录入表 2-39 满配行 ——
> 这**四件必须同一片做完**，即上表的 ②。少任何一件都会交付出一个「路径写了但进不去」
> 或「进得去但取不到满配行」的半片。

**② 的现状**（PR #446）：取值域已全部放开（`fdd|tdd` / ULDL `0..6` / SSUBframe `0..7` /
rmc_version `0..1`）；驱动先比对活体 duplex 与**冻结 profile**、不符即 fail-loud，再按
TDD 分支逐条下发回读。四件同片做完。

⚠️ **这段留档本身是一次镜像漏改的实证**：上面那句「驱动只拿活体 duplex 跟字面量比」
在代码里的孪生（`base_station_mac_profile.py` ①-era 的注释）**已随 ② 删除**，
并在原地留了「清扫要连同被改动符号紧邻的注释一起看」的教训 —— 而**文档这一份漏了**，
到收口内审才抓到。关键词 grep 扫代码，扫不到换了措辞的文档镜像。
...
`required_options`（AND）/ `minimum_firmware`（取值级）/ `requires`（封闭枚举
`Literal["normal_cyclic_prefix"]`），`value` 联合类型加 `None`；profile digest 两处
`model_dump` 收敛成单一 `exclude_none` 口径；契约同步四步走完并加一道**双向**契约门
（既有 G11 只查「文档 ⊆ 活 schema」，加字段这个方向恒绿）。
全量 5946 passed / 4 skipped，33 道门，26 条变异实跑变红，六条 digest 基线回到 main 值。
...
资产类型与逐操作证据强度等原始声明需求，先抵扣后续实现，再保留真正缺项做后续设计。
**通道/端口基数的归属已被用户 2026-09-03 拍板修正**：随 `.smu` 改变的活动拓扑归
ChannelAsset，不进 per-driver manifest；拓扑解析另片，先补 OTA 样本与 Direction 手册依据。
排在当前已复现产品缺陷之后、P2-63 接入之前核对实际需要。先列“现有哪个对象已承载这个事实”，
复用既有来源再补真实缺口，不新造并行真值。下方旧“直通态从来没人撤”已被 P2-59 session 的
`required_safe_idle_action` / `require_channel_emulator_passthrough_clear` 路径覆盖；诊断侧旧标志仍另行待评估。
...
Native/ASC/TDL、输入/输出控制和权威回读则散落在厂商类与 MEASURE 分支。新增型号会继承假的共同能力。

**范围/验收**：建立不可变、版本化 Channel Emulator manifest，结构化声明 load mode、资产类型、
通道/端口基数、输入/输出控制、模型启动/停止、频率/level/path-loss/doppler 回读、错误队列、
SAFE_IDLE/release 与 diagnostic/formal 证据强度。registry 拒绝 manifest、类实现与运行时能力漂移；
...
### P2-58 — ResolvedChannelEmulatorBinding + 分型号 saved presets ✅ Done（① #450 / ② #452）

**可观察故障**：信道仿真器仍缺少与 BaseStation 同等的单一 binding 真值；model、connection、
LabProfile、ChannelAsset、厂商参数、loaded driver 与 execution freeze 可由不同入口读取。下一型号会
复现“GUI 已保存，执行使用另一份 endpoint/profile”的故障。

**范围/验收**：由 InstrumentConnection + LabProfile binding + manifest/profile + selected asset 解析
唯一不可变 `ResolvedChannelEmulatorBinding`；preview、保存、同步、readiness、HAL reload 与 execution
freeze 复用同一 digest。切换型号只改 GUI 草稿，保存原子持久化活动型号与该型号 preset，不覆盖其他
型号；同步只读已保存且 resolver-valid 的配置。解析过程零仪器 I/O，首个 I/O 前复核 loaded driver/
endpoint/transport。

**地点/依赖**：纯非现场平台片；依赖 P2-57。

**① 实际结果（#450，squash `1614e2d9`）**：CE 此前**没有** BaseStation 那条链的任何对应物，配置散在 `instrument.py` 里靠
`category_key in {"baseStation", "channelEmulator"}` 分支搭便车。一次做完 ≈ 12+ 文件，按 P2-56 先例拆 ①/②。
① 落地：`app/services/channel_emulator_binding.py`（resolver + preview + execution freeze，零仪器 I/O，
runtime identity 刻意排除在 `binding_digest` 外；mock 模式取所选型号**注册类**的 manifest，所以 digest 不随 HAL reload
mock↔real 翻动）；消费方 = lab_profile 保存路径 fail-closed 校验 + GET `…/instrument-bindings/channelEmulator/preview`、
readiness 加 `channel_emulator_binding`（yaml required，初版非必填被既有门 P2-27 否掉）、`test_case_runner:271` 与
`commissioning._freeze_instrument_lease` 在 BS freeze 之后紧接 CE freeze；品类键只认驼峰（删 `instrument_test_lease.py:160`
的 `or drivers.get("channel_emulator")`，用户拍板决定 ②）；契约前两步。

**用户拍板（2026-09-03）**：① P2-57 欠下的「通道/端口基数」**不进 manifest**、`.smu` 拓扑解析单独成片（真实 `.smu` 样本已收
`api-service/tests/fixtures/smu/`，未脱敏：端口数是 `[Input N]`/`[Output N]` **节的个数**，每节带物理连接器号，本样本输出为
`COMMON 3,4,1,2` —— 随文件变，放不进不可变 manifest）；② 只删 test_lease 那个 `or`，`driver_registry.py` 整套 snake 键进 Discovered；
③ 切型号丢草稿 + 确认（归 ②）。

**⚠️ 操作员可见行为变化**：execution freeze 对 CE binding **fail-closed**（与同一站点 BS freeze 同语义）—— 现有 PG 里没有
channelEmulator binding 的 LabProfile，正式执行 / 暗室首测会被拒（消息可操作）；部署前已有相位进度的旧 commissioning 会话
下次 run-phase 409 需重建；`sync-current` 保存 CE binding 要求 HAL 当前装着 CE 驱动。5 个既有测试文件 46 条用例因 fixture
从未配 CE 而红，只补 setup、不改断言、不放宽源码。

**如实声明**：「HAL reload 复用同一 digest」在 ① **无载体**（`hal_reload_policy` 不核 binding，BS 侧亦无先例），唯一真实落点是
digest 不随 reload 翻动，不为它发明机制；CE 冻结件在 ① **只写不读**（内审 F1，P2）—— 多相位 commissioning 相位间隙换驱动后
复用旧冻结件照样跑，BS 同位置有 `validate_frozen_*_before_remote`，CE 未镜像，已登记 Discovered（`[discovered 2026-09-03 during P2-58 ① 内审 F1]`）。

**过程**：7 个 subagent 并行实现（A resolver / B lab_profile / C readiness / D freeze / E fixture / F test_lease / G openapi），
主 agent 定接口契约（设计稿 §8 / §8.1a）、逐份自跑复核。E 首次实例撞 429 月度额度在读文件阶段被杀，重派完成 ——
用户随即定「并行 subagent 上限 3」。内审全套 1 轮 2 条无 P1；外审 Gemini R1 明文 clean、R2 一条 medium（binding.driver_mode 缺省默认 auto）经实测反驳未采纳（CE 该段是 BS 逐行镜像，默认 auto 会掏空三方一致性校验）。
全量 6090 passed, 4 skipped；50 条变异全红。设计稿：[`docs/plans/2026-09-03-p2-58-resolved-channel-emulator-binding-design.md`](plans/2026-09-03-p2-58-resolved-channel-emulator-binding-design.md)。

**② 实际结果（#452，squash `e8861adc`）**：镜像 BS 的 `base_station_model_presets`（#426）—— 新列
`instrument_connections.channel_emulator_model_presets` + 迁移 `a3c5e7f9b1d3`（加列 + 回填现有 CE 连接的首个 preset、
**保留** `available_channel_models`、UUID 键规范化）；`ChannelEmulatorModelPreset`（frozen，无 adapter_profile）/ `parse_*` /
`save_*`（原子：先快照旧活动、只改 target 键、目标投影成活动连接）/ `synchronize_*`（活动 `connection_params` **五个写方**
W1-W5 全部回写 preset）/ `require_saved_*`（sync-current fail-closed 检测器，位于 ① 的 resolve 之前）。API：`FEInstrumentConnection`
字段、`_convert_connection` 序列化、`update_instrument_category` 独立 CE 提前 return 块（四级回退）；openapi + 类型重生 + 手写 `api.ts` 补齐。
GUI：`channelEmulatorModelPresetDraft` / `handleModelChange` CE 分支（切型号只改草稿、有未保存草稿先确认；**P1 耦合**：原对非 BS 品类
切型号立刻只发 `{modelId}`，后端镜像 BS 后会 422，同 PR 解）/ readiness CE 灯（`invalid` → 红）/ preview service / mock。

**拍板**：③ 切型号丢草稿 + 确认。§6-① `alignment_name` **按实测定为「算」**（作为 `connection_params` 键仅 F64 驱动读
...
分段本地检查点提交。**三次 API 429**（E、I、全套内审首实例）都死在读阶段、零落盘，重派完成。门：H 12 / I 8 / K 7 / J 9 + 6 node（preset 草稿 / binding 真值）+ pytest 手写类型门 1 文件 4 用例；
变异 47 条实跑（I 的 M1c 绿且不可观察，已解释；收口 1 条先红后绿）。全量 6121 passed, 4 skipped；`npm run build` ✓；浏览器实测切型号确认弹窗（真后端）。
内审全套 3 条无 P1 —— F1 P2 SCD 关联投影按 connection 不按型号 → Discovered、F2 P3 活动 `connection_params` 为 `{}` 时首次切型号误弹确认 → 修（`canonicalParamsText` 空对象归一）、
F3 P3 W2/W3/W4 取 connection 不加锁 → Discovered；精简档复审 1 条 P3（设计稿 §8.F 行号同因漂移）→ 去行号只留符号名。
...
（2026-09-04 立项前复核：两半**均仍成立** —— P2-57 只把 14 个 manifest 操作中的 3 个在 `measure.py` 落地，另有 16 行 `hasattr(emulator, …)` / `getattr(emulator, …)` 探测（`grep -c` 可复现）探的方法全在 14 个名字之外，P2-57 的 AST 门对它们恒绿；且 P2-58 ① 冻进 execution.config 的 CE binding 全仓**零读取方**。设计稿 [`docs/plans/2026-09-04-p2-59-channel-emulator-execution-plan-design.md`](plans/2026-09-04-p2-59-channel-emulator-execution-plan-design.md) 已于 2026-09-04 批准 —— **拍板**：切片顺序 ① 计划 → ③ 单一会话 → ② 去 F64；manifest 词汇分两类（仪器操作进 manifest，证据构造 / 拓扑 getter 改基类协议必实现方法）；`f64_*` 六字段与 `f64.*` 证据键本 P2-59 不改名、另立片；手工 CE 端点与诊断序列不在 P2-59；CE 作用域嵌在 BS 会话内。）

**范围/验收**：把 manifest + binding + ChannelAsset 解析为不可变 execution plan，冻结 load mode、
必需操作、阶段顺序、预期回读与 cleanup/release；四类入口经同一 session 执行 acquire → identity →
load/configure → run/window → safe idle → release → terminal evidence。计划声明与方法缺失/漂移在 I/O 前
fail-loud；共同执行器不得出现 F64/FS16/未来厂商分支。
...
**范围/验收**：为 load/configure/start/adjust/stop/cleanup/release 建立 requested/applied/confirmed/
unknown/not-applicable receipt，保存原始 exchange ids、错误队列、attempt/lease/session、asset/binding/
plan digest 与 provenance。每个写操作的拒绝、超时、取消和安全恢复均有消费方；模拟值只能 diagnostic，
不得进入正式 KPI。F64 只映射现有有据回读，FS16 未实现操作明确 unavailable，不补占位成功。
...
**范围/验收**：复用但不复制 BaseStation 两阶段原则：Diagnostic 允许缺部分站点证明，但黄色披露、
记录操作员/原因且正式 KPI UNKNOWN/N/A；Formal certification 只能由同一真实 completed execution 的
binding/plan/asset、identity/options、逐操作 receipt、频率/level/path-loss 真值、SAFE_IDLE 与 release
...
### P2-62 — 第三种 Channel Emulator Adapter 接入认证套件（✅ PR #459）
...
**范围/验收**：测试域第三 adapter + fake transport 覆盖注册五件套、binding/preset、计划解析、
部分回读、错误队列、超时/取消、资产加载、运行/停止、动态调参、SAFE_IDLE、release、模拟排除、
正式消费者中立性与 certification fail-closed。生产代码零测试 adapter 泄漏；认证套件失败必须指向
...
猜测任何厂商命令，P2-63 仍保持 HOLD。

**最终收口（2026-09-05）**：PR #459 已合并为 `a0c671c4`。功能 HEAD `757dd2e6` 将频率证据
升级为 schema v2，绑定 `measurement_attempt_id`，qualification 摘要同时固定证据版本；认证派生
与共同正式 outcome 均核对 adapter/instrument/当前 completed attempt，现代缺证不能回退旧 F64 字段。
PR 最终记录：focused 242 passed、全后端 6393 passed / 5 skipped、compileall、单一 Alembic
head `c5e7f9a1b3d6` 与 diff-check 通过，R4 覆盖最终功能 HEAD 无 P1。此处是历史验证记录，
本次文档整理未重跑全后端；真实 CE 认证验收继续留在 hardware 表。

### P2-63 — 下一真实 Channel Emulator Adapter（HOLD，型号/手册待定）

**启动条件**：用户确定真实厂商/型号、连接协议、现有选件与可取得的原始手册，并能安排至少一次
真机窗口。禁止先用 F64/FS16 方言猜一套命令或为了占位把未实现能力声明为支持。

**范围/验收**：只实现 P2-57 manifest 声明的该型号能力与有手册出处的 adapter/profile；通过 P2-62
认证套件后进入 Diagnostic，完成同一真实 execution 的 load/configure/run/adjust/SAFE_IDLE/release、
身份/选件/错误队列和站点 certification 后才开放 Formal。原则上不得修改 MEASURE、commissioning、
Analysis、报告、下载、比较或历史；若必须修改，先登记独立平台缺口并单开 roadmap 项。
...
第三 Adapter 的型号命名稍有变化还可能被静默归为 UXM。Mock 的证据形状虽按 adapter 分流，行为能力
却是多厂商并集，不能证明真实 Adapter 的执行合同。

**范围/验收**：Mock 必须由已解析且已注册的 selected Adapter Manifest 派生 adapter identity、RAT、
operation、measurement window 与可本地判定的参数范围；删除名称嗅探与能力并集。Mock 下发侧仍复用
对应真实 adapter 的共同 builder/plan 形状，回读始终带 simulated provenance、正式 KPI 保持 UNKNOWN/N/A。
UXM+LTE、CMW500+NR、未知型号、manifest/registration 漂移均 fail-loud；UXM+NR 与 CMW500+LTE
...
覆盖 manifest 注入、能力并集/型号嗅探回归与真实 Driver 借壳，且带四类坏变异自测。P2-65 已由 PR #434 完成。

### P2-65 — Preview / Readiness / Freeze 共用兼容性判定（✅ PR #434）

**可观察故障**：第一组错误组合的 PRECHECK 仍报告 `5/5 instruments ready`；当前 Readiness 证明
仪表资源在线，却没有回答“所选 TestCase 是否可由当前 Adapter 执行”。若 GUI、同步、Readiness 与
execution freeze 各自解释能力，仍会出现界面绿灯、后台拒绝或反向分叉。

**范围/验收**：保存预览、LabProfile 同步结果、Readiness 与 execution freeze 复用 P1-75 的同一纯
compatibility evaluator、requirements projection 与 digest；分别显示 resource readiness、binding
readiness 与 testcase compatibility，不折叠成一个总绿灯。GUI 只消费服务器结构化 verdict/reasons，
不复制 RAT/operation 矩阵，也不增加 UXM/CMW 分支。未保存草稿、旧 preset、binding/manifest 漂移与
不兼容 TestCase 必须显式红灯；Mock/Diagnostic 且组合兼容只显示黄色“可诊断”。

**地点/依赖**：纯非现场后端/GUI/API 镜像片；依赖 P1-75/P2-64。

**实现（PR #434，2026-09-01）**：保存预览、LabProfile sync、Readiness 与执行冻结已共用 P1-75 的
requirements projection、compatibility evaluator 与 digest；资源、binding、TestCase compatibility
分别发布。未提供保存 TestCase 上下文、未保存 GUI 草稿、错误 LabProfile、binding/manifest 漂移或
不兼容组合均保持红灯；真实兼容组合为绿，Mock/no-adapter 仅黄色诊断。GUI 不含 UXM/CMW 或
RAT 兼容矩阵，live OpenAPI、checked YAML、generated TS、手写类型与 Mock 镜像已同步；G28 永久门
锁定共同判定链和 aggregate 黄灯。覆盖最终 HEAD 的 Codex R2 无 P1 后合并；现场 blocker 未改变。

### P2-66 — BaseStation 执行证据不变量与终态语义（✅ PR #435）

**可观察故障**：第一组执行持久化了 `adapter=uxm`、manifest `rats=[nr5g]`、requested RAT=`lte`
的自相矛盾证据，并以 `completed` 生成 PDF；虽然现有 provenance 门正确把正式 KPI 保持 UNKNOWN/N/A，
“流程完成”仍容易被解释为“配置正确/测试成功”。证据层只记录矛盾而不拒绝，历史读取也缺少明确的
compatibility classification。

**范围/验收**：证据初始化、持久化、解析、重建、下载、比较、ReportDataCollector 与历史投影均校验
冻结 compatibility digest；显式不兼容或畸形证据 fail-closed，不从当前 catalog/manifest 反推旧执行。
终态与展示明确区分 pipeline completed、diagnostic completed、valid test completed；诊断报告可作为
审计包保留，但标题、详情、下载与比较不得形成正式成功假象，数值和判词继续排除在正式 KPI 之外。
历史没有 compatibility snapshot 的行沿既有 provenance 规则读取，不批量猜测回填。

**地点/依赖**：纯非现场证据/消费者/GUI 片；依赖 P1-75/P2-65，不改变正式 provenance 白名单。

**实现（PR #435，2026-09-01）**：执行冻结重验 compatibility 外层/内层 digest、requirements、verdict 与
resolution；共同 outcome 明确区分 `valid_test_completed`、`diagnostic_completed`、
`pipeline_completed` 与 `not_completed`。SCPI evidence、MEASURE/ANALYSIS/REPORT、报告重建/详情/下载/
比较、ReportDataCollector、commissioning/history 与 GUI 全部只读这一冻结结果；显式畸形或不兼容
fail-closed，诊断审计包可保留但不形成正式 KPI/成功假象，历史无 snapshot 继续沿旧 provenance。
...
### P2-67 — BaseStation 日志与导出可追溯性 ✅ Done（#436）
...
`7ae66c69-edc5-422e-8681-d8ad56c23e64`。GUI 导出虽携带当前过滤条件，后端文件名不含 execution id，
跨执行取证容易混淆，附件本身也无法证明过滤上下文。

**范围/验收**：公共 lease/session 日志改成 adapter-neutral 表述并结构化携带冻结 adapter/binding/
execution id，不硬编码 F64/UXM；execution-filtered 导出文件名与首条元数据包含 execution id、时间、
冻结 adapter、TestCase RAT 与 compatibility verdict。屏幕与导出继续复用同一 query builder；选择执行 B
后导出不得仍返回执行 A，两个执行的附件名和内容可独立核对。原始全量日志下载保持原语义，文件名
变化不伪造或改写历史日志内容。

**地点/依赖**：纯非现场可观察性片；按批准顺序在 P2-66 后执行，不涉及仪器命令或正式 KPI。

**本地实现（2026-09-02）**：公共 lease/session 日志已改为 adapter-neutral 文案，并在结构化字段中
携带 execution id 与冻结 adapter/binding；按 execution 筛选的导出文件名包含完整 UUID，首行由服务器
...
fresh 内审 P1/P2/P3=0。**已由 PR #436 合并**（merge `07580d72`，2026-09-01）。
...
`get_instrument_catalog` 捕获后返回 `categories=[]`，全部仪器从 GUI 消失，操作员无法定位损坏项。
2026-09-05 在 `a0c671c4` 用生产函数、内存对象和 fake DB query 构造复现：损坏认证返回 0 类，
相同连接改为无认证返回 1 类。没有发生数据库删除。来源：#458 行内发现 3938243884，本次独立 triage。

**最小方向/验收**：收窄认证解析失败的影响面，保留可显示的 category/model/connection；损坏认证
明确 invalid/unavailable 且不能得到正式资格。枚举 BS/CE certification、preset 与目录各转换入口，
...
→ 新纯模块 `invalidStoredFields.ts::withoutSynthesizedConnectionParams` 在该字段被标坏且操作员未填新 JSON 时不发送它（轻量内审再补：CE「F64 User Alignment」输入框同时禁用，否则填一个名字就会合成 `{alignment_name}` 覆盖原值；R2 P1 再补：管理员修库后抽屉未关、目录刷新清掉标记，旧空草稿仍会被当 `{}` 发出；R3 P1 再补：健康草稿所在的行变坏时，从服务器灌入的旧 JSON 会被当操作员输入放行 → 用户拍板 A：草稿带三态 `connection_params_origin: server | invalid | operator`，标坏时只有 operator 文本保留、其余清空标 invalid，修好时 invalid 用服务器值重建并连带重建派生的 BS profile 草稿，守卫 `connectionParamsGuarded` = origin invalid 或（标记存在且非 operator）；切型号 / JsonInput / alignment / BS profile 字段输入都标 operator；BS 的 `base_station_adapter_profile` 与 connection_params 同源同守卫：行变坏一起清空、修好一起重建、被守卫一起不发；R4 P1 再补：被守卫时 BS profile 输入框与 CE alignment 一样禁用，局部编辑不能解锁整份存值替换；BS 切型号对 deselect / 同型号 noop，与 CE 对称）；P2 —— 契约 description 与
...
2026-09-05 实跑 model_dump 与 YAML 属性集比较，差集为 `['dimensions']`。

**最小方向/验收**：补齐 dimension/value capability 的 live OpenAPI、checked YAML、generated TS 与
手写类型；用实际序列化响应验证契约，不能只验“文档是实现的子集”。不改变 manifest 运行内容、
digest 或正式能力，不为契约补齐顺带放开 CMW 取值域。来源：P2-56 声明半 discovered。

**设计稿与拍板（2026-09-11）**：[`P2-69 设计`](plans/2026-09-11-p2-69-mac-dimensions-contract-design.md)
...
### P2-70 — CMW FDD 矩阵抽样的可达载体（非现场实现；2026-09-17 起不再要求现场验收）
...
已由 PR #475 合并。**2026-09-17 更新**：P2-55 现场半已按改后的验收收口（见 `### P2-55`），本探针降为「正式 schema 扩域（如开放 TM1）时的前置工具」，09-16 现场未跑成（预检拒 TDD 态），也不再排现场窗口；本地测试仍不等于真机验收。以下为此前表述：P2-55 现场半保持开放，不把本地测试写成真机验收。
...
诊断也未提供前一条载体。只把事项标成 hardware blocked，设备到场仍无法完成抽样。

**首片范围**：新增可从现有诊断入口运行的 `cmw500_fdd_matrix_probe` 参数化载体，覆盖 TM1/1 TX
与当前 TM3/2 天线；命令、组合、RMC 行、选件/固件和安全前置必须逐项有手册出处，输出为诊断证据。
不把单维 authoritative 当成跨维组合合法，不默认开放其他 TM、4 层、AMC 或全带宽。
...
### P2-74 — Aerotech 转台：行程超过阻塞预算的移动按段下发（2026-09-18 立项；本地半 ✅ PR #482，现场半待验）

**本地半已由 PR #482 合并（merge `4d6050fb`，2026-09-19）**；本项不再占用非现场 WIP。现场半见下「验收」，随下次现场走。
...
**D8 其余形态的归宿**：现场总结 D8 里「全天 `Disconnect error` ×10 / 懒重连 ×9」是同一个空闲超时在**命令间隙**触发
（09-16 日志：这 10 次每次距上一条转台往返 ≥ 13 s，无一低于 10 s；唯一一次命令中途断连就是那条 `MOVEABS X -90`），
下一条命令前由既有懒重连重建连接，本片不动；A 方案调大 `Socket2Timeout` 两种形态一并治。
...
每道门配变异实跑。**现场半（按此次序）**：① **先在控制器原有超时下**用正式多方位用例复跑（它的方位移动走 `move_to()`）—— 先调大超时再跑，
分段修复就从没在 10 s 约束下被验证过；② 用厂商工具**读出并记录** `Socket2Timeout` 的实际值，**然后**才把它调大
（A 方案，可能要重启控制器）。设计稿 §3 另两个未知：固件是否支持 ASCII `WAIT MODE NOWAIT` —— 目前**没有载体，不盲试**；
断连后 `ABORT` 为何 8 s 才被确认 —— 只有断连再次发生时，才能从正式用例的原始日志里记。
...
重复全量、纯文档触发后端全量，以及“建议默认 backlog”和 AGENTS 的冲突。执行方法唯一入口为
CLAUDE 的 `验证分档与结果复用` / `外审请求与等待`；reviewer 与 PR 模板引用同源规则。
取证与范围见 [复盘 §4](plans/2026-09-05-development-review-and-triage.md#4-回归策略优化)。
外审改为先读状态、确认 HEAD、按 HEAD+Rn 去重、当次消费完成结果，删除固定等待与超时合并冲突。
保留正常 R1→R2 与功能 P1 续审；独立审查、自查与未发生分别记账。不新建测试/审查平台，不批量
删测试，不恢复并行 agent，不改产品代码。后续两个功能 PR 记录耗时与重复验证等数据后再验效率；
本片未修改 GitHub/Codex 调度器或恢复 automation，不能宣称服务端等待时长已缩短。

**P3-1～P3-19 ✅ Done；P3-20/P3-21 已转非阻塞维护池；P3-22 ✅ PR #369；P3-23 规则固化于本 PR、试行待验。** 已完成项的完整 What / Fix / Acceptance 详情已迁出 → [`roadmap-archive.md`](roadmap-archive.md)；新增项的当前范围与顺序只看顶部 Current Focus 表。速览：
...
| U-8 | Aerotech AeroBasic/TCP 活跃控制路径的现场型号与固件如何只读确认？ | **Hardware/Vendor-Protocol Blocked**：[2026-09-22 取证](plans/2026-09-22-p1-79e-aerotech-identity-research.md)确认当前资料不足；P0-5 前仍须取得覆盖现场确切型号/固件的同协议安全只读 recipe，或独立批准 Windows SDK 采集子系统；数据库/连接配置声明不能代替实时身份 |
| U-9 | 转台控制坐标到暗室 DUT 物理方位的偏置真值与标定状态是什么？ | 2026-08-27 诊断已证明 `PFBK - MOVEABS = +90°`；P0-9B-3 已把该真值、可追溯来源和验证时间冻结进正式 execution，writer 同时核对请求物理角、实际 MOVEABS 程序角和原始 PFBK。下一步现场验证四方位反馈误差 ≤ ±1°，并另行取得 HOME 最终 PFBK。 |
| U-10 | CMW500 `PCCBBBoard` 如何权威确认？ | ✅ **2026-09-16 现场已复验**：执行 `dbd53e6f` 专用回读 `SUA1,RF1C,RX1,RF1O,TX1,RF3C,TX2` 与写入逐字段一致，通用 query 的 Controller 字段为 `"No Connection"`（不可用作证据，与设计一致）。以下为此前表述：**本地半完成，待现场复验。** 依据 LTE UE Manual 1173.9628.02-41 §2.6.8.1 pp.630–631 与 Remote Control via SCPI 1179.4592.02-04 §3.6 p.22，已实现 `ROUTe:LTE:SIGN<i>:SCENario:TRO:FLEXible?` 七字段严格回读，并与通用 query 的六个物理路径交叉确认。真机原始响应未取得前 Route 仍只作诊断、正式 KPI UNKNOWN/N/A。 |
...
| U-13 | UXM 在现场 LTE_NR_IRAT Test Application 下，能否按手册的 Single + Length + progress-count 形成可重复、互不继承的原生有限窗口？ | 关联 Blocked 表 P2-52/P2-54。手册示例只证明 NSA/SA 路径，不替代 LTE_NR_IRAT 厂商依据；资料未齐前禁止用软件诊断试写。下一步先取得 IRAT 适用性、值域/前置条件、错误队列和 cleanup/reset 的可审计依据，再设计独占监控、持久 quarantine/reset 解除门与两窗口载体。现有 `uxm_window_boundary_probe` 仅为零写辅助，不能关闭本项；本项继续 Blocked。 |
...
**本表给出 2026-09-16 现场发现的当前出口；原始事实在[现场总结](site-debug/2026-09-16-cmw500-f64-onsite-summary.md)与 runbook §7。** 编号与顺序已由用户 2026-09-18 批准；P1-79A/B/C 已由 PR #491/#493/#494 合并，P1-79D 软件实现与本地验证已完成、交付状态以本片 PR 为准，P1-79E 已完成取证并转 Hardware/Vendor-Protocol Blocked。

| 发现 | 当前出口 | 依据与下一步 |
|---|---|---|
| D4 CMW500 配置回执三个通用字段恒 unknown（→ 签发不了站点认证）；D5 / D6 正式强制证据的基站一侧只有 UXM 实现，CMW500 的配置 / 吞吐证据无写方、无频率身份（同一份证据里 `f64.output_state`、`positioner.azimuth.000` 也是 `unknown`，后者与 U-8 同源） | → **P1-79A/B/C 已由 PR #491/#493/#494 合并；P1-79D 软件完成、交付状态以本片 PR 为准；P1-79E Hardware/Vendor-Protocol Blocked** | 缺口在资格、强制证据两层，与校准无关；三层关系与判定见表下。`dbd53e6f` 是修复前事实；C 已让配置 E3 与吞吐 E4 复用同一冻结回执 / 窗口真值，D 只从同次已确认 CMW Band/EARFCN/BW 形成频率身份；P1-79E 的详细解除条件见取证裁决，CMW500 命令 / 回读须有 R&S 手册出处 |
...
| D1 `instrument_idn_sweep` 范围与身份契约 | ✅ **P2-75 / PR #486** | 「所选 LabProfile binding × 当前启用类别」+ connect 缓存身份投影；零新增仪器 I/O |
| D9 `input_level_calibration` 空回读仍记 `success: true` | ✅ **P2-76 / PR #487** | 拆分下发与验证；空/部分预读只记 pending，复用同次 Cell-ready 功率观察后才可成功，缺真值 fail-closed |
| D10 `.smu` 中心频可被 API 改写，资产频率不排他 | ✅ **P2-77 / PR #488**（现场半 Hardware Blocked） | 软件已按手册边界实现：vendor asset 登记频率只是工程默认，仅在冻结有界能力 + 全组限值预检 + 写错误门 + 全组精确回读全部成立时，TestCase 频率才是本次执行真值。旧冻结件/其他 adapter/其他 source type 不放宽；真实 F8800A 矩阵仍待现场 |
| D18 F64 工程替换后可能沿用上一工程 bounded-frequency proof | ✅ **P2-78 / PR #489** | GCM/local、B2、ASC 三条成功替换路径统一在 FILE 紧邻错误门干净后、首次后续 await 前清除；FTP/CLOSE 前置失败不误清，确认卸载后的 FILE 失败走既有 reset。R2 无 P1；其取消窗口 P2 仅报告、未自动排期 |
| D11 FDD 预验证非必要 | **resolved（本次文档）** | P2-55 验收已换源；runbook §2 降为可选 |
| D12 功率观察窗的厂商分支踩红两道规则门 | ✅ **resolved（PR #480）** | 换成 `_cell_ready_power_observation_is_wired()`：看基站驱动是否实现配置功率回读，不看基站厂商身份（CE 一侧的 `ce_plan.adapter_id == "propsim_f64"` 保留）；两道门转绿。取舍：判据来源是驱动实现而非冻结 manifest（走 manifest 需要 Mock 基站也支持回调，未做） |
...
| **P1-79B（本片软件修复，PR 合并后关闭）** | `config_confirmed=False` → 签发不了基站站点认证（09-16 无人尝试签发，去签也会被拒）、attempt 生命周期与正式信封返回 `config_not_confirmed` | 配置其实回读了：10 个非空请求字段 7 个 confirmed；`radio_technology` / `channel_kind` / `frequency_mhz` 是冻结描述 / 派生字段，不是独立仪器控制；旧规则却要求它们具有设备回读 | 已从配置回执的必需确认集合移出三个描述字段；冻结 request / compatibility / RAT-aware schema 继续保护这些真值，全部实际控制字段仍须同次权威回读，任一缺失或漂移继续 fail-closed；未新增/修改 SCPI，未改变 manifest/digest/provenance |
| **P1-79C（本片软件修复，PR 合并后关闭）** | `base_station.pcell.config_applied`、`base_station.throughput.azimuth.NNN` 恒缺失（45.82 Mbps 读到了但没被记成强制证据） | `measure.py` 以 `hasattr` 守着写方，专用 builder 与身份快照只有 UXM 驱动有；记录器本身按 UXM 目录写死 | 已实现 vendor-neutral 共同投影：配置只消费同 attempt/lease/session 的冻结 manifest、配置与 attach 回执；吞吐只消费 cleanup 后持久化的同一窗口与 metric registry。错误仪表、字段缺 proof、模拟/诊断、漂移与畸形现代证据均 fail-closed；执行器不再按 builder 存在性分叉。UXM 共同配置不足时仅保留既有已核验目录兼容回退，CMW500 不借该路径升级；未新增/修改 SCPI、未改变 provenance 白名单 |
| **P1-79D（软件完成，交付状态以本片 PR 为准）** | 正式判词恒停在 `frequency_identity_not_fully_verified` | 基站侧 `get_frequency_identity` 仅 UXM 有，频率一致性网记 BaseStation「未报告(跳过)」 | 复用现有有 R&S 手册出处的 Band / DL EARFCN / bandwidth 精确回读；仅同次正式配置回执三个 control 均 confirmed 才形成 RAT-aware 身份，失败/取消/旧入口/断连均清旧证明；不新增 SCPI、不从请求或默认缓存补真 |
| **P1-79E**（= U-8；Hardware/Vendor-Protocol Blocked） | `positioner.azimuth.NNN` 恒 `unknown`（角度回读无误，误差 0.0°） | Aerotech 驱动的身份快照把 `model` / `firmware_version` 写死为 `None`；[2026-09-22 取证](plans/2026-09-22-p1-79e-aerotech-identity-research.md)确认当前取得的 Ensemble 3.04 资料没有可审计且覆盖现场未知型号/固件的只读 recipe | 保持 `unknown`；解除条件 = 厂商给出覆盖现场控制器/版本的同协议只读 recipe，或另行批准并设计 Windows SDK 身份采集子系统；禁止把 Automation1 函数、configured model 或 SDK 安装版本移植为运行时真值 |

判「正式」有三层：① 资格 ② 强制证据 ③ ANALYSIS 阶段的 KPI 结论。前两层没有路损 / 校准条件，上述子项都在这两层；第三层按顺序还要过频率身份 → 路损校准 → 吞吐 → RF 指标 → 静区场扫描 —— 子项做完而校准未做时，站点认证签得出、执行可归 formal、证据层通过，但报告 KPI 结论仍 `UNKNOWN`，原因换成路损。候选（不立项）：CMW500 的 RSRP / SINR / RI 无真实来源证据（`rf_kpi_verified=false`），校准启动时再评估。

- `[discovered 2026-09-18 during PR #480 内审；待 triage]` **F64 stop 回执在两种真机形态下恒 unknown，配了直通的真机执行 CE 终态链恒 invalid** —— `scpi.log.2026-09-16` 12 次 GOS：RUNNING 下 7 次「队列干净 + STOPPED」（#480 起回执可确认）；已 STOPPED 再 GOS 3 次、CLOSED 下 GOS 2 次均报 `-200 Wrong device state for command`（与手册 §20.5.2 一致）→ 证据因队列不干净判 rejected、回执 unknown。直通流程在加载后会预停一次（加载后即 STOPPED，GOS 必 `-200`），这条回执落在链中部 → 凡配了 `f64_bypass_mode` 的真机 completed 执行，CE 终态证据链仍判 invalid。方向是 fail-closed、不产假数据；修法要先决定「幂等的已停 GOS」算不算确认，须有手册依据。
- `[discovered 2026-09-18 during PR #480 内审；待 triage]` **Cell-ready 功率观察的严格门判不了「无信号」** —— 真机上无输入信号的实测读数是很低的有限值，不是空值：09-16 执行 `f8f5fd90`（B41 TDD）F64 输入口 2 两次采样均为 −108.0 dBm、输入口 1 为 −28.9 / −30.7 dBm，16 路输出均值 −52.3 / −53.8 dBm，比上一次取到观察的执行 `d058d9de`（−50.0 / −49.8 dBm）低 2.3 / 3.9 dB（第二次多掉的部分与输入口 1 自身低了约 1.8 dB 同向，不全是缺一路造成）（`app.log.2026-09-16`；现场总结原稿未记）。严格门只拦「读不到数」，缺一路 TX 时照常放行（#480 已把放行状态改名为 `recorded` 并写明局限）。要判有无信号须换到带该语义的查询并有 PROPSIM 手册出处，不凭一次观察值加阈值。
- `[discovered 2026-09-18 during PR #480 内审；保留候选]` **CMW 单发窗口「墙钟 + 1 s 宽限」只有一个真机样本** —— `dbd53e6f`：INIT 后 5.14 s 仍 RUN、5.24 s 到 RDY。大统计基下滞后是否变大未知；超宽限的代价是白跑一窗（置空，不出假数）。暂不立片，下次现场多取几个不同统计长度的样本再评估。
- `[discovered 2026-09-16 during CMW500 + F64 现场；2026-09-17 triage]` **CMW500 适配器拿不到正式判词的一组软件缺口（→ 候选 P1-79A～E，低优先级备选：对日常测试没有必要）** —— 执行 `dbd53e6f` 的配置、路由、MAC、窗口、cleanup、release 全部 confirmed，但 `base_station_execution_evidence.config_confirmed=False`：`radio_technology` / `channel_kind` / `frequency_mhz` 三个通用字段 `applied=None, status=unknown`（「CMW500 configuration field was not authoritatively confirmed」），后果是签发不了基站站点认证（`activate_base_station_site_certification` 要求 `config_confirmed is True`）；强制证据 `base_station.pcell.config_applied` 的缺失另有原因 —— 写方以 `hasattr(base_station, "build_p0_5_config_evidence")` 守着，而该接口仅 UXM 驱动有；同理 `RealCmw500Driver` 没有 `get_frequency_identity`（BaseStation 驱动里仅 `uxm_base_station.py` 有；`propsim_f64.py` 的同名方法属信道仿真器一侧），频率一致性网记 BaseStation「未报告(跳过)」→ `fully_verified=false` → `formal reason=frequency_identity_not_fully_verified`。另 `base_station.throughput.azimuth.000` 在窗口 confirmed、`throughput_valid=true` 时仍列为缺失：已核不是前者的连带，而是 CMW500 没有 `build_p0_5_throughput_evidence`，读到的吞吐从未被记成强制证据。这些缺口出在与校准无关的资格、强制证据两层 —— 校准齐备也救不了；反过来，补齐它们后报告的 KPI 结论仍要过 ANALYSIS 层的路损校准条件（见现场总结 §5）。
- `[discovered 2026-09-16 during CMW500 + F64 现场]` **Aerotech 传输不稳与单轴日志噪音（P2 + P3，→ P2-74，本地半已由 PR #482 合并，现场半待复验）** —— 12:13 执行 `7d347c03`：`MOVEABS X -90.0000 XF5.0000` 发出 10 s 后 `ConnectionResetError [Errno 54]`，静默重连只恢复了传输；非幂等命令结局未知、未重放（正确）；ABORT 读 `VFBK(X)` 得空串无法证明停止 → 急停并以零速度确认。全天 `Disconnect error [Errno 54]` ×10、「transport already closed — lazy reconnect」×9、`positioner/hal/activate` 一次 503（旧驱动断开未确认）。另：单轴转台每次连接都先以 ERROR 级打印 `AeroBasic error for 'PFBK(Y)'`（×30）再识别为单轴，污染错误统计。
- `[discovered 2026-09-16 during CMW500 + F64 现场]` **`input_level_calibration` 在回读全空时仍记成功（P2，→ P2-76，已批准）** —— `dbd53e6f` 的 `measure.input_level_calibration`：`success: true`、`failure_reason: null`，而两路 `readback` 的 `avg_dbm` / `crest_db` 均为 `null`（Cell ON 前 `measure_input` 返回空串，日志 `parse failed: ''`）。当日新增的 Cell-ready 功率观察窗在 Cell ON 后取到了真值（−29 / −28 dBm），但这个字段仍把「没测到」写成「成功」。
- `[resolved in software 2026-09-22 by P2-77 / PR #488；原 discovered 2026-09-16 during CMW500 + F64 现场]` **`.smu` 内置中心频可被 API 改写，文件名与资产登记频率不排他（现场半 Hardware Blocked）** —— 同一个 `TS_LTE_MIMO_OTA_SCME_UMa_8DP.smu` 当日先后被 `CALC:FILT:CENT:CH 1,2565.0`（B41）与 `1,1960.0`（B2）改写并被仪器接受。驱动层早已按「显式下发 > 加载后回读 > 文件名」处理，但资产层仍把频率当排他身份：执行 `f7afe522` 在 F64 已接受 1960.0 的情况下，被 P2-11 以「SCD 登记 B41/40340 ≠ 用例 B2/900」拦下，操作员随后改了资产登记才放行（其间两次 `PUT /channel-assets` 的 400 是 `19600 MHz` 笔误被一致性校验正确拦下，与排他性无关）。PROPSIM User Reference Rev 10.2 §20.4.6.1 / §20.4.6.2（pp.280–281）和 §20.4.6.8（p.282）确认逐组写入/回读、允许范围以及越界自动钳位语义。软件现已按 A 方案实现：vendor asset 登记频率降为工程默认/审计值，本次 TestCase 频率只有在冻结 F64 bounded capability、全组限值预检、写错误门与全组精确回读全部成立时才成为执行真值；否则 fail-closed。真实 F8800A 上的两合法频点 + 一越界频点 + 逐组回读仍待现场验收。
- `[resolved 2026-09-22 by P2-78 / PR #489；原 discovered during PR #488 R2]` **F64 工程替换后可能沿用上一工程的 bounded-frequency proof** —— GCM/local、B2 与 ASC 三条 `CALC:FILT:FILE` 成功路径原先都不经过 `_apply_unload()`；一旦 FILE 已替换工程、但后续有界调频 proof 尚未写入，上一工程证明会继续留在驱动缓存。PR #489 让三条路径都在 FILE 完成且紧邻错误门干净后、首次后续 `await` 前清除；FTP 失败或 CLOSE 未确认时保留旧工程证明，FILE 被拒沿用既有 unload/failure reset；未新增/修改 SCPI。R2 无 P1并按规则合并；R2 报告的 FILE 事务中途取消 P2 未自动进入 Discovered/backlog。
...
### 2026-09-05 本轮开发 triage checkpoint

**本表给出当前出口，覆盖下方同源历史发现的旧“待 triage”标记；原文保留审计。**
本次用户明确要求整理后才独立重评 #458 的剩余项；不是把 R2+ P2/P3 自动积压。
未列出的早期发现仍是待评估，不视为已经复现或已经关闭。
...
| #442 自然语言检查器返工、roadmap 顺序/链接门、测试冗余 | → **P3-23 流程优化评估** | 优先删冲突、少重复运行；是否改检查器/删测试另需具体证据，不按测试行数定目标 |
...
- `[discovered 2026-09-16 during CMW500 + F64 现场调试]` **`instrument_idn_sweep` 把停用的旧 LabProfile binding 纳入扫描，并把已加载真实驱动的身份接口差异误报成仪表无响应（→ P2-75，2026-09-18 已批准）** —— 现场 `diagnostic_run_id=94a93c8a-9035-4190-acef-baea28b41cad` 报 `7/7` 失败；当前仪器目录实际仅启用 BaseStation、Channel Emulator、Positioner、RF Switch 四类，而 `CAICT-Lab-1.instrument_bindings` 仍有七类，且 VNA endpoint 已与目录漂移。四个已加载驱动均在 `0 ms` 因序列只探测 `get_identity/query_idn/query` 而失败，未发生仪器 I/O；CMW500、F64/UXM 已有 adapter 结构化身份/环境快照，转台明确没有安全型号/固件查询，RF Switch 尚无统一身份投影。后续设计须先列全目标选择、租约、身份产生方、GUI/审计消费方：默认使用“所选 LabProfile binding × 当前启用类别”的受控范围并显式列出排除/漂移项；以统一只读结构化身份投影输出 loaded adapter、endpoint、observed identity 及 `match/mismatch/unknown/not_applicable`，服务器核对 selected model，不开放任意底层 `query("*IDN?")`、不新增/猜测身份 SCPI。测试必须覆盖真实 CMW500/UXM/F64/转台/RF Switch 接口形状及“七条历史 binding、四类启用”的反例；现场原始证据见 [`guides/2026-09-16-cmw500-f64-onsite-runbook.md`](guides/2026-09-16-cmw500-f64-onsite-runbook.md#instrument_idn_sweep-%E8%8C%83%E5%9B%B4%E4%B8%8E%E8%BA%AB%E4%BB%BD%E8%AF%BB%E5%8F%96%E5%A5%91%E7%BA%A6%E5%A4%B1%E6%95%88)。本条当时尚未升格；2026-09-22 已获用户批准并进入 P2-75。

> **P2-75 本地收口（2026-09-22，待外审）**：扫描范围已收窄为“所选
> LabProfile binding × 当前启用类别”，并在同一服务器快照内核对 selected
> model、saved endpoint 与 loaded adapter。CMW500 / UXM / F64 只消费连接阶段
> 已缓存身份；转台 / RF Switch 因没有获批的无侵入身份投影而标
> `not_applicable`；Mock 或真实身份快照不完整为 `UNDETERMINED`，配置 / adapter
> 漂移与驱动缺失为 `BLOCKER`。R1 后又补齐 driver mode 三方一致性，并在
> 公共仪器租约内按 category → LabProfile → connection 顺序锁定、重读当前真值，
> 因此等待租约期间发生的保存 / 启停变更不能再以旧快照显示成功。历史 / 停用
> binding 在 `excluded` 中逐项留因，
> 不再抢租约，也不再调用通用 `query("*IDN?")`。本地最终验证：P2-75 专项
> 21 passed，相关上下文 / 规则门 184 passed，全后端 6624 passed / 5 skipped，
...
- `[discovered 2026-09-16 during CMW500 + F64 现场调试]` **`propsim_f64_p08_gate` 的序列语义硬编码 UXM，却不校验当前 BaseStation adapter/binding，CMW500 场次误勾选后可对 F64 执行错误条件下的有状态动作（P2，→ P2-73，2026-09-18 已批准）** —— 序列 metadata 只要求 `channelEmulator`，参数门只判 `uxm_dl_confirmed is True`，不读 BaseStation selected model、loaded adapter、endpoint 或 LabProfile binding；而界面和失败文案明确写着“UXM 满 RB DL”/“UXM DL 接入”。当日 CMW500 配置下的 `diagnostic_run_id=78aa4816-7eab-435b-aec9-7866a1152d31` 因未勾选而在 624 ms 参数门安全拒绝，未进入 F64 动作；这不能证明它会阻止“CMW500 + 误勾选”，因为代码无该判定。立即处置是从 CMW500 现场手册移除该步，不切换 UXM、不虚假勾选、不为诊断改写 LabProfile。后续修复前必须先枚举 params schema / run gate / metadata categories / GUI / DiagnosticRun / runbook / P0-8 载体全部消费方；最小安全修复是让现有序列按服务器权威 BaseStation 身份 fail-closed 为 UXM-only，若要支持 CMW500，则须单独设计厂商中立的输入源契约及 CMW 下行活动的权威证据，不能只把布尔参数改名。本条不改变 2026-08-27 已归档的 CMW500 输入历史观察，但当前序列不能作为 CMW500 复验载体。

> **P2-73 收口（2026-09-22）**：上述 `propsim_f64_p08_gate` 软件缺口已关闭。
> metadata 现在要求 BaseStation；服务器在租约锁内复用权威 resolver，只有
> selected model、LabProfile binding 与 loaded adapter 同时为真实 UXM 才可在
> Remote/F64 I/O 前通过，诊断结果归档同一份冻结 binding。CMW500、Mock、
> 缺少或漂移均 fail-closed；`uxm_dl_confirmed` 仅保留为物理满 RB DL 确认。
...
- `[discovered 2026-09-04 during P2-58 ② 内审 F1]` **SCD 关联投影按 connection 归属、不按 CE 型号 —— 非 F64 型号活动时关联 SCD，F64 的派生条目会灌进活动型号的清单与 preset（P2）** —— `standard_channel_service._sync_projection_for_binding`（:339）用该 connection 下**全部**已关联 SCD 重建派生条目写进活动 `connection_params`，再由 ② 的 W4 回写进**当前活动型号**的 preset；而 SCD 只挂 connection（`StandardChannelDefinition.instrument_connection_id`），没有型号归属；GUI 的 `StandardChannelDefinitionCard` 也只按 `category.connection.id` 渲染（`App.tsx:2443`），不看活动型号。内审探针实跑（复用 ② sync 用例的 fixture）：F64 存 preset → 关联 SCD1 → 切 FS16 保存 → 抽屉里 SCD 卡仍在、再关联 SCD2 → FS16 的活动 `available_channel_models` 与 FS16 preset 同时含 SCD1（另一台仪器上的文件）；切回 F64 → 清单里没有 SCD2，直到下次在 F64 活动时再触发 W4 重建。不产假数据（下游加载会在仪器侧失败）故 P2 不 P1。② 之前 `connection_params` 本就 per-connection 共享、投影没有型号概念；② 把参数分到型号、W4 又镜像进活动型号 preset，SCD 却仍只认 connection —— 边界只切了一半。修法：**加机制**（SCD 记 `instrument_model_id`，或投影按型号分桶）；「只在活动型号是 F64 时投影」要按型号名判，正是设计稿 §8.E 拒掉的脆判据。⑤ 审查轮不加机制，② 未修；设计稿 §8.F 已补「W4 不分型号」一句。
- `[discovered 2026-09-04 during P2-58 ② 内审 F3]` **W2 / W3 / W4 取 connection 行不加锁，与 PUT 切型号并发时 CE preset map 会丢项（P3，窗口窄）** —— `api/instrument.py` 的 `add_channel_model_entry`（:1952）/ `remove_channel_model_entry`（:2044）用 `.first()` 取 connection，`standard_channel_service._sync_projection_for_binding`（:346）用 `db.get`；对照同文件 PUT CE 块（:2110 category / :2280 connection）、BS topology-profile（:1111 / :1121）与 W5 `smu_project_inventory._lock_sync_truth_rows`（:672）都 `with_for_update()`。PG 下：A 发 PUT 切 F64→FS16（持锁，写 map {F64, FS16}、活动=FS16）；B 在 A 提交前读到 conn（map {F64}、selected=F64）发 `POST channel-models`，其 UPDATE 等 A 释放行锁后落地 → map 被盖成 {F64: …}（FS16 项丢），且活动 `connection_params` 被写回 F64 的清单而活动型号已是 FS16。前置是两个操作员同时操作同一台 CE。`connection_params` 的这类竞态 ② 之前就在（W2 vs W1），② 把它扩到了 preset map。修法：**收窄** —— 三处镜像 PUT 块的锁序（先 category 再 connection，别反过来，否则与 PUT 互相等锁）；注意 W4 的连接对象在 `_resolve_channel_emulator_binding`（:50，`db.get` 在 :60）已进 identity map，:346 的 `db.get` 不再发 SQL，加锁须走 `Session.get(..., with_for_update=True)`（SQLAlchemy 2.0.51 `Session._get_impl` 的 identity-map 旁路条件含 `for_update_arg is None`），并核实已加载属性是否被刷新。SQLite 下 `FOR UPDATE` 被方言忽略，测不出丢项也测不出死锁，须在 PG 上验。② 按 ⑤ 未修。
...
- `[implementation verified by P2-67 on 2026-09-02；discovered 2026-08-30 during 双执行日志取证]` **公共 release 日志硬编码 UXM、execution-filtered 导出文件名不含 execution id（P2）** —— 公共租约日志现已厂商中立并携带冻结身份，execution-filtered 导出拥有独立 UUID 文件名和服务器冻结元记录，普通导出与 raw download 保持原语义；fresh 内审 P1/P2/P3=0。**出口：→ P2-67，已由 PR #436 合并**。
...
- `[discovered 2026-08-30 during P2-51 CMW500 MAC 取证]` **LTE TDD 正式路径缺口（P2）** —— TestCase 契约无 LTE TDD 配比字段；`CELL[:PCC]:ULDL`（手册 p.687）+ `RMC:VERSion:DL<s>`（p.803）未实现。NR slot 字符串不可如实翻译为 LTE 0..6 配比。⚠️ 「当前 TDD duplex 下 configure_mac_throughput_test 整体 fail-loud」这句**已随 P2-56 ②（#446）作废**：驱动现在按 TDD 分支下发并回读，只在活体 duplex 与冻结 profile 不符时才 fail-loud。详见取证清单 §8。**出口：→ P2-54（RAT-neutral profile）+ P2-56（LTE TDD 与真机认证）。**
...
- `[resolved to P1-75/P2-64 on 2026-08-30；discovered during P2-53]` **MAC blocker 的 mock 豁免仍在计划判据外** —— 后续 UXM Mock + LTE TestCase 已形成可观察故障，不再维持“当前无故障”的旧判断。**出口：→ P1-75** 在执行准入层统一对账 TestCase requirements 与 manifest；**→ P2-64** 删除 Mock 能力并集与型号嗅探。P2-53 认证套件历史范围不回写。
...
08-21 checkpoint 的出口表只"覆盖"了下方原文、没有划掉原行，读者扫列表会看到 63 条像是开着的。
本次按出口表逐条对账并改写原行：**51 条**加了划线与出口（已交付 / 维护池 / 功能启用池 /
硬件 Blocked / 流程已解决），**12 条仍真实开放**（见下方未划线行），分三族：

| 族 | 条目 | 出口建议 |
|---|---|---|
| 真机证据链 | `uxm_kpi_readback` 前置写未包 `try`；`_read_orig/_csi_state` `.strip()` 破坏逐字 raw；`configure_mac_throughput_test` 返回值无人消费（ON 态写被拒静默带旧配置测）；05-27 现场结论未喂回驱动 | 合为一条 **P2 候选「UXM KPI/MAC 配置真机证据链收口」**，下次现场前做 |
| 日志可分析性 | `/system-logs/tail\|search` 无 `instrument_id` 过滤 —— 与顶部「阶段性 release 评估」缺口 A 同源：`current_instrument_id` contextvar **全仓零写方**，驱动层行 0% 带仪器身份 | 并入顶部候选 1 |
| 配置 / GUI / 门 | `.env.example` 无 log 段（APP_VERSION 同一教训）；P1-26 跨频段 band 不符 + 三站点无门；`is_mock_driver()` 权威源；执行偏好开关未接线；`created_by="gui"` 占位；旧报告真假标注支线重做 | 逐条 P3，按价值单独 triage |
...
| Pattern real activation | ✅ P2-32B：唯一 production caller、显式 Real/Mock、冻结路由/warning 与正式消费 fail-closed；真机验收仍 Hardware Blocked |
...
- ~~`[discovered 2026-08-11 during P3-18 G11 external review, Codex #327 R1]` **G11 未合并 OpenAPI Path Item 级 parameters（P3，待 triage；不阻塞已合并子片）** —— 当前 location 比对只读 operation 自身的 `parameters`；若 query/header/cookie 参数声明在 Path Item 级并对全部动词生效，它从 live 消失或换 location 时本门仍可能假绿。后续若继续精化 G11，应把 path-item 与 operation 两层参数按 OpenAPI 覆盖规则合并后再比较；按测试门意见上限记 P3，不为其启动第二轮。~~ → 非阻塞测试门维护池（2026-08-23 账面清理）

- ~~`[discovered 2026-08-11 during P3-18 p08 external review, Codex #328 R1]` **p08 参数门锁住 checkpoint 存在与标签，但未直接锁调用位置（P3，待 triage；不阻塞已合并子片）** —— 当前 fake 错误是在 `residue()` 内注入；若以后把某个 checkpoint 移到受保护写操作之前但保留标签，参数用例仍可能通过。功能实现与本轮逐站错误归档均已复核正确，本项只登记为测试精化候选，不为其启动第二轮。~~ → 非阻塞测试门维护池（2026-08-23 账面清理）

- ~~`[discovered 2026-08-11 during P3-18 G11 internal review]` **G11 curl 动词抽取仍不覆盖引号 + 反斜杠续行（P3，待 triage；本片 Ready，不阻塞）** —— 当前判据只在 URL 同一行、且动词位于 URL 前时识别 `-X/--request`；live `docs/api/swagger-guide.md` 已存在 `curl -X 'POST' \\` 后下一行写 URL 的形态，若把 POST 错改成 DELETE，门仍只核路径而假绿。后续应先把反斜杠续行归一成一条逻辑 curl 命令，再允许引号包裹的动词；不为测试门增强启动本轮第二次修复。~~ → 非阻塞测试门维护池（2026-08-23 账面清理）

- ~~`[discovered 2026-08-11 during P3-18 G11 internal review]` **G11 schema 原始类型比较尚未表达 JSON Schema 可赋值关系（P3，待 triage；本片 Ready，不阻塞）** —— 本片要收口的明确故障是 object/array 容器相反时仍只比 properties；当前 helper 同时严格比较全部显式 `type`，可能把 checked-in `number`、live `integer` 这类合法收窄误报为不兼容。后续可只对 object/array 容器做本门承诺的比较，或另片实现 JSON Schema assignability；不扩大当前边界。~~ → 非阻塞测试门维护池（2026-08-23 账面清理）
...
- ~~`[discovered 2026-08-06 during P1-45/46 立项，Codex #293 R2→R3 两轮才判对]` **P1-6 现场半（真 idle-close 复现）没有载体序列（P3）** —— ⚠️ **我判错过一次**：R2 时写成「F64 侧剧本，跟 `propsim_f64_state_machine` 一起排」，而 P1-6 的正式定义（`### P1-6`）是「**FS16 / UXM / ENA** silent-reconnect 集成测试」，条目原文明写「**F64 已有 12 个集成测试**，FS16 / UXM / ENA 继承了同一模式但没有各自的集成测试」——**我指向了唯一已经覆盖了的那个驱动**。正解：载体要打在 **FS16**（`propsim_fs16_health` 可扩）/ **UXM** / **ENA**（`vna_ena_health` 可扩）三个上，且它是 C 类（要制造 idle 再看重连），不是只读普查。~~ → 硬件 Blocked 表 P1-6（HOLD 行）（2026-08-23 账面清理）
- ~~`[discovered 2026-08-06 during P1-39 内审 F7 域枚举 —— 判定为越界，本片未做]` **「待归档执行」是第 4 个拿不到 ID 的界面（P3）** —— `gui/src/features/Reports/components/PendingExecutionsList.tsx` 的两张表都只把 `record.id` / `record.execution_id` 当 React `key`，列是 用例名/时长/完成日期/来源/操作，**跟 P1-39 改动前那三处一模一样**。它就在报告页第一个页签，跟系统日志同页并列。修法**已经现成**：套用本片的 `CopyableId` + `formatExecutionTag`，零新机制。⚠️ 同族第二例：`gui/src/features/TestManagement/README.md` 的用法示例写 `<TestManagement />`，没提 P1-39 新加的 `onViewLogs` —— 而同一份 README 里已有一段专门讲「`onCreateNew` 没人传所以入口不可达」，同一个坑的第二次。~~ → 非阻塞维护池（纯 UI 微调）（2026-08-23 账面清理）
- ~~`[discovered 2026-08-06 during P1-39 浏览器实测 —— pre-existing，判定为越界，本片未做]` **系统日志表「消息」列被挤到 83px，正文竖着排（P3）** —— 1280 视口实测各列宽：展开 32 / 时间 107 / 级别 70 / 请求 86 / 执行 86 / 模式 60 / **Logger 338** / **消息 83**。`Logger` 声明 `w={250}` 但被 `app.services.test_case_runner` 这类长 token 撑到 338，而**「消息」列没有 min-width 也不横向滚**（容器 `scrollWidth == clientWidth`），于是正文被压成一条竖带，等于读不了。**与 P1-39 无关**（该片一处列宽都没动，`git diff` 已核），但 P1-36 加「执行」列后更明显。修法候选：给消息列 `miw` + Logger 列 `truncate`/`ellipsis`，或整表加 `overflow-x: auto`（⚠ 后者要跟 sticky 表头一起验，别滚出错位）。~~ → 非阻塞维护池（纯 UI 微调）（2026-08-23 账面清理）
- ~~`[discovered 2026-08-06 during P1-39]` **执行快照名里的时间戳是 UTC，跟界面上所有其它时间差一个时区（P3）** —— `test_case_runner.py:134` 用 `datetime.utcnow().strftime('%Y%m%d-%H%M%S')` 拼进快照用例名（`S6-验收-五步闭环 [执行 20260806-005515]`），而 P1-34 已把界面时间统一成**本地时区**。实测同一次执行：名字里 `20260806-005515`，P1-39 新增的执行标签 `20260806-085515`，完成时间列 `2026/8/6 08:55` —— **两个数字并排，看着像自相矛盾**。P1-39 的标签取本地是对的（它要跟日志时间线和墙上的钟对上），**错的是那个名字**。⚠️ 修它要小心：那是**已落库的历史行的名字**，改生成逻辑只影响新行，存量不一致仍在；要不要回填是单独决定。~~ → 非阻塞维护池（纯 UI 微调）（2026-08-23 账面清理）
...
- ~~`[discovered 2026-08-03 during 立项 review, Codex #276 R2]` **`Uxm5GNRTestAppProfile`（非 IRAT 方言）的命令形式是独立问题 —— 出路是查手册，不是现场探** —— #275 把 KPI reader 换成新命令字段，只给 IRAT 填了 `BSE:` 形式；5G_NR_Test 方言继承基类的 `None`，那批 KPI 现在整组读不到（有 warn-once 兜着，不静默）。⚠️ **不能靠现场探**：该方言用的是**无前缀**形式，而手册给的两个变体（`BSE:MEASure:NR5G:...` / `BSE:NR5G:MEASure:...`）**都带 `BSE:`** —— 现场去试无前缀拼写就是猜，正是 #275 整片在治的病。**正解**：像 2026-08-03 查 IRAT 那样，直接查手册 / NotebookLM 问 5G_NR_Test 方言（Test Application 名 `5G_NR_Test`）下这批命令的根前缀与完整形式；查得到就本地补齐，查不到就如实标"该方言 KPI 不可用"并让 warn-once 继续响。**本地片，不需要现场**。~~ → Known unknown（手册查无 `ROUTe:…:ANTenna:PORT` / `:AMC:`，P1-58 设计稿 §2 已附出处）（2026-08-23 账面清理）
...
- ~~`[discovered 2026-08-03 during UXM KPI 修复, 用户明确要求记号]` **[→ 载体 = P1-31]** **⚠️ #275 的 KPI 回读改动 —— 本地零真机验证，整批必须现场核验** —— 命令形式 / 元素下标 / 单位 / 前置条件全部来自**手册与真机历史日志**，**没有一条在真机上跑过**。合并 #275 是"按手册应该对"的版本，不是"验过是对的"版本。**下次现场必须逐条对账**（走诊断序列，禁临时脚本）。⚠️ 原第 ⑩ 项「探出 5G 方言的无前缀形式」**已删**（Codex #276）——~~ → ✅ 2026-08-07 现场 P1-33 验证 14 条 MAC 命令全被接受；KPI 回读口径剩余部分 → 硬件 Blocked / Known unknown（2026-08-23 账面清理）
...
- ~~`[discovered 2026-08-03 during P1-30 内审 F4]` **取消 / `wait_for` 超时路径在 scpi.log 里零证据** —— `app/hal/base.py` 四处用的是 `except Exception`，而 `CancelledError` 在 Python 3.8+ **继承 `BaseException`**，抓不到。内审实跑探针：`asyncio.wait_for(driver._query("SLOW?"), 0.05)` 超时 → scpi 记录只有 `[('TX','TX: SLOW?')]`，**无 ERR**；`task.cancel()` 同样只有 TX。后果：P1-30 之后「被上层取消」与「coroutine 从未被 await」**仍是同一种签名**，分不开 —— 而现场超时恰恰是最常见的那一类。修法是**加机制**（`except BaseException: log; raise` —— 裸 `raise`，控制流零变化，取消展开期不 await，代价仅多一行日志），因「审查一轮里想加机制 = 停下来报告」（⓪⑤）**P1-30 只改了文档断言、机制没做**。代价不对称偏向补记：漏记 = 证据缺口延续，补记 = 一行。**小片，可与下一条同批。**~~ ✅ **P1-47A**（2026-08-07）（2026-08-23 账面清理）

- ~~`[discovered 2026-08-03 during P1-30 内审 F3]` **`OK` 行没有 `query` 字段，配对只能靠 msg 文本** —— `TX` / `OK` 两类记录都不带 `query`（`RX` / `ERR` 带）。而 TX 记在 `_scpi_lock` **之外**，并发与嵌套下 TX 与结果行**不相邻**（F64 超时排水时每条 `SYST:ERR?` 上界 64 次，各产生一对 TX/RX，才写外层 ERR）—— 所以"按行序配对"是错的读法，"按 `query` 配对"又对 TX/OK 不可用。给 `OK` 与 `TX` 补 `query` 字段属**加机制**，P1-30 未做（文档已改成"按 query 配对 + 说明何时不相邻"）。**小片。**~~ ✅ **P1-47A**（`exchange_id` 前置配对）（2026-08-23 账面清理）

- ~~`[discovered 2026-08-03 during P1-30 内审域枚举]` **两条 SCPI 日志旁路不经 base 模板，拿不到 P1-30 的任何改进** —— ① `app/hal/aerotech_positioner.py` 两处**自己直写 `_scpi_logger`**（不走 `_do_*`，因此无 OK/ERR/`resp_len`/`duration_ms`/截断标记）；② `app/api/instrument.py` 的 SCPI 控制台族 12 处，`instrument_id` 填的是 `category_key`、`direction` 用的是另一套 `CONNECT/WRITE/READ/ERROR` 值。P1-30 **刻意不碰**（aerotech 根本不用 `_do_*`，硬套模板是改驱动而非改日志），但 `GEMINI.md` 的四类记录表已补上这两条例外说明。要统一得逐条判「该不该走模板」。**中等，独立片。**~~ ✅ **P1-47A**（Aerotech `_send` 路径并入同一证据结构）（2026-08-23 账面清理）
...
- ~~`[discovered 2026-08-03 during P1-30]` **165 处 `except` 吞掉异常且不记日志不重抛（③ 静默分支普查）** —— ⚠️ **这个数没有唯一值，别引用具体数字去定范围** —— 口径稍变结果就变：全块扫描得 **162 / 43**，12 行窗口得 **165 / 51**，10 行窗口得 **185 / 42**（内审 F11 独立复算得 175 / 38）。量级是 **160–185 处**，其中整块只有 `pass` 的 **40–50 处**。真要动这片，先把口径钉死成一条能跑的命令：~~ → 非阻塞维护池（`app/hal/` 下由 G4 棘轮守住新增；其余按片顺带）（2026-08-23 账面清理）
...
- ~~`[discovered 2026-07-20 during 出发前门审 F5]` **UXM 幂等捷径生效后剩余写全在小区 ON 态执行 — ON 态同值写 band/duplex 是否触发 UXM 内部重配 (掉 DUT) 真机零实证**。ARFCN/功率有回读对账兜底; band/duplex ON 态被拒 (-221 类) 只进错误队列无对账项即静默。现场若"BW 已同仍重启"按 onsite-plan-20260721 风险⑧②排查; 正修方向 = band/duplex 也纳入幂等预读 (值同跳写) 或对账。TDD 主线 (n78) 幂等已限定 (F3 保守化: 仅 TDD + readback 能力位开才走捷径)。~~ → 硬件 Blocked / Known unknown（真机零实证）（2026-08-23 账面清理）
- ~~`[discovered 2026-07-20 during 三开关门审 #216]` **开关1 inherit 的"知情继承"只核对频率, 层数盲区**: 小区级层数继承仪器态但 RRC recon 仍按 TestCase 推层, CSI-RS 端口按 TestCase 层数算 — 三方可各不相同; Phase 6 读 UE 能力抓不到 cell 生效层数低于请求。正修方向 = read_live_frequency_identity 扩展读层数 (注意 #114 教训: 配置旋钮回读是 echo, 要找真生效读法) 或 inherit 下强制核对面板。当前缓解 = inherit 日志显式披露"层数未核对"。~~ → 硬件 Blocked / Known unknown（inherit 模式按 P1-47 设计保持 unknown）（2026-08-23 账面清理）
...
- ~~`[discovered on-site 2026-07-03 during RF体检]` **InputLevelController 闭环方向存疑**: "AUTOSET 失败→降功率"像按过载假设写的 (收不到信号应升不应降), 且起点 -10 dBm/SCS 语义过热 (BW100 满业务 = +25 dBm); UXM 510 告警实证被 HW 口限二次钳制。修法 = 闭环失败分支按"无信号/过载"分诊 + 起点从 TestCase 功率推导。待 promote (建议 P2, 半天)。~~ → 硬件 Blocked（P0-8 现场半，载体 `propsim_f64_p08_gate`）（2026-08-23 账面清理）
...
- ~~`[discovered 2026-08-11 during P3-19 UXM ARFCN 内审]` **[P2 / 测试，不阻塞] custom map 的 NR-ARFCN 域界保护未被定点回归直接锁住** —— live 已在驱动加载时拒绝 `<0` / `>3279165`，但本片参数化只覆盖文本、非整数浮点与 bool；若未来误删域检查，现有 focused 仍可能全绿。按测试规则只登记一次，不在本轮追加外审循环；后续可把 `-1` / `3279166` 加到同一参数表。~~ → 非阻塞测试门维护池（2026-08-23 账面清理）
- ~~`[discovered 2026-08-11 during PR #334 Codex 外审]` **[P2 / 测试，不阻塞] attach shared-defaults 定点只比较 schema，未直接观察 run fallback 引用** —— 外审确认运行功能正确，但若把 `run()` 改回同值硬编码，当前共享源测试与既有 baseline dispatch 都可能保持绿色。按测试意见一轮止损规则登记，不在 PR #334 启动第二轮；后续如处理，应用可变 sentinel/执行级注入证明 runtime 确实从共享源读取。~~ → 非阻塞测试门维护池（2026-08-23 账面清理）
...
- ~~`[discovered 2026-09-02 during P2-56 声明半]` **驱动里三处 TDD 文案在 ② 落地后会过时（P3）**
  —— ① `configure_mac_throughput_test` 的 fail-loud 分支「平台缺口（Discovered）」；
  ② `MAC_CFG_NO_EQUIVALENT["TDD_SLOT_PATTERN"]` 的「整体 fail-loud」；
  ③ `base_station_mac_profile.py` 里 LTE profile 的 TDD 字段注释「放开取值域属**现场半**」
  （归属与条数都错）。~~ ✅ **随 P2-56 ② 一并改掉（PR #446）**，三处 grep 残留实测均为 0。
  ⚠️ 留档一条记账教训：本条初版说「两处都写着『平台缺口（Discovered）』」是**假的**
  ——实测全文件只有 ① 写了，是把一处的措辞当成了两处的；③ 在另一个文件里，
  按文件名找的 grep 扫不到它。
- ~~`[discovered 2026-09-02 during P2-56 声明半]` **`api/openapi.yaml` 的 `BaseStationMacProfileCapability` 缺 `dimensions`，且 `additionalProperties: false`（P2）** ——
  ...
  **出口：待 triage**；改法是补 `BaseStationMacDimensionCapability` /
  ...
  ✅ **P2-69 / PR #473**：checked OpenAPI、generated TS 与手写类型已补齐，并以真实 CMW
  目录响应递归验约；merge `4ca4aba6`。
