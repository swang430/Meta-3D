...
**Current Focus = P3-19 / tail 反向扫描字节上限子片** —— P3-18 五片已由 PR #326–#330 全部合并。P3-19 先按风险与边界拆片：① tail 损坏/巨行文件的无界内存读取（当前片，边界最小）→ ② `is_docker_pid` 默认杀的安全方向反转 → ③ UXM 终止符/ARFCN 三项 → ④ mock-only 类型/handler → ⑤校准 warnings 两片 → ⑥日志/告警生产者。当前片只给 `_scan_reverse_entries` 增加 16 MiB 单请求字节预算；预算内找不到安全换行边界时明确 422，不返回假成功/不前进游标。`/tail` 与 `/history` 共用同一预算，不混入日志级别多选或生产告警。
...
| P3-19 | 日志/告警/留痕卫生批（tail 反向扫描字节上限 + 执行失败告警通道 + 校准 warnings DB 持久化 + 借用 acquire 的清理失败警告可见 + UXM 终止符大小写 + UXM ARFCN 三条加固 + mock-only 遗留响应类型与 handler 收口 + `is_docker_pid` 改 allowlist）——2026-08-02 拍板。**拆序**：tail 字节预算 → Docker PID 安全方向 → UXM 两片 → mock-only 类型 → warnings 两片 → 日志/告警生产者。⚠️ 原含的 app.log 噪声治理已于 2026-08-05 摘出提升为 P1-35。 | 🔄 |
...
- `[discovered 2026-08-11 during P3-19 首片 Codex 外审，P2，非本轮阻塞]` **字节预算可能从父 ERROR 行前切断超大 traceback 续行组** —— 当一组 RAW traceback 续行自身超过 16 MiB 时，扫描器可能先产生可推进 `next_offset`，随后因预算耗尽而未读到父 ERROR 行；按 `level=ERROR` 过滤会返回空页，而下一页只剩父行，已跨页的续行不再随父行返回。后续独立片应收窄为：预算耗尽且仍有 `pending_continuations` 时 fail-closed，禁止拆开逻辑日志组；本 P2 按外审轮次规则只登记一次，不阻塞 PR #331。
