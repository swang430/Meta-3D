...
**Current Focus = P1-56「转台动作真值门与诊断载体」（第二次 fresh 内审 4 条 P1 已按 TDD 修复，最终完整验证通过、待 fresh 复审，WIP=1）**。P1-55
...
| **P1-56** | 转台命令成功但编码器不动：本地动作真值门与诊断载体 | 🟡 第二次内审 4 条 P1（正式状态解析、settled 门、ABORT 失败止损、post-ABORT 真值）已修，最终验证通过、待 fresh 复审；物理现场验证保持 Hardware Blocked |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → ~~P1-50~~ ✅ → ~~P1-51~~ ✅ → ~~P1-52~~ ✅ → ~~P1-53~~ ✅ → ~~P2-26~~ ✅ → ~~P1-54~~ ✅ → ~~P1-55~~ ✅ → P1-56 🟡 → P2-27 → P2-28 → P2-29 → P2-30 → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；P1-55 已由 PR #349 合并。P1-56 已完成真实 Aerotech `MOVEABS/HOME` 的有限 PFBK 变化与目标容差门、cleanup False 告警，以及 `aerotech_positioner_motion_truth` 带/不带 XF raw 轨迹诊断。两次 fresh 内审累计 9 条 P1 已按 TDD 修复；第二轮重点收口正式/诊断共用严格 AXISSTATUS、最终 settled 门、ABORT 失败止损与 post-ABORT PFBK 真值。最新相关 145 项、rule gates 53 项与 GUI production build 通过；后端全量 3983 passed / 5 skipped，唯一既有 execution-context 顺序污染独立复跑 1 passed；compileall/diff-check 通过，当前待 fresh 复审，WIP=1。现场物理单位/方向/偏置/型号实证仍保持 Hardware Blocked。完整编号、范围与状态只看顶部 Current Focus 表。 |
