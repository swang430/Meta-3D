...
| U-13 | UXM 在现场 LTE_NR_IRAT Test Application 下，能否按手册的 Single + Length + progress-count 形成可重复、互不继承的原生有限窗口？ | 关联 Blocked 表 P2-52/P2-54。checked-in 受控载体 `uxm_native_window_truth` 已完成；现场在同一诊断租约内核对 `CONTinuous:ALL 0` / `LENGth:ALL` 的 requested 与行为观察、progress-count 精确到界、到界停住、连续两次不继承及 cleanup，外层审计核对 lease release。序列不发送无权威 IRAT 来源的错误查询，收尾恢复 `Continuous=1`；不发明查询形，所以不把 requested 写成 applied，也不声称设备接受。手册示例与 NotebookLM 摘要只证明 NSA/SA 文档路径，不替代 LTE_NR_IRAT 真机证据；真机运行前本项继续 Blocked。 |
