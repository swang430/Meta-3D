...
**Current Focus = P1-50（设计中，WIP=1）**。P1-49 已由 PR #342（merge `3e0a11d`）
...
| **P1-50** | 日志留存清理失败时禁止回流重开执行文件并泄漏 fd | 🔄 设计中 |
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **~~P2-25~~ ✅ → ~~P1-49~~ ✅ → P1-50 🔄 → P1-51 → P1-52 → P1-53 → P2-26 → P2-27 → P2-28 → P2-29 → P2-30 → P2-31 → P2-32 → P2-33 → P2-34 → P3-20 → P3-21**；当前 P1-50 WIP=1、设计中。完整编号、范围与状态只看顶部 Current Focus 表。 |
...
- `[discovered 2026-08-09 during #303 迟到外审]` **[→ 提升 P1-50；2026-08-16 设计中] 留存清理失败的告警会把它自己要清的文件重新打开 —— 该文件此后永不被清理 + 每次失败泄漏一个 fd** —— Codex P2，`api-service/app/core/logging_config.py:364` 附近。链条：`close_execution()` 在 `unlink()` 失败时发 WARNING → 调用方（用例执行 / HTTP / WebSocket 收尾）**此时仍持有当前 `execution_id`** → 告警传播到根 logger → `ContextFilter` 把这个 ID 注进记录 → 记录**回流进同一个 `ExecutionFileHandler`** → 重新打开刚关闭的 `exec-<execution_id>.log` 并把流留在 `_streams` 里 → 此后该文件被误判成"仍在写入"而**永远跳过清理**。修法采用**显式隔离执行上下文**：告警仍进入 app/console，但执行文件 handler 看到 `execution_id=-` 后忽略。⚠️ 保护门必须把待测 handler 挂到根 logger 复现真实回流，不能只用 `caplog` 捕获模块 logger。**来源与它为什么现在才被发现**：`af51abb`（"留存清理补两个洞 —— 运行期复查 + 告警不走 handleError"）是 #303 R2 findings 的修复，**它自己从未过外审** —— Codex R2 覆盖 `652ae18`，而 merge 时 HEAD 已是 `df33191`；"push 新 commit 不触发 Codex，必须显式再发一次"这个已知陷阱当时没被执行。2026-08-09 用户要求补审时，Codex 对 squash commit `1430a6e` 重审抓出。**建议**：跟 P1-48 同域（`logging_config.py`）但**不同事**（P1-48 是"分不出真假"，本条是"清理失效 + fd 泄漏"），按 ⑦ 不并片。
