...
| **P2-52 / P2-54 UXM 证据边界** | **UXM 原生有限窗口与 IRAT MAC 错误队列权威来源** | **2026-09-09 原始手册复核纠偏**：NotebookLM 所用的归档手册并非只说明 `CLEar`；`Examples > Measuring BLER` 明确给出 `CONTinuous:ALL 0`（Single）+ `LENGth:ALL 5000` + `STATe 1`，并规定结果首字段 `progress-count` 达到配置长度即 “measurement is done”。当前正式实现仍未冻结/下发/核验这组原生窗口参数，观察到的 `requested=3` 只是 TestCase/开发采样策略，不是 UXM 硬件要求；同时该章节只标 NSA/SA，LTE_NR_IRAT 适用性仍无书面依据。现场须确认当前 Test Application/选件接受 Single/Length、progress-count 到界且连续两次窗口不继承；任一项不成立均维持 `clear_read_only` / unknown。`uxm.error_queue` 仍 unverified，不能借 F64 目录或一次零错误响应变成 confirmed | ✅ 出发前受控载体已补：[`uxm_native_window_truth`](../api-service/app/diagnostics/sequences/uxm_native_window_truth.py) 固定 CELL1、需操作员确认，在同一诊断租约内跑两个 Single+Length 窗口并记录 requested、进度和 cleanup；它不发送无权威 IRAT 来源的 `SYSTem:ERRor?`，收尾只记录 `STATe 0` / `Continuous=1` 命令已发送，不发明查询形，也不声称设备接受、cleanup complete 或仪表可复用。只要实际写入，结果固定 `BLOCKED` / `success=false` / `formal_verdict=unverified`，并要求下一次正式执行前由操作员在仪表侧完整 preset/reset；HAL 重载、transport release/reconnect 均不能替代。⚠️ 现场真机证据仍未取得，本项保持 Blocked；旧 `uxm_window_boundary_probe` 只作零写查询辅助，不足以关闭本项 |
...
| U-13 | UXM 在现场 LTE_NR_IRAT Test Application 下，能否按手册的 Single + Length + progress-count 形成可重复、互不继承的原生有限窗口？ | 关联 Blocked 表 P2-52/P2-54。checked-in 受控载体 `uxm_native_window_truth` 已完成；现场在同一诊断租约内核对 `CONTinuous:ALL 0` / `LENGth:ALL` 的 requested 与行为观察、progress-count 精确到界、到界停住、连续两次不继承及 cleanup sent 状态。序列不发送无权威 IRAT 来源的错误查询，收尾尝试发送 `STATe 0` / `Continuous=1`，但只记 sent/unverified，不把 transport release 或 HAL reload 当复位。任何写入后须由操作员在仪表侧完整 preset/reset 才能复用 UXM；不发明查询形，不把 requested 写成 applied。手册示例与 NotebookLM 摘要只证明 NSA/SA 文档路径，不替代 LTE_NR_IRAT 真机证据；真机运行前本项继续 Blocked。 |
