...
**Current Focus = P3-19 / UXM SOCKET 终止符大小写子片** —— P3-19 的 tail 字节预算与端口清理 allowlist 已分别由 PR #331/#332 合并。当前片只统一 UXM 初次 connect 与 silent reconnect 的 VISA resource token 比较：`SOCKET` / `socket` 等大小写形式都设置既有 LF read/write termination；资源串仍原样交给 PyVISA，不新增或修改任何 SCPI 命令。下一片才处理 ARFCN custom map 三项。
...
| P3-19 | 日志/告警/留痕卫生批（~~tail 反向扫描字节上限~~ ✅ PR #331 + ~~端口清理进程 allowlist~~ ✅ PR #332 + 执行失败告警通道 + 校准 warnings DB 持久化 + 借用 acquire 的清理失败警告可见 + **当前片：UXM 终止符大小写** + UXM ARFCN 三条加固 + mock-only 遗留响应类型与 handler 收口）——2026-08-02 拍板。**拆序**：~~tail 字节预算~~ ✅ → ~~端口清理安全方向~~ ✅ → UXM 两片 → mock-only 类型 → warnings 两片 → 日志/告警生产者。⚠️ 原含的 app.log 噪声治理已于 2026-08-05 摘出提升为 P1-35。 | 🔄 |
