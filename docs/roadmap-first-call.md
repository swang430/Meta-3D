...
- `[discovered 2026-06-06 during probe_number 局部化, Codex audit on probe-number PR]` **校准表 probe_calibration.probe_id 无 chamber 维度**. probe_number 已改按 chamber 局部 (复合唯一 uq_probes_chamber_probe_number); 但 `probe_calibration.probe_id` (Integer, 非 FK) 用 probe_number 当 join key 且不带 chamber_config_id → 多 chamber 校准数据会在相同 probe_id 上冲突 (如 CAICT-FS #1 vs 3GPP-16 #1)。当前只单 chamber 有校准数据, 不破; 将来多 chamber 校准需给校准表加 chamber 维度 (probe_calibration 模型 + channel_engine_client._query_calibration_entries + seed_dummy_calibration)。预存缺口, 局部化放大了它, 非本次解决。
