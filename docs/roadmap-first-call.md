...
P0-9 关闭后开放的 P0 只剩 P0-5（UXM 5G NR，等现场），按治理规则 2 焦点降到非现场队列：用户 2026-09-17 批准先做「现场改动落地」（已由 PR #480 完成），2026-09-18 批准其后按队列继续，**当前片 = P1-80**（队列第 1 项 P2-74 的本地半已由 PR #482 合并；见下方 Current Focus（非现场）行）。
...
Current Focus（非现场）= **P1-80：P1-2 许可序列修复**（现场改动落地已由 PR #480 合并，merge `7aa652da`；用户 2026-09-18 批准按队列继续；队列第 1 项 P2-74 的本地半已由 PR #482 合并（merge `4d6050fb`，2026-09-19），现场半待复验）。队列：P1-80（P1-2 许可序列修复）→ P2-73（`propsim_f64_p08_gate` 限 UXM）→ P2-75（`instrument_idn_sweep` 范围与身份）→ P2-76（`input_level_calibration` 空回读不得记成功）→ P2-77（信道资产频率语义，设计先行、须先有手册出处）；各片照常走「设计稿 → 用户过目 → 实现 → 内审 / 外审」，WIP = 1。P2-71 调研已形成[证据裁决](plans/2026-09-13-p2-71-f64-file-provisioning-research.md)，设备侧仍待现场；P2-57 / P2-70 已由 PR #476 / #475 合并。**
...
| 桶 | 内容 |
|----|------|
| **LOCAL-OPEN (roadmap 内)** | **当前唯一 WIP：P1-80（P1-2 许可序列修复）**；其后 P2-73 → P2-75 → P2-76 → P2-77（用户 2026-09-18 批准的顺序；2026-09-16 现场改动落地已由 PR #480 合并；队列第 1 项 P2-74 的本地半已由 PR #482 合并（merge `4d6050fb`，2026-09-19），现场半待复验）。P1-79A～E 为低优先级候选，不在队列内。P2-68 / P2-69 / P2-70 已由 PR #471 / #473 / #475 合并；P2-57 静态资产来源声明和注册对账由 PR #476 合并，`.smu` 拓扑仍归 ChannelAsset 独立待评估。P2-71 经用户启动，非现场调研见[结论](plans/2026-09-13-p2-71-f64-file-provisioning-research.md)；设备侧发布/验约仍 Hardware Blocked，不自动开始实现。P1-77 / P1-76 已按 P3-23 记录试行数据。P2-32 位于功能启用池，P3-20/P3-21 位于非阻塞维护池，均不得自动启动。现场静区线性 XY 扫描平台仍保持 Hardware Blocked。 |
...
| ~~P0-8~~ ✅ **2026-09-16 现场完成** | ~~F64 driver 现场修复落地~~ —— real F64 上 load→run→改参全 0 error + 输入口变绿 + DL 不失真 | **P0-8b ✅**：执行 `dbd53e6f` 在 SCME UMa 衰落下 DL ACK 100 % / BLER 0 %，吞吐 45.82 Mbps，且 NACK 0 % / DTX 0 % —— 调度的传输块全部被确认，即该调度下的满额（原始回复见[现场总结](site-debug/2026-09-16-cmw500-f64-onsite-summary.md) §3）；最难的衰落态已满额，bypass 半不再单独要求。✅ **2026-08-27 P0-8a 历史现场证据已通过**：LTE UMa 20 MHz SMU 加载/运行，输入侧测到 CMW500 信号，配置链正常。仅余 P0-8b：DUT Attach 后证明 DL 非 0% ACK/不失真 | ⚠️ 当前 [`propsim_f64_p08_gate`](../api-service/app/diagnostics/sequences/propsim_f64_p08_gate.py) 硬编码 UXM 满 RB DL 前提，不是 CMW500 复验载体，不得虚假勾选 `uxm_dl_confirmed`。CMW500 当日复验用 P0-9 同一 LTE MIMO_OTA TestCase；诊断契约缺口见 2026-09-16 Discovered 条目 |
| P1-2 | F64 license probe SCPI 现场验证 | ⚠️ **2026-09-16 现场发现阻塞，不能关闭**：同机连续两次运行 `e916b910` / `c3a445a2` 均在第一轮已排空队列后复现同一 `-200 No simulation opened` + `-100 ATE command not supported`。许可真值 `SYSTem:INFO?` 已确认且与驱动 `INT-GEN` 对账一致，但 CLOSED 下 `CALIBration:VALid?` 空回复、`USER:GET?` 空回复均被错误显示为绿色；当前校准有效性与用户对齐状态未确认 | ✅ 载体仍为 [`propsim_f64_license_truth`](../api-service/app/diagnostics/sequences/propsim_f64_license_truth.py)，但必须先修：错误逐查询归属、空回复语义 fail-closed、license/calibration/user-alignment 分判；不新增/猜测 SCPI。修后在同一 F8800A 上从干净队列复验，只有有效子判决 + 终态零残留才关闭。现场原始证据见 [`guides/2026-09-16-cmw500-f64-onsite-runbook.md`](guides/2026-09-16-cmw500-f64-onsite-runbook.md#propsim_f64_license_truth-%E6%8A%8A%E9%94%99%E8%AF%AF%E7%A9%BA%E5%9B%9E%E5%A4%8D%E6%98%BE%E7%A4%BA%E4%B8%BA%E6%88%90%E5%8A%9F) |
...
（2026-09-19 更正：原句写「用 `aerotech_positioner_motion_truth` 的原始时间序列记录三个未知」，而该序列只发 ≤ 30° 小步、
每 0.2 s 采样、线路从不空闲，也不含 `WAIT MODE`，答不了其中任何一个。）`HOME` 同样是阻塞命令、同样受这 10 s 限制，本片未改。
...
| D2 `propsim_f64_license_truth` 空回复显示成功 | → **P1-80**（已批准，队列第 2；即 P1-2 的软件半） | 两次运行稳定复现；修后同机复验 |
| D3 `propsim_f64_p08_gate` 硬编码 UXM | → **P2-73**（已批准，队列第 3） | 最小修复：按服务器权威 BaseStation 身份 fail-closed 为 UXM-only |
| D8 Aerotech 传输 reset / 结局未知 / 单轴 `PFBK(Y)` ERROR 噪音 | → **P2-74**（本地半 ✅ PR #482；现场半待复验） | 根因 = 阻塞命令撞上套接字空闲超时；按段下发，安全判据未放松 |
| D1 `instrument_idn_sweep` 范围与身份契约 | → **P2-75**（已批准，队列第 4） | 「所选 LabProfile binding × 当前启用类别」+ 结构化身份投影 |
| D9 `input_level_calibration` 空回读仍记 `success: true` | → **P2-76**（已批准，队列第 5） | 功率观察窗已补后置真值，字段本身仍须 fail-closed |
| D10 `.smu` 中心频可被 API 改写，资产频率不排他 | → **P2-77**（已批准，队列第 6；设计先行） | 开工条件：取得并记录可核对的 PROPSIM 手册章节 / 页码或原文，说明 `CENT` 可改范围的限制（一次现场接受不算依据）；查不到就保持现有 fail-closed 语义。查到后再定「默认值 vs 本次执行真值」 |
...
- `[discovered 2026-09-16 during CMW500 + F64 现场调试]` **`instrument_idn_sweep` 把停用的旧 LabProfile binding 纳入扫描，并把已加载真实驱动的身份接口差异误报成仪表无响应（→ P2-75，2026-09-18 已批准）** —— 现场 `diagnostic_run_id=94a93c8a-9035-4190-acef-baea28b41cad` 报 `7/7` 失败；当前仪器目录实际仅启用 BaseStation、Channel Emulator、Positioner、RF Switch 四类，而 `CAICT-Lab-1.instrument_bindings` 仍有七类，且 VNA endpoint 已与目录漂移。四个已加载驱动均在 `0 ms` 因序列只探测 `get_identity/query_idn/query` 而失败，未发生仪器 I/O；CMW500、F64/UXM 已有 adapter 结构化身份/环境快照，转台明确没有安全型号/固件查询，RF Switch 尚无统一身份投影。后续设计须先列全目标选择、租约、身份产生方、GUI/审计消费方：默认使用“所选 LabProfile binding × 当前启用类别”的受控范围并显式列出排除/漂移项；以统一只读结构化身份投影输出 loaded adapter、endpoint、observed identity 及 `match/mismatch/unknown/not_applicable`，服务器核对 selected model，不开放任意底层 `query("*IDN?")`、不新增/猜测身份 SCPI。测试必须覆盖真实 CMW500/UXM/F64/转台/RF Switch 接口形状及“七条历史 binding、四类启用”的反例；现场原始证据见 [`guides/2026-09-16-cmw500-f64-onsite-runbook.md`](guides/2026-09-16-cmw500-f64-onsite-runbook.md#instrument_idn_sweep-%E8%8C%83%E5%9B%B4%E4%B8%8E%E8%BA%AB%E4%BB%BD%E8%AF%BB%E5%8F%96%E5%A5%91%E7%BA%A6%E5%A4%B1%E6%95%88)。本条尚未升格为正式 backlog，不自动启动。
- `[discovered 2026-09-16 during CMW500 + F64 现场调试；并入 P1-2]` **`propsim_f64_license_truth` 把会入错误队列的空回复显示为成功，许可成立却可能把校准/用户对齐未知误写成绿色（P2，→ P1-80，2026-09-18 已批准）** —— 同机连续运行 `e916b910-a7cf-4d1e-a89c-54941c4e6ccd` / `c3a445a2-cad7-427f-8567-1b6d2cf20c40`，第一轮已排空队列后第二轮仍精确复现 `-200 No simulation opened` + `-100 ATE command not supported`。当日 CLOSED 下 `CALIBration:VALid?` 回 `""`；2026-08-27 同机 `4aa35f2b-d240-409c-8158-776015a173a4` 在 STOPPED 下该查询回 `1,0` 且尾队列只剩 `-100`，故新增 `-200` 可归到 CLOSED 下的 VALid 查询。三次 `USER:GET?` 均为空，历史 STOPPED 运行仍留下唯一 `-100`，故本机空串不能解释为“未启用用户对齐”。当前 `Recorder.read` 只按 `raw is not None` 判成功，空串直接打 `✓`；最终 SUCCESS 分支也允许 `in_use=None, valid=None`。修复边界：归档开场 residue 后逐查询对账错误队列，空串仅在零错误且符合手册值域时成立；license/calibration/user-alignment 分判，许可成功不得补真校准；按已观察的状态/许可收窄查询，不新增/猜测 SCPI，不把本机结论推广到所有 F64。修后须在同机干净队列复验有效子判决与零残留；完整原始事实见 [`guides/2026-09-16-cmw500-f64-onsite-runbook.md`](guides/2026-09-16-cmw500-f64-onsite-runbook.md#propsim_f64_license_truth-%E6%8A%8A%E9%94%99%E8%AF%AF%E7%A9%BA%E5%9B%9E%E5%A4%8D%E6%98%BE%E7%A4%BA%E4%B8%BA%E6%88%90%E5%8A%9F)。
