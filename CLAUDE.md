...
**外审最多两轮；第二轮无 P1 就收口**（2026-08-12 用户定）：R1 用于核实并处理本片内
可执行意见，随后触发最后一轮 R2。R2 后不发 R3；R2 无 P1 立即合，若仍有 P1，修完并过
内审/回归后如实记录尾部修复未获外审覆盖，再收口。R1 的本片内 P2 可在 R2 前收口；
R2 的 P2/P3 只报告一次，**不自动进入 Discovered / backlog**。完整判据见 AGENTS.md 第 10 条。
